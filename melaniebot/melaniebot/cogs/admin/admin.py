...
GENERIC_FORBIDDEN = "I attempted to do something that Discord denied me permissions for. Your command failed to successfully complete."

HIERARCHY_ISSUE_ADD = (
    "I can not give {role.name} to {member.display_name} because that role is higher than or equal to my highest role in the Discord hierarchy."
)

HIERARCHY_ISSUE_REMOVE = (
    "I can not remove {role.name} from {member.display_name} because that role is higher than or equal to my highest role in the Discord hierarchy."
)

ROLE_HIERARCHY_ISSUE = "I can not edit {role.name} because that role is higher than my or equal to highest role in the Discord hierarchy."

USER_HIERARCHY_ISSUE_ADD = (
    "I can not let you give {role.name} to {member.display_name} because that role is higher than or equal to your highest role in the Discord hierarchy."
)

USER_HIERARCHY_ISSUE_REMOVE = (
    "I can not let you remove {role.name} from {member.display_name} because that role is higher than or equal to your highest role in the Discord hierarchy."
)

ROLE_USER_HIERARCHY_ISSUE = "I can not let you edit {role.name} because that role is higher than or equal to your highest role in the Discord hierarchy."

NEED_MANAGE_ROLES = _('I need the "Manage Roles" permission to do that.')

RUNNING_ANNOUNCEMENT = "I am already announcing something. If you would like to make a different announcement please use `{prefix}announce cancel` first."
...
class Admin(commands.Cog):
    ...
    def __init__(self, bot) -> None:
        ...
        self.config.register_guild(announce_channel=None, selfroles=[])  # Integer ID  # List of integer ID's
    ...
    async def _addrole(self, ctx: commands.Context, member: discord.Member, role: discord.Role, *, check_user=True):
        ...
        try:
            ...
            await ctx.send(f"I successfully added {role.name} to {member.display_name}")
    ...
    async def _removerole(self, ctx: commands.Context, member: discord.Member, role: discord.Role, *, check_user=True):
        ...
        try:
            ...
            await ctx.send(f"I successfully removed {role.name} from {member.display_name}")
    ...
    async def addrole(self, ctx: commands.Context, rolename: discord.Role, *, user: discord.Member = None):
        ...
        Use double quotes if the role contains spaces. If user is left
        blank it defaults to the author of the command.
    ...
    async def selfroleset_add(self, ctx: commands.Context, *roles: discord.Role):
        """Add a role, or a selection of roles, to the list of available
        selfroles.
        ...
        for role in roles:
            ...
            if role.id not in current_selfroles:
                ...
                await ctx.send(_('The role "{role.name}" is already a selfrole.').format(role=role))
                return

        await self.config.guild(ctx.guild).selfroles.set(current_selfroles)
        if (count := len(roles)) > 1:
            message = f"Added {count} selfroles."
        else:
    ...
    async def selfroleset_remove(self, ctx: commands.Context, *roles: SelfRole):
        """Remove a role, or a selection of roles, from the list of available
        selfroles.
        ...
        if (count := len(roles)) > 1:
            message = f"Removed {count} selfroles."
        else:
    ...
    async def on_guild_join(self, guild: discord.Guild):
        if await self.config.serverlocked():
            if len(self.bot.guilds) == 1:  # will be 0 once left
                log.warning(
                    f"Leaving guild '{guild.name}' ({guild.id}) due to serverlock. You can temporarily disable serverlock by starting up the bot with the --no-cogs flag.",
                )
            else:
