...
class AliasEntry:
    ...
    def __init__(self, name: str, command: Union[tuple[str], str], creator: int, guild: Optional[int]) -> None:
        super().__init__()
        self.name = name
...
class AliasCache:
    def __init__(self, config: Config, cache_enabled: bool = True) -> None:
        self.config = config
    ...
    async def anonymize_aliases(self, user_id: int):
        ...
        async for guild_id, guild_data in AsyncIter(all_guilds.items(), steps=20):
    ...
    async def get_alias(self, guild: Optional[discord.Guild], alias_name: str) -> Optional[AliasEntry]:
        """Returns an AliasEntry object if the provided alias_name is a registered
        alias.
        """
        server_aliases: list[AliasEntry] = []
        ...
        if self._cache_enabled:
            ...
            if guild is not None and guild.id in self._aliases and alias_name in self._aliases[guild.id]:
    ...
    def format_command_for_alias(command: str) -> str:
        if indices := findall(r"{(\d*)}", command):
            try:
                ...
                msg = "Arguments must be specified with a number."
                raise ArgParseError(msg) from e
            low = min(indices)
            indices = [a - low for a in indices]
            high = max(indices)
            if gaps := set(indices).symmetric_difference(range(high + 1)):
                raise ArgParseError("Arguments must be sequential. Missing arguments: " + ", ".join(str(i + low) for i in gaps))
