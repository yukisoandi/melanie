...
from melaniebot.core import VersionInfo
...
class Installable(RepoJSONMixin):
    ...
    def __init__(self, location: Path, repo: Optional[Repo] = None, commit: str = "") -> None:
    ...
    async def copy_to(self, target_dir: Path) -> bool:
        """Copies this cog/shared_lib to the given directory. This will overwrite
        ...
        try:
            ...
            log.exception("Error occurred when copying path: {}", self._location)
            return False
        return True
...
class InstalledModule(Installable):
    """Base class for installed modules, this is basically instance of installed
    `Installable` used by Downloader.
    ...
    def __init__(self, location: Path, repo: Optional[Repo] = None, commit: str = "", pinned: bool = False, json_repo_name: str = "") -> None:
