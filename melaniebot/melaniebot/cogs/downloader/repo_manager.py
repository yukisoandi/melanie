...
from melaniebot.core import Config, commands, data_manager
from melaniebot.core.utils._internal_utils import safe_delete
...
class _RepoCheckoutCtxManager(Awaitable[None], AsyncContextManager[None]):  # pylint: disable=duplicate-bases
    def __init__(self, repo: Repo, rev: Optional[str], exit_to_rev: Optional[str] = None, force_checkout: bool = False) -> None:
        self.repo = repo
        self.rev = rev
...
class Repo(RepoJSONMixin):
    ...
    AMBIGUOUS_ERROR_REGEX = re.compile(r"^hint: {3}(?P<rev>[A-Za-z0-9]+) (?P<type>commit|tag) (?P<desc>.+)$", re.MULTILINE)

    def __init__(self, name: str, url: str, branch: Optional[str], commit: str, folder_path: Path, available_modules: tuple[Installable, ...] = ()) -> None:
        self.url = url
        self.branch = branch
        self.commit = commit
    ...
    async def is_ancestor(self, maybe_ancestor_rev: str, descendant_rev: str) -> bool:
        ...
        if stderr.startswith(("fatal: Not a valid object name", "fatal: Not a valid commit name")):
            ...
            msg = f"Revision {rev} cannot be found."
            raise errors.UnknownRevision(msg, git_command)

        msg = f"Git failed to determine if commit {maybe_ancestor_rev} is ancestor of {descendant_rev} for repo at path: {self.folder_path}"
        raise errors.GitException(msg, git_command)
    ...
    async def _get_file_update_statuses(self, old_rev: str, new_rev: Optional[str] = None) -> dict[str, str]:
        """Gets the file update status letters for each changed file between the
        two revisions.
        ...
        if p.returncode != 0:
            msg = f"Git diff failed for repo at path: {self.folder_path}"
            raise errors.GitDiffError(msg, git_command)
    ...
    async def get_last_module_occurrence(self, module_name: str, descendant_rev: Optional[str] = None) -> Optional[Installable]:
        ...
        p = await self._run(
            ProcessFormatter().format(self.GIT_CHECK_IF_MODULE_EXISTS, path=self.folder_path, rev=descendant_rev, module_name=module_name),
            debug_only=True,
        )
        if p.returncode == 0:
        ...
        if p.returncode != 0:
            msg = f"Git log failed for repo at path: {self.folder_path}"
            raise errors.GitException(msg, git_command)
    ...
    async def get_modified_modules(self, old_rev: str, new_rev: Optional[str] = None) -> tuple[Installable, ...]:
        """Gets modified modules between the two revisions. For every module that
        doesn't exist in :code:`new_rev`, it will try to find last commit,
    ...
    async def _get_commit_notes(self, old_rev: str, relative_file_path: str) -> str:
        ...
        if p.returncode != 0:
            msg = f"An exception occurred while executing git log on this repo: {self.folder_path}"
            raise errors.GitException(msg, git_command)
    ...
    async def get_full_sha1(self, rev: str) -> str:
        ...
        if p.returncode != 0:
            ...
            for substring in ambiguous_errors:
                ...
                msg = f"Revision {rev} cannot be found."
                raise errors.UnknownRevision(msg, git_command)
            if candidates := [Candidate(match["rev"], match["type"], match["desc"]) for match in self.AMBIGUOUS_ERROR_REGEX.finditer(stderr, pos)]:
                msg = f"Short SHA1 {rev} is ambiguous."
                raise errors.AmbiguousRevision(msg, git_command, candidates)
            msg = f"Revision {rev} cannot be found."
            raise errors.UnknownRevision(msg, git_command)
    ...
    async def _run(self, *args: Any, valid_exit_codes: tuple[int, ...] = (0,), debug_only: bool = False, **kwargs: Any) -> CompletedProcess:
        ...
        async with self._repo_lock:
            ...
            if stderr := p.stderr.decode(encoding="utf-8", errors="replace").strip():
    ...
    async def _checkout(self, rev: Optional[str] = None, force_checkout: bool = False) -> None:
        ...
        if not exists:
            msg = f"A git repo does not exist at path: {self.folder_path}"
            raise errors.MissingGitRepo(msg)

        git_command = ProcessFormatter().format(self.GIT_CHECKOUT, path=self.folder_path, rev=rev)
        p = await self._run(git_command)

        if p.returncode != 0:
            msg = f"Could not checkout to {rev}. This revision may not exist"
            raise errors.UnknownRevision(msg, git_command)
    ...
    async def clone(self) -> tuple[Installable, ...]:
        ...
        if exists:
            msg = f"A git repo already exists at path: {path}"
            raise errors.ExistingGitRepo(msg)
        ...
        if p.returncode:
            ...
            msg = "Error when running git clone."
            raise errors.CloningError(msg, git_command)
    ...
    async def current_branch(self) -> str:
        ...
        if p.returncode != 0:
            msg = f"Could not determine current branch at path: {self.folder_path}"
            raise errors.GitException(msg, git_command)
    ...
    async def current_commit(self) -> str:
        ...
        if p.returncode != 0:
            msg = "Unable to determine commit hash."
            raise errors.CurrentHashError(msg, git_command)
    ...
    async def latest_commit(self, branch: Optional[str] = None) -> str:
        ...
        if p.returncode != 0:
            msg = "Unable to determine latest commit hash."
            raise errors.CurrentHashError(msg, git_command)
    ...
    async def current_url(self, folder: Optional[Path] = None) -> str:
        ...
        if p.returncode != 0:
            msg = "Unable to discover a repo URL."
            raise errors.NoRemoteURL(msg, git_command)
    ...
    async def hard_reset(self, branch: Optional[str] = None) -> None:
        ...
        if p.returncode != 0:
            msg = f"Some error occurred when trying to execute a hard reset on the repo at the following path: {self.folder_path}"
            raise errors.HardResetError(msg, git_command)
    ...
    async def update(self) -> tuple[str, str]:
        ...
        if p.returncode != 0:
            msg = f"Git pull returned a non zero exit code for the repo located at path: {self.folder_path}"
            raise errors.UpdateError(msg, git_command)
    ...
    async def install_cog(self, cog: Installable, target_dir: Path) -> InstalledModule:
        ...
        if cog not in self.available_cogs:
            msg = "That cog does not exist in this repo"
            raise errors.DownloaderException(msg)

        if not target_dir.is_dir():
            msg = "That target directory is not actually a directory."
            raise ValueError(msg)

        if not target_dir.exists():
            msg = "That target directory does not exist."
            raise ValueError(msg)

        if not await cog.copy_to(target_dir=target_dir):
            msg = "There was an issue during copying of cog's files"
            raise errors.CopyingError(msg)
    ...
    async def install_libraries(
        ...
        if libraries:
            if any(i not in self.available_libraries for i in libraries):
                msg = "Some given libraries are not available in this repo."
                raise ValueError(msg)
        else:
    ...
    async def install_raw_requirements(self, requirements: Iterable[str], target_dir: Path) -> bool:
        ...
        if p.returncode != 0:
            log.exception(f'Something went wrong when installing the following requirements: {", ".join(requirements)}')
...
class RepoManager:
    ...
    def validate_and_normalize_repo_name(name: str) -> str:
        if not name.isidentifier():
            msg = "Not a valid Python variable name."
            raise errors.InvalidRepoName(msg)
        return name.lower()
    ...
    async def add_repo(self, url: str, name: str, branch: Optional[str] = None) -> Repo:
        ...
        if self.does_repo_exist(name):
            msg = "That repo name you provided already exists. Please choose another."
            raise errors.ExistingGitRepo(msg)
    ...
    async def delete_repo(self, name: str) -> None:
        ...
        if repo is None:
            msg = f"There is no repo with the name {name}"
            raise errors.MissingGitRepo(msg)
    ...
    async def update_repos(self, repos: Optional[Iterable[Repo]] = None) -> tuple[dict[Repo, tuple[str, str]], list[str]]:
        """Calls `Repo.update` on passed repositories and catches failing ones.
        ...
        for repo in repos:
            try:
                ...
                log.exception("Repository {}  failed to update. URL: '{}'on branch '{}' ", repo.name, repo.url, repo.branch)
    ...
    async def _load_repos(self, set_repos: bool = False) -> dict[str, Repo]:
        ...
        for folder in self.repos_folder.iterdir():
            ...
            try:
                ...
                log.warning("A remote URL does not exist for repo {}", folder.stem)
            except errors.DownloaderException as err:
                log.opt(exception=err).error("Ignoring repo {} due to error.", folder.stem)
