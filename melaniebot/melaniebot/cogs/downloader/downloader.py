...
from melaniebot.core.utils.chat_formatting import box, humanize_list, inline, pagify
...
class Downloader(commands.Cog):
    ...
    Community cogs, also called third party cogs, are not included in
    the default Melanie install.

    Community cogs come in repositories. Repos are a group of cogs you
    can install. You always need to add the creator's repository using
    the `;repo` command before you can install one or more cogs from the
    creator.
    ...
    def create_init_task(self):
        def _done_callback(task: asyncio.Task) -> None:
            exc = task.exception()
            if exc is not None:
                log.opt(exception=exc).error("An unexpected error occurred during Downloader's initialization.")
    ...
    async def _schema_0_to_1(self):
        """This contains migration to allow saving state of both installed cogs
        ...
        # but it only helps `;cog update` run faster so it's not an issue
    ...
    async def _remove_from_installed(self, modules: Iterable[InstalledModule]) -> None:
        """Remove modules from the saved list of installed modules (corresponding
        to type of module).
    ...
    async def pipinstall(self, ctx: commands.Context, *deps: str) -> None:
        ...
        **Arguments**
        ...
        if success:
            await ctx.send("Libraries installed." if len(deps) > 1 else ("Library installed."))
        else:
            await ctx.send(
                "Some libraries failed to install. Please check your logs for a complete list."
                if len(deps) > 1
                else ("The library failed to install. Please check your logs for a complete list."),
            )
    ...
    async def _repo_add(self, ctx: commands.Context, name: str, repo_url: str, branch: str = None) -> None:
        ...
        **Arguments**
        ...
        if name.startswith(".") or name.endswith("."):
            await ctx.send("Repo names cannot start or end with a dot.")
            return
        if re.match(r"^[a-zA-Z0-9_\-\.]+$", name) is None:
            await ctx.send("Repo names can only contain characters A-z, numbers, underscores, and hyphens.")
            return
        try:
            async with ctx.typing():
                # noinspection PyTypeChecker
            ...
            await ctx.send("The repo name you provided is already in use. Please choose another name.")
        except errors.CloningError:
            await ctx.send("Something went wrong during the cloning process. See logs for more information.")
            log.opt(exception=True).exception("Something went wrong whilst cloning {} (to revision: {})", repo_url, branch)
        except OSError:
            log.exception("Something went wrong trying to add repo {} under name {}", repo_url, name)
            await ctx.send("Something went wrong trying to add that repo. See logs for more information.")
        else:
            await ctx.send(f"Repo `{name}` successfully added.")
            if repo.install_msg:
                await ctx.send(repo.install_msg.replace(";", ctx.clean_prefix))
    ...
    async def _repo_del(self, ctx: commands.Context, *repos: Repo) -> None:
        ...
        --------
            - `;repo delete 26-Cogs`
            - `;repo delete 26-Cogs Laggrons-Dumb-Cogs`
        ...
        **Arguments**
        ...
        await ctx.send(
            ("Successfully deleted repos: " if len(repos) > 1 else ("Successfully deleted the repo: ")) + humanize_list([inline(i.name) for i in set(repos)]),
        )
    ...
    async def _repo_list(self, ctx: commands.Context) -> None:
        ...
        if len(repos) == 0:
            ...
            joined = "Installed Repos:\n\n" if len(repos) > 1 else "Installed Repo:\n\n"
    ...
    async def _repo_info(self, ctx: commands.Context, repo: Repo) -> None:
        ...
        **Arguments**
        ...
        """
        made_by = ", ".join(repo.author) or ("Missing from info.json")

        information = f"Repo url: {repo.clean_url}\n"
        if repo.branch:
            information += f"Branch: {repo.branch}\n"
        information += f"Made by: {made_by}\nDescription:\n{repo.description or ''}"

        msg = f"Information on {inline(repo.name)} repo:{box(information)}"
    ...
    async def _repo_update(self, ctx: commands.Context, *repos: Repo) -> None:
        ...
        --------
            - `;repo update`
            - `;repo update 26-Cogs`
            - `;repo update 26-Cogs Laggrons-Dumb-Cogs`
        ...
        **Arguments**
        ...
        async with ctx.typing():
            ...
            if updated := {repo.name for repo in updated_repos}:
                message = "Repo update completed successfully."
                message += "\nUpdated: " + humanize_list(tuple(map(inline, updated)))
            elif not repos:
                message = "All installed repos are already up to date."
            elif len(updated_repos) > 1:
                message = "These repos are already up to date."
            else:
                message = "This repo is already up to date."
    ...
    async def _cog_reinstallreqs(self, ctx: commands.Context) -> None:
        """This command should not be used unless Melanie specifically asks for
        it.

        This command will reinstall cog requirements and shared
        libraries for all installed cogs.

        Melanie might ask the owner to use this when it clears contents
        of the lib folder because of change in minor version of Python.
        ...
        message = ""
        if failed_reqs:
            message += ("Failed to install requirements: " if len(failed_reqs) > 1 else ("Failed to install the requirement: ")) + humanize_list(
                tuple(map(inline, failed_reqs)),
            )
        if all_failed_libs:
            libnames = [lib.name for lib in failed_libs]
            message += ("\nFailed to install shared libraries: " if len(all_failed_libs) > 1 else ("\nFailed to install shared library: ")) + humanize_list(
                tuple(map(inline, libnames)),
            )
        if message:
            await self.send_pagified(
                ctx,
                "Cog requirements and shared libraries for all installed cogs have been reinstalled but there were some errors:\n" + message,
            )
        else:
            await ctx.send("Cog requirements and shared libraries for all installed cogs have been reinstalled.")
    ...
    async def _cog_install(self, ctx: commands.Context, repo: Repo, *cog_names: str) -> None:
        ...
        Examples
        --------
            - `;cog install 26-Cogs defender`
            - `;cog install Laggrons-Dumb-Cogs say roleinvite`
    ...
    async def _cog_installversion(self, ctx: commands.Context, repo: Repo, revision: str, *cog_names: str) -> None:
        ...
        Example:
        -------
            - `;cog installversion Broken-Repo e798cc268e199612b1316a3d1f193da0770c7016 cog_name`
        ...
        async with ctx.typing():
            if rev is not None:
                try:
                    ...
                    msg = f"Error: short sha1 `{rev}` is ambiguous. Possible candidates:\n"
                    for candidate in e.candidates:
                        msg += f"**{candidate.object_type} {candidate.rev}** - {candidate.description}\n"
                    await self.send_pagified(ctx, msg)
                    return
                except errors.UnknownRevision:
                    await ctx.send(f"Error: there is no revision `{rev}` in repo `{repo.name}`")
                    return
            ...
            async with repo.checkout(commit, exit_to_rev=repo.branch):
                ...
                failed_reqs = await self._install_requirements(cogs)
                if failed_reqs:
                    message += ("\nFailed to install requirements: " if len(failed_reqs) > 1 else ("\nFailed to install the requirement: ")) + humanize_list(
                        tuple(map(inline, failed_reqs)),
                    )
                    await self.send_pagified(ctx, message)
                    return
            ...
            if repo.available_libraries:
                DEPRECATION_NOTICE.format(repo_list=inline(repo.name))
            installed_libs, failed_libs = await repo.install_libraries(target_dir=self.SHAREDLIB_PATH, req_target_dir=self.LIB_PATH)
            if rev is not None:
                for cog in installed_cogs:
                    cog.pinned = True
            ...
            if failed_cogs:
                ...
                message = ("\nFailed to install cogs: " if len(failed_cogs) > 1 else ("\nFailed to install the cog: ")) + humanize_list(cognames) + message
            if installed_cogs:
                cognames = [inline(cog.name) for cog in installed_cogs]
                message = (
                    ("Successfully installed cogs: " if len(installed_cogs) > 1 else ("Successfully installed the cog: "))
                    + humanize_list(cognames)
                    + (
                        f"\nThese cogs are now pinned and won't get updated automatically. To change this, use `{ctx.clean_prefix}cog unpin <cog>`"
                        if rev is not None
                        else ""
                    )
                    + message
    ...
    async def _cog_uninstall(self, ctx: commands.Context, *cogs: InstalledCog) -> None:
        ...
        Examples
        --------
            - `;cog uninstall defender`
            - `;cog uninstall say roleinvite`
        ...
        **Arguments**
        ...
        async with ctx.typing():
            ...
            message = ""
            if uninstalled_cogs:
                message += ("Successfully uninstalled cogs: " if len(uninstalled_cogs) > 1 else ("Successfully uninstalled the cog: ")) + humanize_list(
                    uninstalled_cogs,
                )
            if failed_cogs:
                if len(failed_cogs) > 1:
                    message += (
                        ...
                        ).format(command_1=inline(f"{ctx.clean_prefix}cog uninstall"), command_2=inline(f"{ctx.clean_prefix}unload {' '.join(failed_cogs)}"))
                    )
                else:
                    message += (
                        ...
                        ).format(command_1=inline(f"{ctx.clean_prefix}cog uninstall"), command_2=inline(f"{ctx.clean_prefix}unload {failed_cogs[0]}"))
                    )
        await self.send_pagified(ctx, message)
    ...
    async def _cog_pin(self, ctx: commands.Context, *cogs: InstalledCog) -> None:
        ...
        --------
            - `;cog pin defender`
            - `;cog pin outdated_cog1 outdated_cog2`
        ...
        **Arguments**
        ...
        if pinned:
            ...
            message += ("Pinned cogs: " if len(pinned) > 1 else ("Pinned cog: ")) + humanize_list(cognames)
        if already_pinned:
            message += ("\nThese cogs were already pinned: " if len(already_pinned) > 1 else ("\nThis cog was already pinned: ")) + humanize_list(
                already_pinned,
            )
        await self.send_pagified(ctx, message)
    ...
    async def _cog_unpin(self, ctx: commands.Context, *cogs: InstalledCog) -> None:
        ...
        --------
            - `;cog unpin defender`
            - `;cog unpin updated_cog1 updated_cog2`
        ...
        **Arguments**
        ...
        if unpinned:
            ...
            message += ("Unpinned cogs: " if len(unpinned) > 1 else ("Unpinned cog: ")) + humanize_list(cognames)
        if not_pinned:
            message += ("\nThese cogs weren't pinned: " if len(not_pinned) > 1 else ("\nThis cog was already not pinned: ")) + humanize_list(not_pinned)
        await self.send_pagified(ctx, message)
    ...
    async def _cog_listpinned(self, ctx: commands.Context):
        ...
        message = humanize_list(pinned_list) if pinned_list else ("None.")
        if await ctx.embed_requested():
            embed = discord.Embed(color=(await ctx.embed_colour()))
            for page in pagify(message, delims=[", "], page_length=900):
                name = "(continued)" if page.startswith(", ") else ("Pinned Cogs:")
                if page.startswith(", "):
                    page = page[2:]
            ...
            for page in pagify(message, delims=[", "], page_length=1900):
                page = page[2:] if page.startswith(", ") else ("Pinned Cogs: \n") + page
    ...
    async def _cog_checkforupdates(self, ctx: commands.Context) -> None:
        ...
        async with ctx.typing():
            ...
            if cogs_to_update:
                ...
                message += ("These cogs can be updated: " if len(cognames) > 1 else ("This cog can be updated: ")) + humanize_list(tuple(map(inline, cognames)))
            if libs_to_update:
                libnames = [cog.name for cog in libs_to_update]
                message += ("\nThese shared libraries can be updated: " if len(libnames) > 1 else ("\nThis shared library can be updated: ")) + humanize_list(
                    tuple(map(inline, libnames)),
                )
            if not cogs_to_update and not libs_to_update and filter_message:
                message += "No cogs can be updated."
            message += filter_message

            if not message:
                message = "All installed cogs are up to date."
    ...
    async def _cog_updateallfromrepos(self, ctx: commands.Context, *repos: Repo) -> None:
        ...
        Examples
        --------
            - `;cog updateallfromrepos 26-Cogs`
            - `;cog updateallfromrepos Laggrons-Dumb-Cogs 26-Cogs`
    ...
    async def _cog_updatetoversion(self, ctx: commands.Context, repo: Repo, revision: str, *cogs: InstalledCog) -> None:
        """Update all cogs, or ones of your choosing to chosen revision of one
        repo.
        ...
        See `;cog installversion` for an explanation of `revision`.

        Example:
        -------
            - `;cog updatetoversion Broken-Repo e798cc268e199612b1316a3d1f193da0770c7016 cog_name`
    ...
    async def _cog_update_logic(
        ...
        *,
        ...
        async with ctx.typing():
            ...
            if repo is not None:
                ...
                try:
                    ...
                    message = f"Error: there is no revision `{rev}` in repo `{repo.name}`"
            ...
            if not cogs_to_check:
                ...
                if updates_available:
                    ...
                    message += "Cogs from provided repos are already up to date."
                elif repo:
                    if cogs:
                        message += "Provided cogs are already up to date with this revision."
                    else:
                        message += "Cogs from provided repo are already up to date with this revision."
                elif cogs:
        ...
        if repos_with_libs := {inline(module.repo.name) for module in cogs_to_update + libs_to_update if module.repo.available_libraries}:
    ...
    async def _cog_list(self, ctx: commands.Context, repo: Repo) -> None:
        ...
        **Arguments**
        ...
        installed_str = "\n".join(f'- {i.name}{f": {i.short}" if i.short else ""}' for i in installed_cogs_in_repo)

        if len(installed_cogs_in_repo) > 1:
            installed_str = f"Installed Cogs:\n{installed_str}"
        elif installed_cogs_in_repo:
            installed_str = f"Installed Cog:\n{installed_str}"

        available_cogs = [cog for cog in repo.available_cogs if not (cog.hidden or cog in installed_cogs_in_repo)]
        available_str = "\n".join(f'+ {cog.name}{f": {cog.short}" if cog.short else ""}' for cog in available_cogs)

        if not available_str:
            cogs = "Available Cogs:\nNo cogs are available."
        elif len(available_cogs) > 1:
            cogs = f"Available Cogs:\n{available_str}"
        else:
            cogs = f"Available Cog:\n{available_str}"
        cogs = cogs + "\n\n" + installed_str
    ...
    async def _cog_info(self, ctx: commands.Context, repo: Repo, cog_name: str) -> None:
        ...
        **Arguments**
        ...
        if cog is None:
            await ctx.send(f"There is no cog `{cog_name}` in the repo `{repo.name}`")
            return

        msg = f"Information on {cog.name}:\n{cog.description or ''}\n\nRequirements: {', '.join(cog.requirements) or 'None'}"
    ...
    async def is_installed(self, cog_name: str) -> Union[tuple[bool, InstalledModule], tuple[bool, None]]:
        ...
        """
        return next(((True, installed_cog) for installed_cog in await self.installed_cogs() if installed_cog.name == cog_name), (False, None))
    ...
    async def _filter_incorrect_cogs_by_names(self, repo: Repo, cog_names: Iterable[str]) -> tuple[tuple[Installable, ...], str]:
        ...
        if unavailable_cogs:
            message = ("\nCouldn't find these cogs in {repo.name}: " if len(unavailable_cogs) > 1 else ("\nCouldn't find this cog in {repo.name}: ")).format(
                repo=repo,
            ) + humanize_list(unavailable_cogs)
        if already_installed:
            message += ("\nThese cogs were already installed: " if len(already_installed) > 1 else ("\nThis cog was already installed: ")) + humanize_list(
                already_installed,
            )
    ...
    def _filter_incorrect_cogs(self, cogs: Iterable[Installable]) -> tuple[tuple[Installable, ...], str]:
        correct_cogs: list[Installable] = list(cogs)
        message = ""
    ...
    async def _update_cogs_and_libs(
        ...
        if failed_libs:
            libnames = [lib.name for lib in failed_libs]
            message += ("\nFailed to install shared libraries: " if len(failed_cogs) > 1 else ("\nFailed to install shared library: ")) + humanize_list(
                tuple(map(inline, libnames)),
            )
    ...
    async def _ask_for_cog_reload(self, ctx: commands.Context, updated_cognames: set[str]) -> None:
        ...
        if not updated_cognames:
            await ctx.send("None of the updated cogs were previously loaded. Update complete.")
            return

        if not ctx.assume_yes:
            message = "Would you like to reload the updated cogs?" if len(updated_cognames) > 1 else ("Would you like to reload the updated cog?")
    ...
    async def findcog(self, ctx: commands.Context, command_name: str) -> None:
        ...
        **Arguments**
        ...
        if cog := command.cog:
            ...
            if installed:
                made_by = humanize_list(cog_installable.author) if cog_installable.author else ("Missing from info.json")
                repo_url = "Missing from installed repos" if cog_installable.repo is None else cog_installable.repo.clean_url
                cog_name = cog_installable.name
            elif cog.__module__.startswith("melaniebot."):  # core commands or core cog
                made_by = "Cog Creators"
                repo_url = "git@github.com:monty-dev/admire.git"
            ...
            msg = "This command is not provided by a cog."
            await ctx.send(msg)
            return
        ...
        if await ctx.embed_requested():
            ...
            if installed and cog_installable.repo is not None and cog_installable.repo.branch:
                embed.add_field(name="Repo branch:", value=cog_installable.repo.branch, inline=False)
            await ctx.send(embed=embed)

        else:
            msg = f"Command: {command_name}\nCog name: {cog_name}\nMade by: {made_by}\nRepo URL: {repo_url}\n"
            if installed and cog_installable.repo is not None and cog_installable.repo.branch:
                msg += f"Repo branch: {cog_installable.repo.branch}\n"
    ...
    def format_failed_repos(failed: Collection[str]) -> str:
        ...
        """
        message = "Failed to update the following repositories:" if len(failed) > 1 else ("Failed to update the following repository:")
        message += f" {humanize_list(tuple(map(inline, failed)))}" + "\n"
        message += "The repository's branch might have been removed or the repository is no longer accessible at set url. See logs for more information."
        return message
