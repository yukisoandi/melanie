...
from melaniebot.core import checks, commands, config
    ...
    Or(
        {
            UseOptional(COMMAND): Or(
                {
                    Or(str, int): Or(
                        {Or(int, "default"): And(bool, error="Rules must be either `true` or `false`.")},
                        {},
...
class Permissions(commands.Cog):
    ...
    async def __permissions_hook(self, ctx: commands.Context) -> Optional[bool]:
        """Purpose of this hook is to prevent guild owner lockouts of permissions
        specifically without modifying rule behavior in any other case.

        Guild owner is not special cased outside of these configuration
        commands to allow guild owner to restrict the use of potentially
        damaging commands such as, but not limited to, cleanup to
        specific channels.

        Leaving the configuration commands special cased allows guild
        owners to fix any misconfigurations.
        ...
        ):  # main top level group  # acl group
    ...
    async def permissions_canrun(self, ctx: commands.Context, user: discord.Member, *, command: str):
        ...
        if com is None:
            ...
            out = "That user can run the specified command." if can else ("That user can not run the specified command.")
        await ctx.send(out)
    ...
    async def permissions_acl_yaml_example(self, ctx: commands.Context):
        ...
        await ctx.send(
            "Example YAML for setting rules:\n"
            + box(
                textwrap.dedent(
                    """\
                        COMMAND:
    ...
    async def permissions_acl_getglobal(self, ctx: commands.Context):
        ...
        try:
            ...
            await ctx.send("I'm not allowed to DM you.")
        else:
            if not isinstance(ctx.channel, discord.DMChannel):
                await ctx.send("I've just sent the file to you via DM.")
        finally:
            file.close()
    ...
    async def permissions_addglobalrule(
        ...
        *who_or_what: GlobalUniqueObjectFinder,
        ...
        `<who_or_what...>` is one or more users, channels or roles the
        rule is for.
    ...
    async def permissions_setdefaultguildrule(self, ctx: commands.Context, allow_or_deny: ClearableRuleType, cog_or_command: CogOrCommand):
        ...
        This is the rule a command will default to when no other rule is
        found.

        `<allow_or_deny>` should be one of "allow", "deny" or "clear".
        "clear" will reset the default rule.

        `<cog_or_command>` is the cog or command to set the default rule
    ...
    async def _permissions_acl_set(self, ctx: commands.Context, guild_id: int, update: bool) -> None:
        ...
        try:
            ...
            await ctx.send(f"Invalid syntax: {str(e)}")
        except SchemaError as e:
            await ctx.send(f"Your YAML file did not match the schema: {translate(e.errors[-1])}")
        else:
            await ctx.send("Rules set.")
    ...
    async def initialize(self) -> None:
        ...
        This will load all rules from config onto every currently loaded
        command.
