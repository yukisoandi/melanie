...
async def get_locale_from_guild(bot: Melanie, guild: Optional[discord.Guild]) -> str:
...
async def get_regional_format_from_guild(bot: Melanie, guild: Optional[discord.Guild]) -> str:
...
async def set_contextual_locales_from_guild(bot: Melanie, guild: Optional[discord.Guild]) -> None:
    """Set contextual locales (locale and regional format) for given guild
    context.
...
class Translator(Callable[[str], str]):
    ...
    def __init__(self, name: str, file_location: Union[str, Path, os.PathLike]) -> None:
    ...
    def __call__(self, untranslated: str) -> str:
        ...
        This will look for the string in the translator's :code:`.pot`
        file, with respect to the current locale.
    ...
    def load_translations(self):
        ...
        if locale.lower() == "en-us":
            # Melanie is written in en-US, no point in loading it
            return
...
def _get_babel_locale(red_locale: str) -> babel.core.Locale:
    ...
    try:  # Handles cases where red_locale is already Babel supported
        ...
        try:
            ...
            try:
                # Try to find a babel locale that's close to the one used by melanie
