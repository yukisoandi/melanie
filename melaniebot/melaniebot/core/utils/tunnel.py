...
class TunnelMeta(type):
    """Lets prevent having multiple tunnels with the same places involved."""
    ...
    def __call__(self, *args, **kwargs):
        ...
        while True:
            try:
                if all(lockout_tuple[0] != x[0] for x in _instances.keys()) and all(lockout_tuple[1] != x[1] for x in _instances.keys()):
...
class Tunnel(metaclass=TunnelMeta):
    ...
    def __init__(self, *, sender: discord.Member, origin: discord.TextChannel, recipient: discord.User) -> None:
        self.sender = sender
        self.origin = origin
    ...
    async def react_close(self, *, uid: int, message: str = ""):
        ...
        closer = next((x for x in (self.sender, self.recipient) if x.id == uid), None)
    ...
    async def message_forwarder(
        *,
        ...
        """This does the actual sending, use this instead of a full tunnel if you
        are using command initiated reactions instead of persistent event based
        ones.
    ...
    async def files_from_attach(m: discord.Message, *, use_cached: bool = False, images_only: bool = False) -> list[discord.File]:
        """Makes a list of file objects from a message returns an empty list if
        none, or if the sum of file sizes is too large for the bot to send.
    ...
    async def close_because_disabled(self, close_message: str):
        """Sends a mesage to both ends of the tunnel that the tunnel is now
        closed.
    ...
    async def communicate(self, *, message: discord.Message, topic: str = None, skip_message_content: bool = False):
        ...
        if not skip_message_content:
            content = f"{topic}\n{message.content}" if topic else message.content
        else:
            content = topic
        ...
        if message.attachments:
            ...
            if not attach:
                await message.channel.send("Could not forward attachments. Total size of attachments in a single message must be less than 8MB.")
        else:
            attach = []
