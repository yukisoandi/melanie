...
class ContextManagerMixin(ProvidesCursor):
    ...
    def with_cursor(self) -> Generator[apsw.Cursor, None, None]:
        """Apsw cursors are relatively cheap, and are gc safe In most cases, it's
        fine not to use this.
        """
    ...
    def transaction(self) -> Generator[apsw.Cursor, None, None]:
        """Wraps a cursor as a context manager for a transaction which is rolled
        back on unhandled exception, or committed on non-exception exit.
        """
        c = self.cursor()  # pylint: disable=assignment-from-no-return
        try:
...
class APSWConnectionWrapper(apsw.Connection, ContextManagerMixin):
    """Provides a few convenience methods, and allows a path object for
    construction.
    """
