...
__all__ = ("bounded_gather", "bounded_gather_iter", "deduplicate_iterables", "AsyncIter", "get_end_user_data_statement", "get_end_user_data_statement_or_raise")
...
def deduplicate_iterables(*iterables):
    """Returns a list of all unique items in ``iterables``, in the order they were
    first encountered.
    """
...
class AsyncFilter(AsyncIterator[_T], Awaitable[list[_T]]):  # pylint: disable=duplicate-bases
    ...
    def __init__(self, func: Callable[[_T], Union[bool, Awaitable[bool]]], iterable: Union[AsyncIterable[_T], Iterable[_T]]) -> None:
        ...
        if isinstance(iterable, AsyncIterable):
            ...
            msg = "Must be either an async predicate, an async iterable, or both."
            raise TypeError(msg)
    ...
    *coros_or_futures
    ...
    for cof in coros_or_futures:
        if isfuture(cof) and cof._loop is not loop:
            msg = "futures are tied to different event loops"
...
def bounded_gather(*coros_or_futures, return_exceptions: bool = False, limit: int = 4, semaphore: Optional[Semaphore] = None) -> Awaitable[list[Any]]:
...
class AsyncIter(AsyncIterator[_T], Awaitable[list[_T]]):  # pylint: disable=duplicate-bases
    """Asynchronous iterator yielding items from ``iterable`` that sleeps for
    ``delay`` seconds every ``steps`` items.
    ...
    def __init__(self, iterable: Iterable[_T], delay: Union[float, int] = 0, steps: int = 1) -> None:
        if steps < 1:
            msg = "Steps must be higher than or equals to 1"
            raise ValueError(msg)
        self._delay = delay
    ...
    def map(self, func: Callable[[_T], Union[_S, Awaitable[_S]]]) -> AsyncIter[_S]:
        ...
        if not callable(func):
            msg = "Mapping must be a callable."
            raise TypeError(msg)
        self._map = func
        return self
...
def get_end_user_data_statement(file: Union[Path, str]) -> Optional[str]:
    """This function attempts to get the ``end_user_data_statement`` key from
    cog's ``info.json``. This will log the reason if ``None`` is returned.
    ...
    >>> from melaniebot.core.utils import get_end_user_data_statement
...
def get_end_user_data_statement_or_raise(file: Union[Path, str]) -> str:
    ...
    -------
    str
        The end user data statement found in the info.ujson.
