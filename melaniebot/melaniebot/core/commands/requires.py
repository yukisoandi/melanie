...
_T = TypeVar("_T")
GlobalPermissionModel = Union[discord.User, discord.VoiceChannel, discord.TextChannel, discord.CategoryChannel, discord.Role, discord.Guild]
GuildPermissionModel = Union[discord.Member, discord.VoiceChannel, discord.TextChannel, discord.CategoryChannel, discord.Role, discord.Guild]
    ...
    async def from_ctx(cls, ctx: Context) -> PrivilegeLevel:
        ...
        for snowflake in await guild_settings.admin_role():
            if member_snowflakes.has(snowflake):  # DEP-WARN
                return cls.ADMIN
        return next((cls.MOD for snowflake in await guild_settings.mod_role() if member_snowflakes.has(snowflake)), cls.NONE)
...
class PermState(enum.Enum):
    ...
    """
    This command has been actively allowed, default user checks should be
    ignored.
    """

    NORMAL = enum.auto()
    """
    No overrides have been set for this command, make determination from
    default user checks.
    """

    PASSIVE_ALLOW = enum.auto()
    """
    There exists a subcommand in the `ACTIVE_ALLOW` state, continue down the
    subcommand tree until we either find it or realise we're on the wrong
    branch.
    """

    CAUTIOUS_ALLOW = enum.auto()
    """
    This command has been actively denied, but there exists a subcommand in the
    `ACTIVE_ALLOW` state.
    ...
    to developers. It is treated as `ACTIVE_ALLOW` for the current
    command and `PASSIVE_ALLOW` for subcommands.

    """

    DENIED_BY_HOOK = enum.auto()
    """
    This command has been actively denied by a permission hook check validation
    swaps this out, but the information may be useful to developers.

    It is treated as `ACTIVE_DENY` for the current command and any
    subcommands.
...
}  # We can only start from ACTIVE_DENY if it is set on a cog.  # Should never happen  # Should never happen  # Should never happen
...
class Requires:
    ...
    def _get_rule_from_ctx(self, ctx: Context) -> PermState:
        ...
        if ctx.guild is None:
            ...
            return rule if rule is not None else self.get_rule(self.DEFAULT, self.GLOBAL)
        rules_chain = [self._global_rules]
        if guild_rules := self._guild_rules.get(ctx.guild.id):
    ...
    async def _verify_checks(self, ctx: Context) -> bool:
        return await discord.utils.async_all(check(ctx) for check in self.checks) if self.checks else True

    @staticmethod
    def _get_perms_for(ctx: Context, user: discord.abc.User) -> discord.Permissions:
        return DM_PERMS if ctx.guild is None else ctx.channel.permissions_for(user)
    ...
    def _member_as_user(member: discord.abc.User) -> discord.User:
        return member._user if isinstance(member, discord.Member) else member
...
def permissions_check(predicate: CheckPredicate):
    ...
    This has the same behaviour as `discord.ext.commands.check`, however
    this check can be ignored if the command is allowed through a
    permissions cog.
...
def bot_has_permissions(**perms: bool):
    ...
    permissions, it will send a message describing which permissions are
    missing.
...
def has_permissions(**perms: bool):
    ...
    """
    if perms is None:
        msg = "Must provide at least one keyword argument to has_permissions"
        raise TypeError(msg)
...
class _RulesDict(dict[Union[int, str], PermState]):
    ...
    def __getitem__(self, key: Any) -> PermState:
        if key != Requires.DEFAULT and not isinstance(key, int):
            msg = f'Expected "{Requires.DEFAULT}" or int key, not "{key}"'
            raise TypeError(msg)
        return super().__getitem__(key)  # pylint: disable=no-member
...
def _validate_perms_dict(perms: dict[str, bool]) -> None:
    if invalid_keys := set(perms.keys()) - set(discord.Permissions.VALID_FLAGS):
        msg = f"Invalid perm name(s): {', '.join(invalid_keys)}"
        raise TypeError(msg)
    for perm, value in perms.items():
        if value is not True:
            # We reject any permission not specified as 'True', since this is the only value which
            # makes practical sense.
            msg = f"Permission {perm} may only be specified as 'True', not {value}"
            raise TypeError(msg)
