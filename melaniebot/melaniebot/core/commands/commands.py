...
This module contains extended classes and functions which are intended
to be used instead of those from the `discord.ext.commands` module.
...
import discord
import regex as re
from discord.ext.commands import BadArgument, CheckFailure, CommandError, DisabledCommand, Greedy
...
RESERVED_COMMAND_NAMES = ("cancel",)  # reserved due to use in ``melaniebot.core.utils.MessagePredicate``
...
class RedUnhandledAPI(Exception):
    """An exception which can be raised to signal a lack of handling specific
    APIs.
    """
...
class CogCommandMixin:
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        decorated = self.callback if isinstance(self, Command) else self
        self.requires: Requires = Requires(
            privilege_level=getattr(decorated, "__requires_privilege_level__", PrivilegeLevel.NONE),
    ...
    def format_text_for_context(self, ctx: Context, text: str) -> str:
        ...
        The steps are (currently, roughly) the following:
            ...
            - substitute ``;`` with ``ctx.clean_prefix``
        ...
        def replacement(m: re.Match) -> str:
            ...
            return ctx.me.display_name if s == "[botname]" else s
    ...
    def format_help_for_context(self, ctx: Context) -> str:
        ...
        """
        help_str = self.help
        return self.format_text_for_context(ctx, help_str) if help_str else help_str
    ...
    def clear_rule_for(self, model_id: Union[int, str], guild_id: int) -> tuple[PermState, PermState]:
        ...
        """
        cur_rule = self.requires.get_rule(model_id, guild_id=guild_id)
        if cur_rule is PermState.ACTIVE_ALLOW or cur_rule is PermState.ACTIVE_DENY:
...
class Command(CogCommandMixin, DPYCommand):
    ...
    .. warning::
        ...
        be dunder names which start with melanie (eg. ``__red_test_thing__``)
        ...
        if self.parent is None:
            for name in (self.name, *self.aliases):
                if name in RESERVED_COMMAND_NAMES:
                    msg = f"The name `{name}` cannot be set as a command name. It is reserved for internal use."
                    raise RuntimeError(msg)
        if len(self.qualified_name) > 60:
            msg = f"This command ({self.qualified_name}) has an excessively long qualified name, and will not be added to the bot to prevent breaking tools and menus. (limit 60)"
            raise RuntimeError(msg)
    ...
    def callback(self, function):
        ...
        for key, value in self.params.items():
            if isinstance(value.annotation, str):
                self.params[key] = value = value.replace(annotation=eval(value.annotation, globals_))
    ...
    def help(self):
        ...
        default to that. If not, it will attempt to translate the
        docstring of the command's callback function.
        ...
        return "" if command_doc is None else inspect.cleandoc(command_doc)
    ...
    async def can_run(self, ctx: Context, *, check_all_parents: bool = False, change_permission_state: bool = False) -> bool:
    ...
    async def can_see(self, ctx: Context):
        ...
        for cmd in (self, *self.parents):
            if cmd.hidden:
                return False
            try:
                can_run = await self.can_run(ctx, check_all_parents=True, change_permission_state=False)
    ...
    def error(self, coro):
        ...
        Melanie's global error handler will ignore commands with a registered error handler.

        To have melanie handle specific errors with the default behavior,
        call ``Melanie.on_command_error`` with ``unhandled_by_cog`` set to True.
    ...
    def format_shortdoc_for_context(self, ctx: Context) -> str:
        """This formats the short version of the help string based on values in
        context.
...
class GroupMixin(discord.ext.commands.GroupMixin):
    """Mixin for `Group` and `Melanie` classes.
...
class CogGroupMixin:
    ...
    def reevaluate_rules_for(self, model_id: Union[str, int], guild_id: int = 0) -> tuple[PermState, bool]:
        ...
        if cur_rule not in (PermState.NORMAL, PermState.ACTIVE_ALLOW, PermState.ACTIVE_DENY):
            ...
            if any(cmd.requires.get_rule(model_id, guild_id=guild_id) in PermStateAllowedStates for cmd in all_commands.values()):
...
class Group(GroupMixin, Command, CogGroupMixin, DPYGroup):
    """Group command class for Melanie.
...
class CogMixin(CogGroupMixin, CogCommandMixin):
    ...
    async def can_run(self, ctx: Context, **kwargs) -> bool:
        """This really just exists to allow easy use with other methods using
        can_run on commands and groups such as help formatters.
...
class Cog(CogMixin, DPYCog, metaclass=DPYCogMeta):
    ...
    .. warning::
        ...
        with discord.py and with any of melanie's methods and attributes.
    ...
    def all_commands(self) -> dict[str, Command]:
        """This does not have identical behavior to Group.all_commands but should
...
class _AlwaysAvailableMixin:
    """This should be used for commands which should not be disabled or removed.

    These commands cannot belong to any cog except Core
    (core_commands.py) to prevent issues with the appearance of certain
    behavior.

    These commands do not respect most forms of checks, and should only
...
class _RuleDropper(CogCommandMixin):
    """Objects inheriting from this, be they command or cog, should not be
    interfered with operation except by their own rules, or by global checks
    which are not tailored for these objects but instead on global abuse
    prevention (such as a check that disallows blocked users and bots from
    interacting.).

    This should not be used by 3rd-party extensions directly for their
    own objects.
