...
from melaniebot.core import commands as red_commands
from melaniebot.core.utils import menus
from melaniebot.core.utils._internal_utils import (
...
from melaniebot.core.utils.mod import mass_purge
...
class HelpSettings:
    ...
    def pretty(self):
        ...
        def bool_transformer(val):
            if val is False:
                return "No"
            return "Yes" if val is True else val

        data = {k: bool_transformer(v) for k, v in dc_asdict(self).items()}

        data["delete_delay"] = humanize_timedelta(seconds=self.delete_delay) if self.delete_delay else ("Disabled")
...
class NoSubCommand(Exception):
    def __init__(self, *, last, not_found) -> None:
        self.last = last
...
class HelpFormatterABC(abc.ABC):
    ...
    .. note::
        ...
        Melanie will not initialize a formatter for you,
        and must be passed an initialized formatter.

        If you want to use Melanie's existing settings, use ``HelpSettings.from_context``
    ...
    @abc.abstractmethod
    async def send_help(self, ctx: Context, help_for: HelpTarget = None, *, from_help_command: bool = False):
...
class RedHelpFormatter(HelpFormatterABC):
        ...
        if isinstance(help_for, str):
            try:
                ...
                if help_settings.verify_exists:
                    await self.subcommand_not_found(ctx, exc.last, exc.not_found, help_settings=help_settings)
                    return
                help_for = exc.last
    ...
    async def get_cog_help_mapping(self, ctx: Context, obj: commands.Cog, help_settings: HelpSettings):
        iterator = (c for c in ctx.bot.commands if c.parent is None and c.cog is obj)
        return {com.name: com async for com in self.help_filter_func(ctx, iterator, help_settings=help_settings)}

    async def get_group_help_mapping(self, ctx: Context, obj: commands.Group, help_settings: HelpSettings):
        return {com.name: com async for com in self.help_filter_func(ctx, obj.all_commands.values(), help_settings=help_settings)}
    ...
    def get_default_tagline(ctx: Context):
        return f"Type {ctx.clean_prefix}help <command> for more info on a command. You can also type {ctx.clean_prefix}help <category> for more info on a category."
    ...
    async def format_command_help(self, ctx: Context, obj: commands.Command, help_settings: HelpSettings):
        ...
        signature = f"Syntax: {self.get_command_signature(ctx, command)}"
        ...
        if await ctx.embed_requested():
            ...
            if command_help:
                ...
                name = splitted[0]
                value = "\n\n".join(splitted[1:]) or EMPTY_STRING
            ...
            if subcommands:
                ...
                def shorten_line(a_line: str) -> str:
                    return a_line if len(a_line) < 70 else f"{a_line[:67].rstrip()}..."
                ...
                def width_maker(cmds):
                    ...
                    for nm, com in sorted(cmds):
                        ...
                        if len(doc) > doc_max_width:
                            doc = f"{doc[: doc_max_width - 3].rstrip()}..."
                        yield nm, doc, max_width - width_gap

                subtext = "\n".join(f"  {name:<{width}} {doc}" for name, doc, width in width_maker(subcommands.items()))

            to_page = "\n\n".join(filter(None, (description, signature, command.format_help_for_context(ctx), subtext_header, subtext)))
    ...
    async def format_cog_help(self, ctx: Context, obj: commands.Cog, help_settings: HelpSettings):
        ...
        if await ctx.embed_requested():
            ...
            if coms:
                ...
                subtext = "\n".join(f"  {name:<{width}} {doc}" for name, doc, width in width_maker(coms.items()))
    ...
    async def format_bot_help(self, ctx: Context, help_settings: HelpSettings):
        ...
        if await ctx.embed_requested():
            ...
            for cog_name, data in coms:
                title = f"**__{cog_name}:__**" if cog_name else ("**__No Category:__**")
            ...
            names = []
            for k, v in coms:
                names.extend([v.name for v in v.values()])
    ...
    @staticmethod
    async def help_filter_func(ctx, objects: Iterable[SupportsCanSee], help_settings: HelpSettings, bypass_hidden=False) -> AsyncIterator[SupportsCanSee]:
        ...
        for obj in objects:
            if verify_checks and not show_hidden:
                # Default Melanie behavior, can_see includes a can_run check.
    ...
    async def command_not_found(self, ctx, help_for, help_settings: HelpSettings):
        ...
        fuzzy_commands = await fuzzy_command_search(
            ctx,
            help_for,
            commands=self.help_filter_func(ctx, ctx.bot.walk_commands(), help_settings=help_settings),
            min_score=75,
        )
        ...
        if fuzzy_commands:
            ...
            ret = f"Help topic for *{help_for}* not found."
            if use_embeds:
    ...
    async def subcommand_not_found(self, ctx, command, not_found, help_settings: HelpSettings):
        """Sends an error."""
        ret = f"Command *{command.qualified_name}* has no subcommand named *{not_found[0]}*."
        if await ctx.embed_requested():
            ret = discord.Embed(color=(await ctx.embed_color()), description=ret)
            ret.set_author(name=f"{ctx.me.display_name} Help Menu", icon_url=ctx.me.avatar_url)
            tagline = help_settings.tagline or self.get_default_tagline(ctx)
    ...
    async def send_pages(self, ctx: Context, pages: list[Union[str, discord.Embed]], embed: bool = True, help_settings: HelpSettings = None):
        """Sends pages based on settings."""
        # save on config calls
        channel_permissions = ctx.channel.permissions_for(ctx.me)

        if not (channel_permissions.add_reactions and channel_permissions.read_message_history and help_settings.use_menus):
            ...
            for page in pages:
                try:
                    ...
                    return await ctx.send("I couldn't send the help message to you in DM. Either you blocked me or you disabled DMs in this server.")
                else:
                    messages.append(msg)
            ...
            if (
                not use_DMs and delete_delay > 0 and channel_permissions.manage_messages
            ):  # we're not in DMs  # delete delay is enabled  # we can manage messages here
                # We need to wrap this in a task to not block after-sending-help interactions.
                # The channel has to be TextChannel as we can't bulk-delete from DMs
                async def _delete_delay_help(channel: discord.TextChannel, messages: list[discord.Message], delay: int):
            ...
            asyncio.create_task(menus.menu(ctx, pages, c, message=m, timeout=help_settings.react_timeout))
