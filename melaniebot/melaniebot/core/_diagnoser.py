...
class IssueDiagnoserBase:
    def __init__(self, bot: Melanie, original_ctx: commands.Context, channel: discord.TextChannel, author: discord.Member, command: commands.Command) -> None:
        self.bot = bot
    ...
    def _format_multiple_resolutions(self, resolutions: Iterable[str]) -> str:
        parts = ["To fix this issue, you need to do one of these:"]
        parts.extend(f"{string.ascii_lowercase[idx]}) {resolution}" for idx, resolution in enumerate(resolutions))
...
class DetailedGlobalCallOnceChecksMixin(IssueDiagnoserBase):
    async def _check_is_author_bot(self) -> CheckResult:
        label = "Check if the command caller is not a bot"
        return (
            CheckResult(
                False,
                label,
                "The user is a bot which prevents them from running any command.",
                "This cannot be fixed - bots should not be listening to other bots.",
            )
            if self.author.bot
            else CheckResult(True, label)
        )

    async def _check_can_bot_send_messages(self) -> CheckResult:
        label = "Check if the bot can send messages in the given channel"
        if self.channel.permissions_for(self.guild.me).send_messages:
            return CheckResult(True, label)
        return CheckResult(
            False,
            label,
            "Bot doesn't have permission to send messages in the given channel.",
            "To fix this issue, ensure that the permissions setup allows the bot to send messages per Discord's role hierarchy:\nhttps://support.discord.com/hc/en-us/articles/206141927",
        )
    ...
    async def _check_ignored_issues(self) -> CheckResult:
        label = "Check if the channel and the server aren't set to be ignored"
        if await self.bot.ignored_channel_or_guild(self.message):
            return CheckResult(True, label)

        if self.channel.category is None:
            resolution = (
                "To fix this issue, check the list returned by the {command} command and ensure that the {channel} channel and the server aren't a part of that list."
            ).format(command=self._format_command_name("ignore list"), channel=self.channel.mention)
        else:
            resolution = (
                "To fix this issue, check the list returned by the {command} command and ensure that the {channel} channel, the channel category it belongs to ({channel_category}), and the server aren't a part of that list."
            ).format(command=self._format_command_name("ignore list"), channel=self.channel.mention, channel_category=self.channel.category.mention)

        return CheckResult(False, label, "The bot is set to ignore commands in the given channel or this server.", resolution)
    ...
    async def _get_detailed_local_whitelist_blacklist_result(self, label: str) -> CheckResult:
        ...
        details = "Local blocklist prevents the user from running this command."
    ...
    async def _check_whitelist_blacklist_issues(self) -> CheckResult:
        label = "Allowlist and blocklist checks"
...
class DetailedCommandChecksMixin(IssueDiagnoserBase):
    ...
    async def _check_dpy_can_run(self) -> CheckResult:
        label = "Global, cog and command checks"
        command = self.ctx.command
        try:
            ...
            details = (
                "The given command is disabled in this guild." if command is self.command else ("One of the parents of the given command is disabled globally.")
            )
            return CheckResult(
                False,
                label,
                details,
                ("To fix this issue, you can run {command} which will enable the {affected_command} command in this guild.").format(
        ...
        return await self._check_until_fail(
            label,
            (self._check_dpy_can_run_bot, self._check_dpy_can_run_cog, self._check_dpy_can_run_command),
            final_check_result=CheckResult(
                False,
                "Other issues related to the checks",
                f"There's an issue related to the checks for {self._format_command_name(command)} but we're not able to determine the exact cause.",
                "To fix this issue, a manual review of the global, cog and command checks is required.",
            ),
        )
    ...
    async def _check_dpy_can_run_cog(self) -> CheckResult:
        ...
        return self._command_error_handler(
            ...
            "The cog check for the command {command} failed without a message.",
        )
    ...
    async def _check_requires_command(self) -> CheckResult:
        return await self._check_requires("Permissions verification", self.ctx.command)

    async def _check_requires_cog(self) -> CheckResult:
        label = f"Permissions verification for {inline(self.ctx.cog.qualified_name)} cog"
    ...
    async def _check_requires(self, label: str, cog_or_command: commands.CogCommandMixin) -> CheckResult:
        ...
        try:
            ...
            return CheckResult(
                False,
                label,
                "The cog of the given command is disabled in this guild.",
                ("To fix this issue, you can run {command} which will enable the {affected_cog} cog in this guild.").format(
                    command=self._format_command_name(f"command enablecog {self.ctx.cog.qualified_name}"),
            ...
            details = (
                f"Bot is missing some of the channel permissions ({format_perms_list(e.missing)}) required by the {inline(cog_or_command.qualified_name)} cog."
                if cog_or_command is self.ctx.cog
                else f"Bot is missing some of the channel permissions ({format_perms_list(e.missing)}) required by the {self._format_command_name(cog_or_command)} command."
            )
            return CheckResult(
                False,
                label,
                details,
                "To fix this issue, grant the required permissions to the bot through role settings or channel overrides.",
            )
        if allowed:
            return CheckResult(True, label)
        ...
        return await self._check_until_fail(
            ...
            final_check_result=CheckResult(
                False,
                "Other issues related to the permissions.",
                (
                    "Fatal error: There's an issue related to the permissions for the {cog} cog but we're not able to determine the exact cause."
                    if cog_or_command is self.ctx.cog
                    else ("Fatal error: There's an issue related to the permissions for the {command} command but we're not able to determine the exact cause.")
                ),
                "This is an unexpected error, please report it on Melanie's issue tracker.",
            ),
        )

    async def _check_requires_bot_owner(self, cog_or_command: commands.CogCommandMixin) -> CheckResult:
        label = "Ensure that the command is not bot owner only"
        ...
        return CheckResult(
            False,
            label,
            "The command is bot owner only and the given user is not a bot owner.",
    ...
    async def _check_requires_permission_hooks(self, cog_or_command: commands.CogCommandMixin) -> CheckResult:
        ...
        if result is True:
            ...
            return CheckResult(
                False,
                label,
                "Fatal error: the result of permission hooks is inconsistent.",
                "To fix this issue, a manual review of the installed cogs is required.",
            )
        return CheckResult(
            False,
            label,
            "The access has been denied by one of the bot's permissions hooks.",
            "To fix this issue, a manual review of the installed cogs is required.",
        )
    ...
    async def _check_requires_permission_rules(self, cog_or_command: commands.CogCommandMixin) -> CheckResult:
        label = "User's discord permissions, privilege level and rules from Permissions cog"
        ...
        return CheckResult(
            ...
            label,
            (
                f"The access has been denied due to the rules set for the {inline(cog_or_command.qualified_name)} cog with Permissions cog."
                if cog_or_command is self.ctx.cog
                else f"The access has been denied due to the rules set for the {self._format_command_name(cog_or_command)} command with Permissions cog."
            ),
            "To fix the issue, a manual review of the rules is required.",
        )

    async def _check_requires_verify_user(self, label: str, cog_or_command: commands.CogCommandMixin) -> CheckResult:
        return await self._check_until_fail(
            label,
            (partial(self._check_requires_permission_checks, cog_or_command), partial(self._check_requires_user_perms_and_privilege_level, cog_or_command)),
            final_check_result=CheckResult(
                False,
                "Other issues related to the permissions.",
                (
                    f"There's an issue related to the permissions of {inline(cog_or_command.qualified_name)} cog but we're not able to determine the exact cause."
                    if cog_or_command is self.ctx.cog
                    else f"There's an issue related to the permissions of {self._format_command_name(cog_or_command)} command but we're not able to determine the exact cause."
                ),
                "To fix this issue, a manual review of the command is required.",
            ),
        )
    ...
    async def _check_requires_permission_checks(self, cog_or_command: commands.CogCommandMixin) -> CheckResult:
        ...
        if await cog_or_command.requires._verify_checks(self.ctx):
            return CheckResult(True, label)
        details = (
            f"The access has been denied by one of the permissions checks of {inline(cog_or_command.qualified_name)} cog."
            if cog_or_command is self.ctx.cog
            else f"The access has been denied by one of the permission checks of {self._format_command_name(cog_or_command)} command."
        )
        return CheckResult(False, label, details, "To fix this issue, a manual review of the permission checks is required.")

    async def _check_requires_user_perms_and_privilege_level(self, cog_or_command: commands.CogCommandMixin) -> CheckResult:
        label = "User's discord permissions and privilege level"
        ...
        if requires.user_perms is not None:
            ...
            resolutions.append("grant the required permissions to the user through role settings or channel overrides")
            details = (
                f"The user is missing some of the channel permissions ({permissions}) required by the {inline(cog_or_command.qualified_name)} cog."
                if cog_or_command is self.ctx.cog
                else f"The user is missing some of the channel permissions ({permissions}) required by the {self._format_command_name(cog_or_command)} command."
            )
        if requires.privilege_level is not None:
            if requires.privilege_level is commands.PrivilegeLevel.GUILD_OWNER:
                privilege_level = "the guild owner"
            else:
                if requires.privilege_level is commands.PrivilegeLevel.MOD:
                    privilege_level = "the mod role"
                elif requires.privilege_level is commands.PrivilegeLevel.ADMIN:
                    privilege_level = "the admin role"
                else:
                    msg = "Ran into unexpected privilege level."
                    raise RuntimeError(msg)
                resolutions.append("assign appropriate role to the user")
            details = (
                f"The user is missing the privilege level ({privilege_level}) required by the {inline(cog_or_command.qualified_name)} cog."
                if cog_or_command is self.ctx.cog
                else f"The user is missing the privilege level ({privilege_level}) required by the {self._format_command_name(cog_or_command)} command."
            )
        ...
        resolutions.append("add appropriate rule in the Permissions cog")
        if requires.user_perms is not None and requires.privilege_level is not None:
            details = (
                f"The user has neither the channel permissions ({permissions}) nor the privilege level ({privilege_level}) required by the {inline(cog_or_command.qualified_name)} cog."
                if cog_or_command is self.ctx.cog
                else (
                    "The user has neither the channel permissions ({permissions}) nor the privilege level ({privilege_level}) required by the {command} command."
                ).format(permissions=permissions, privilege_level=privilege_level, command=self._format_command_name(cog_or_command))
            )
    ...
    async def _check_dpy_checks_and_requires(self, command: commands.Command) -> CheckResult:
        label = f"Checks and permissions verification for the command {self._format_command_name(command)}"
        ...
        return await self._check_until_fail(
            label,
            (self._check_dpy_can_run, self._check_requires_command),
            final_check_result=CheckResult(
                False,
                "Other command checks",
                "The given command is failing one of the required checks.",
...
class RootDiagnosersMixin(DetailedGlobalCallOnceChecksMixin, DetailedCommandChecksMixin, IssueDiagnoserBase):
    async def _check_global_call_once_checks_issues(self) -> CheckResult:
        ...
        return await self._check_until_fail(
            label,
            (self._check_is_author_bot, self._check_can_bot_send_messages, self._check_ignored_issues, self._check_whitelist_blacklist_issues),
            final_check_result=CheckResult(
                False,
                "Other global 'call once' checks",
                "One of the global 'call once' checks implemented by a 3rd-party cog prevents this command from being ran.",
                "To fix this issue, a manual review of the installed cogs is required.",
            ),
        )

    async def _check_disabled_command_issues(self) -> CheckResult:
        label = "Check if the command is disabled"
        ...
        for parent in reversed(command.parents):
            ...
            return CheckResult(
                ...
                ("To fix this issue, you can run {command} which will enable the {affected_command} command globally.").format(
        ...
        return (
            CheckResult(True, label)
            if command.enabled
            else CheckResult(
                False,
                label,
                "The given command is disabled globally.",
                ("To fix this issue, you can run {command} which will enable the {affected_command} command globally.").format(
    ...
    async def _check_can_run_issues(self) -> CheckResult:
        label = "Checks and permissions verification"
        ctx = self.ctx
        try:
        ...
        return await self._check_until_fail(
            ...
            itertools.chain(
                ...
                (partial(self._check_dpy_checks_and_requires, command) for command in itertools.chain(reversed(self.command.parents), (self.command,))),
            ),
            final_check_result=CheckResult(
                False,
                "Other command checks",
...
class IssueDiagnoser(RootDiagnosersMixin, IssueDiagnoserBase):
    def _get_message_from_check_result(self, result: CheckResult, *, prefix: str = "") -> list[str]:
        ...
        for idx, subresult in enumerate(result.details, start=1):
            status = "Passed" + " \N{WHITE HEAVY CHECK MARK}" if subresult.success else ("Failed") + " \N{NO ENTRY}\N{VARIATION SELECTOR-16}"
    ...
    async def diagnose(self) -> str:
        ...
        if result.success:
            lines.append(
                "All checks passed and no issues were detected. Make sure that the given parameters correspond to the channel, user, and command name that have been problematic.\n\nIf you still can't find the issue, it is likely that one of the 3rd-party cogs you're using adds a global or cog local before invoke hook that prevents the command from getting invoked as this can't be diagnosed with this tool.",
            )
        else:
            lines.append("The bot has been able to identify the issue.")
            if details := self._get_details_from_check_result(result):
                lines.append(bold("Detected issue: ") + details)
            if result.resolution:
                lines.append(bold("Solution: ") + result.resolution)

        lines.extend(
            (
                "\nHere's a detailed report in case you need it:",
                ">>> " + bold("Channel: ") + self.channel.mention,
                bold("Command caller: ") + escape(str(self.author), formatting=True),
                bold("Command: ") + self._format_command_name(self.command),
                bold("Tests that have been ran:"),
            ),
        )
