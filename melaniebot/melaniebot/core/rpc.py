...
class RPC:
    ...
    async def initialize(self, port: int):
        ...
        try:
            ...
            self._started, _discard, self._site = (True, await self._runner.setup(), web.TCPSite(self._runner, host="0.0.0.0", port=port, shutdown_timeout=0))
        except Exception as exc:
            log.opt(exception=exc).exception("RPC setup failure")
        else:
            await self._site.start()
            log.info("Created RPC server listener on port {}", port)
    ...
    def add_method(self, method, prefix: str = None):
        ...
        if not asyncio.iscoroutinefunction(method):
            msg = "RPC methods must be coroutines."
            raise TypeError(msg)
...
class RPCMixin:
    ...
    def register_rpc_handler(self, method):
        """Registers a method to act as an RPC handler if the internal RPC server
        is active.
        ...
        .. important::
            RPC support is included in Melanie on a provisional basis. Backwards incompatible changes (up to and including removal of the RPC) may occur if deemed necessary.
