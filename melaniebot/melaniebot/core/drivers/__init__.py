...
__all__ = ["get_driver", "ConfigCategory", "IdentifierData", "BaseDriver", "JsonDriver", "PostgresDriver", "BackendType"]
...
def _get_driver_class_include_old(storage_type: Optional[BackendType] = None) -> type[BaseDriver]:
    """ONLY for use in CLI for moving data away from a no longer supported
    backend.
    """
    if not storage_type or storage_type != BackendType.MONGO:
...
def get_driver_class(storage_type: Optional[BackendType] = None) -> type[BaseDriver]:
    ...
    try:
        ...
        msg = f"No driver found for storage type {storage_type}"
        raise ValueError(msg) from None


def get_driver(cog_name: str, identifier: str, storage_type: Optional[BackendType] = None, *, allow_old: bool = False, **kwargs):
    ...
    **kwargs
    ...
    try:
        driver_cls: type[BaseDriver] = _get_driver_class_include_old(storage_type) if allow_old else get_driver_class(storage_type)

    except ValueError:
        if storage_type in (BackendType.MONGOV1, BackendType.MONGO):
            msg = "Please convert to JSON first to continue using the bot.Mongo support was removed in 3.2."
        else:
            msg = f"Invalid driver type: '{storage_type}'"
