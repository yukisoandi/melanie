...
class MongoDriver(BaseDriver):
    ...
    async def initialize(cls, **storage_details) -> None:
        if motor is None:
            msg = "Melanie must be installed with the [mongo] extra to use the MongoDB driver"
        ...
        ports = "" if port == 0 else f":{port}"
        if user is not None and password is not None:
            url = f"{uri}://{quote_plus(user)}:{quote_plus(password)}@{host}{ports}/{database}"
    ...
    def get_config_details():
        ...
        port = int(input("Enter host port: ")) if uri == "mongodb" else 0
        ...
        return {"HOST": host, "PORT": port, "USERNAME": admin_uname, "PASSWORD": admin_password, "DB_NAME": db_name, "URI": uri}
    ...
    async def get(self, identifier_data: IdentifierData):
        ...
        if partial is None:
            msg = "No matching document was found and Config expects a KeyError."
            raise KeyError(msg)
    ...
    async def set(self, identifier_data: IdentifierData, value=None):
        ...
        if num_pkeys >= identifier_data.primary_key_len:
            if dot_identifiers := ".".join(map(self._escape_key, identifier_data.identifiers)):
                ...
                try:
                    async with session.start_transaction():
                        ...
                        await mongo_collection.insert_many(
                            self.generate_documents_to_insert(uuid, primary_key, value, identifier_data.primary_key_len),
                            session=session,
                        )
    ...
    def generate_documents_to_insert(cls, uuid: str, primary_keys: list[str], data: dict[str, dict[str, Any]], pkey_len: int) -> Iterator[dict[str, Any]]:
        ...
        if num_missing_pkeys == 1:
            ...
            for pkey, inner_data in data.items():
                yield from cls.generate_documents_to_insert(uuid, [*primary_keys, pkey], inner_data, pkey_len)
    ...
    async def clear(self, identifier_data: IdentifierData):
        ...
        if identifier_data.identifiers:
            ...
            results = await db.list_collections(filter={"name": {"$regex": rf"^{super_collection.name}\."}})
    ...
    async def delete_all_data(cls, *, interactive: bool = False, drop_db: Optional[bool] = None, **kwargs) -> None:
        ...
        """
        if interactive and drop_db is None:
            print(
                "Please choose from one of the following options:\n 1. Drop the entire MongoDB database for this instance, or\n 2. Delete all of Melanie's data within this database, without dropping the database itself.",
            )
            options = ("1", "2")
            while True:
                resp = input("> ")
