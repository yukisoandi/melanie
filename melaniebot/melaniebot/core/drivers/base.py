...
from melaniebot.core.utils._internal_utils import RichIndefiniteBarColumn
...
class ConfigCategory(str, enum.Enum):
    ...
    @classmethod
    def get_pkey_info(cls, category: Union[str, ConfigCategory], custom_group_data: dict[str, int]) -> tuple[int, bool]:
        """Get the full primary key length for the given category, and whether or
        not the category is a custom category.
        """
        try:
...
class IdentifierData:
    ...
    def table_key(self) -> str:
        vals = (self._cog_name, self._uuid, self._category, self._is_custom)
    ...
    def __repr__(self) -> str:
        return f"<IdentifierData  cog_name={self.cog_name} uuid={self.uuid} category={self.category} primary_key={self.primary_key} identifiers={self.identifiers}  >"

    def __eq__(self, other) -> bool:
        return (
            (self.uuid == other.uuid and self.category == other.category and self.primary_key == other.primary_key and self.identifiers == other.identifiers)
            if isinstance(other, IdentifierData)
            else False
        )

    def __hash__(self) -> int:
        return hash((self.uuid, self.category, self.primary_key, self.cog_name, self.identifiers))

    def get_child(self, *keys: str) -> IdentifierData:
        if not all(isinstance(i, str) for i in keys):
            msg = "Identifiers must be strings."
        ...
        return IdentifierData(self.cog_name, self.uuid, self.category, primary_keys, identifiers, self.primary_key_len, self.is_custom)
    ...
    def to_tuple(self) -> tuple[str, ...]:
        return tuple(filter(None, (self.cog_name, self.uuid, self.category, *self.primary_key, *self.identifiers)))


class BaseDriver(abc.ABC):
    def __init__(self, cog_name: str, identifier: str, **kwargs) -> None:
    ...
    def get_config_details() -> dict[str, Any]:
        """Asks users for additional configuration information necessary to use
        this config driver.
    ...
    @classmethod
    async def migrate_to(cls, new_driver_cls: type[BaseDriver], all_custom_group_data: dict[str, dict[str, dict[str, int]]]) -> None:
        ...
        with rich.progress.Progress(
            ...
            async with asyncio.TaskGroup() as tg:
                ...
                async def do_update(cog_name, cog_id):
                ...
                async for cog_name, cog_id in cls.aiter_cogs():
                    tg.create_task(do_update(cog_name, cog_id))
    ...
    async def export_data(self, custom_group_data: dict[str, int]) -> list[tuple[str, dict[str, Any]]]:
        categories = [c.value for c in ConfigCategory]
        categories.extend(custom_group_data.keys())

        ret = []
        for c in categories:
            ident_data = IdentifierData(self.cog_name, self.unique_cog_identifier, c, (), (), *ConfigCategory.get_pkey_info(c, custom_group_data))
            try:
    ...
    async def import_data(self, cog_data: list[tuple[str, dict[str, Any]]], custom_group_data: dict[str, int]) -> None:
