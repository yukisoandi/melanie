...
basic_config_default: dict[str, Any] = {"DATA_PATH": None, "COG_PATH_APPEND": "cogs", "CORE_PATH_APPEND": "core"}
...
def create_temp_config():
    """Creates a default instance for Melanie, so it can be ran without creating
    an instance.

    .. warning:: The data of this instance will be removed     on next
    system restart.
...
def load_basic_configuration(instance_name_: str):
    """Loads the basic bootstrap configuration necessary for `Config` to know
    where to store or look for data.
    ...
    try:
        ...
        print("You need to configure the bot instance using `melaniebot-setup` prior to running the bot.")
        raise
    try:
        basic_config = config[instance_name]
    except KeyError:
        print("Instance with this name doesn't exist. You can create new instance using `melaniebot-setup` prior to running the bot.")
        raise


def _base_data_path() -> Path:
    if basic_config is None:
        msg = "You must load the basic config before you can get the base data path."
...
def cog_data_path(cog_instance=None, raw_name: str = None) -> Path:
    """Gets the base cog data path. If you want to get the folder with which to
    store your own cog's data please pass in an instance of your cog class.
    ...
    try:
        ...
        msg = "You must load the basic config before you can get the cog data path."
...
def core_data_path() -> Path:
    try:
        ...
        msg = "You must load the basic config before you can get the core data path."
        raise RuntimeError(msg) from e
...
def bundled_data_path(cog_instance: commands.Cog) -> Path:
    ...
    if not bundled_path.is_dir():
        msg = f"No such directory {bundled_path}"
...
def storage_type() -> str:
    ...
    try:
        ...
        msg = "Bot basic config has not been loaded yet."
        raise RuntimeError(msg) from e
