...
class PrefixManager:
    def __init__(self, config: Config, cli_flags: Namespace) -> None:
        self._config: Config = config
        self._global_prefix_overide: Optional[list[str]] = sorted(cli_flags.prefix, reverse=True) or None
    ...
    async def get_prefixes(self, guild: Optional[discord.Guild] = None) -> list[str]:
        ...
        if gid in self._cached:
            ...
            if gid is not None:
                ret = await self._config.guild_from_id(gid).prefix() or await self.get_prefixes(None)
            else:
    ...
    async def set_prefixes(self, guild: Optional[discord.Guild] = None, prefixes: Optional[list[str]] = None):
        ...
        if not isinstance(prefixes, list) and not all(isinstance(pfx, str) for pfx in prefixes):
            msg = "Prefixes must be a list of strings"
            raise TypeError(msg)
        prefixes = sorted(prefixes, reverse=True)
        if gid is None:
            if not prefixes:
                msg = "You must have at least one prefix."
...
class I18nManager:
    ...
    async def get_locale(self, guild: Union[discord.Guild, None]) -> str:
        ...
        if guild is not None and guild.id in self._guild_locale and self._guild_locale[guild.id] is None or guild is None:
    ...
    async def set_locale(self, guild: Union[discord.Guild, None], locale: Union[str, None]) -> None:
        ...
        if guild is None:
            if locale is None:
                ...
                msg = "Global locale can't be None!"
                raise ValueError(msg)
    ...
    async def get_regional_format(self, guild: Union[discord.Guild, None]) -> Optional[str]:
        ...
        if guild is not None and guild.id in self._guild_regional_format and self._guild_regional_format[guild.id] is None or guild is None:
...
class IgnoreManager:
    ...
    async def get_ignored_channel(self, channel: discord.TextChannel, check_category: bool = True) -> bool:
        ret: bool
    ...
    async def set_ignored_channel(self, channel: Union[discord.TextChannel, discord.CategoryChannel], set_to: bool):
        cid: int = channel.id
...
class WhitelistBlacklistManager:
    ...
    async def discord_deleted_user(self, user_id: int):
        async with self._access_lock:
            async for guild_id_or_none, ids in AsyncIter(self._cached_whitelist.items(), steps=20):
                ids.discard(user_id)

            async for guild_id_or_none, ids in AsyncIter(self._cached_blacklist.items(), steps=20):
                ids.discard(user_id)
    ...
    async def add_to_whitelist(self, guild: Optional[discord.Guild], role_or_user: Iterable[int]):
        async with self._access_lock:
            ...
            if not all(isinstance(r_or_u, int) for r_or_u in role_or_user):
                msg = "`role_or_user` must be an iterable of `int`s."
                raise TypeError(msg)
