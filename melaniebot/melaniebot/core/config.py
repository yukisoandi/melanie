...
from typing import Any, AsyncContextManager, Optional, TypeVar, Union
...
class ConfigMeta(type):
    """We want to prevent re-initializing existing config instances while having a
    singleton.
    """

    def __call__(self, cog_name: str, unique_identifier: str, driver: BaseDriver, force_registration: bool = False, defaults: dict = None):
        if cog_name is None:
            msg = "You must provide either the cog instance or a cog name."
...
class _ValueCtxManager(Awaitable[_T], AsyncContextManager[_T]):  # pylint: disable=duplicate-bases
    ...
    def __init__(self, value_obj: Value, coro: Awaitable[Any], *, acquire_lock: bool) -> None:
    ...
    async def __aenter__(self):
        ...
        if not isinstance(self.raw_value, (list, dict)):
            msg = "Type of retrieved value must be mutable (i.e. list or dict) in order to use a config value as a context manager."
            raise TypeError(msg)
        self.__original_value = orjson.loads(orjson.dumps(self.raw_value))
...
class Value:
    ...
    driver : `melaniebot.core.drivers.BaseDriver`
        A reference to `Config.driver`.

    """

    def __init__(self, identifier_data: IdentifierData, default_value, driver, config: Config) -> None:
...
class Group(Value):
    ...
    def __init__(self, identifier_data: IdentifierData, defaults: dict, driver, config: Config, force_registration: bool = False) -> None:
    ...
    @property
    def defaults(self):
        return orjson.loads(orjson.dumps(self._defaults))
    ...
    async def _get(self, default: dict[str, Any] = ...) -> dict[str, Any]:
        ...
        return self.nested_update(raw, default) if isinstance(raw, dict) else raw
    ...
    def __getattr__(self, item: str) -> Union[Group, Value]:
        ...
        if is_group:
            ...
            return Value(identifier_data=new_identifiers, default_value=self._defaults[item], driver=self.driver, config=self._config)
        elif self.force_registration:
            msg = f"'{item}' is not a valid registered Group or value."
            raise AttributeError(msg)
        else:
            return Value(identifier_data=new_identifiers, default_value=None, driver=self.driver, config=self._config)
    ...
    def is_group(self, item: Any) -> bool:
        """A helper method for `__getattr__`. Most developers will have no need to
        use this.
    ...
    async def get_raw(self, *nested_path: Any, default=...):
        ...
        try:
            ...
            return self.nested_update(raw, default) if isinstance(default, dict) else raw
    ...
    def nested_update(self, current: collections.abc.Mapping, defaults: dict[str, Any] = ...) -> dict[str, Any]:
        """Robust updater for nested dictionaries.

        If no defaults are passed, then the instance attribute
        ...
        for key, value in current.items():
            if isinstance(value, collections.abc.Mapping):
                ...
                defaults[key] = orjson.loads(orjson.dumps(current[key]))
        return defaults

    async def set(self, value):
        if not isinstance(value, dict):
            msg = "You may only set the value of a group to be a dict."
            raise ValueError(msg)
        await super().set(value)

    async def set_raw(self, *nested_path: Any, value):
        """Allows a developer to set data as if it was stored in a standard Python
        dictionary.
...
class Config(metaclass=ConfigMeta):
    """Configuration manager for cogs and Melanie.
    ...
    unique_identifier : `int`
        ...
        conflicts occur.
    driver
        An instance of a driver that implements `melaniebot.core.drivers.BaseDriver`.
    force_registration : `bool`
    ...
    def __init__(self, cog_name: str, unique_identifier: str, driver: BaseDriver, force_registration: bool = False, defaults: dict = None) -> None:
    ...
    def get_conf(cls, cog_instance, identifier: int, force_registration=False, cog_name=None, allow_old: bool = False):
        ...
        """
        if allow_old:
            log.warning("DANGER! This is getting an outdated driver. Hopefully this is only being done from convert")
        uuid = str(identifier)
        if cog_name is None:
    ...
    def _get_defaults_dict(key: str, value) -> dict:
        """Since we're allowing nested config stuff now, not storing the _defaults
        as a flat dict sounds like a good idea.

        May turn out to be an awful one but we'll see.
        ...
        for i, k in enumerate(splitted, start=1):
            if not k.isidentifier():
                msg = f"'{k}' is an invalid config key."
    ...
    def _update_defaults(to_add: dict[str, Any], _partial: dict[str, Any]):
        """This tries to update the _defaults dictionary with the nested partial
        dict generated by _get_defaults_dict.

        This WILL throw an error if you try to have both a value and a
        group registered under the same name.
        ...
        for k, v in to_add.items():
            if k in _partial:
                ...
                if val_is_dict != existing_is_dict:
                    # != is XOR
                    msg = "You cannot register a Group and a Value under the same name."
                    raise KeyError(msg)
                if val_is_dict:
    ...
    def _register_default(self, key: str, **kwargs: Any):
        ...
        data = orjson.loads(orjson.dumps(kwargs))
    ...
    def register_global(self, **kwargs):
        """Register default values for attributes you wish to store in `Config` at
        a global level.
    ...
    def init_custom(self, group_identifier: str, identifier_count: int):
        ...
        if identifier_count != self.custom_groups.setdefault(group_identifier, identifier_count):
            msg = f"Cannot change identifier count of already registered group: {group_identifier}"
            raise ValueError(msg)
    ...
    def _get_base_group(self, category: str, *primary_keys: str) -> Group:
        ...
        return Group(identifier_data=identifier_data, defaults=defaults, driver=self.driver, force_registration=self.force_registration, config=self)
    ...
    def guild_from_id(self, guild_id: int) -> Group:
        ...
        Returns
        -------
        `Group <melaniebot.core.config.Group>`
    ...
    def custom(self, group_identifier: str, *identifiers: str):
        ...
        if group_identifier not in self.custom_groups:
            msg = f"Group identifier not initialized: {group_identifier}"
            raise ValueError(msg)
        return self._get_base_group(group_identifier, *map(str, identifiers))
    ...
    async def _all_from_scope(self, scope: str) -> dict[int, dict[Any, Any]]:
        ...
        try:
            ...
            for k, v in dict_.items():
                data = orjson.loads(orjson.dumps(defaults))
                data.update(v)
                ret[int(k)] = data
    ...
    def _all_members_from_guild(self, guild_data: dict) -> dict:
        ...
        for member_id, member_data in guild_data.items():
            new_member_data = orjson.loads(orjson.dumps(defaults))
    ...
    def get_members_lock(self, guild: Optional[discord.Guild] = None) -> asyncio.Lock:
        ...
        if guild is None:
            return self.get_custom_lock(self.GUILD)
        id_data = IdentifierData(self.cog_name, self.unique_identifier, category=self.MEMBER, primary_key=(str(guild.id),), identifiers=(), primary_key_len=2)
    ...
    def get_custom_lock(self, group_identifier: str) -> asyncio.Lock:
        ...
        """
        try:
            pkey_len, is_custom = ConfigCategory.get_pkey_info(group_identifier, self.custom_groups)
        except KeyError:
            msg = f"Custom group not initialized: {group_identifier}"
