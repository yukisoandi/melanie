...
_bot_ref: Optional[Melanie] = None
...
async def _process_data_deletion(*, requester: Literal["discord_deleted_user", "owner", "user", "user_strict"], user_id: int):
    ...
    async with _data_deletion_lock:
        ...
        async for guild_id_str, guild_cases in AsyncIter(all_cases.items(), steps=20):
            async for case_num_str, case in AsyncIter(guild_cases.items(), steps=20):
                key_paths.extend((guild_id_str, case_num_str) for keyname in ("user", "moderator", "amended_by") if (case.get(keyname, 0) or 0) == user_id)
...
async def _init(bot: Melanie):
    ...
    async def on_member_unban(guild: discord.Guild, user: discord.User):
        ...
        while attempts < 12 and guild.me.guild_permissions.view_audit_log:
            attempts += 1
            try:
                entry = await guild.audit_logs(action=discord.AuditLogAction.unban, before=before, after=after).find(
...
async def handle_auditype_key():
    all_casetypes = {
        casetype_name: {inner_key: inner_value for inner_key, inner_value in casetype_data.items() if inner_key != "audit_type"}
...
async def _migrate_config(from_version: int, to_version: int):
    ...
    if from_version < 2 <= to_version:
        ...
        for guild_id, guild_data in all_guild_data.items():
            if guild_cases := guild_data.pop("cases", None):
    ...
    if from_version < 4 <= to_version:
        ...
        for guild_id, cases in (await _config.custom(_CASES).all()).items():
            if cases:
                await _config.guild(cast(discord.Guild, discord.Object(id=guild_id))).latest_case_number.set(max(map(int, cases.keys())))
...
class Case:
    ...
    async def edit(self, data: dict):
        ...
        try:
            ...
            log.info(
                "Modlog failed to edit the Discord message for the case #%s from guild with ID %s due to missing permissions.",
                self.case_number,
                self.guild.id,
            )
        except discord.NotFound:
            log.info(
                "Modlog failed to edit the Discord message for the case #%s from guild with ID %s as it no longer exists. Clearing the message ID from case data...",
                self.case_number,
                self.guild.id,
            )
            ...
            log.exception(
                "Modlog failed to edit the Discord message for the case #%s from guild with ID %s due to unexpected error.",
                self.case_number,
                self.guild.id,
            )
        finally:
    ...
    async def message_content(self, embed: bool = True):
        ...
        if self.amended_by is None:
            ...
            if self.amended_by == 3553:
                amended_by = "Deleted User."
            else:
                translated = "Unknown or Deleted User"
                amended_by = f"[{translated}] ({self.amended_by})"
        else:
            amended_by = escape_spoilers(f"{self.amended_by} ({self.amended_by.id})")
        last_modified = f"<t:{int(self.modified_at)}>" if self.modified_at else None
        if isinstance(self.user, int):
            ...
            user = escape_spoilers(filter_invites(f"{self.user} ({self.user.id})"))
        if embed:
            if self.reason:
            ...
            case_text = f"{title}\n"
            case_text += f"**User:** {user}\n"
            case_text += f"**Moderator:** {moderator}\n"
            case_text += f"{reason}\n"
            if until and duration:
                case_text += f"**Until:** {until}\n**Duration:** {duration}\n"
            if self.channel:
                if isinstance(self.channel, int):
                    case_text += f"**Channel**: {self.channel} (Deleted)\n"
                else:
                    case_text += f"**Channel**: {self.channel.name}\n"
            if amended_by:
                case_text += f"**Amended by:** {amended_by}\n"
            if last_modified:
                case_text += f"**Last modified at:** {last_modified}\n"
    ...
    @classmethod
    async def from_json(cls, mod_channel: discord.TextChannel, bot: Melanie, case_number: int, data: dict, **kwargs):
...
class CaseType:
    ...
    def __init__(self, name: str, default_setting: bool, image: str, case_str: str, guild: Optional[discord.Guild] = None, **kwargs) -> None:
        self.name = name
        ...
        if "audit_type" in kwargs:
            ...
            log.warning("Fix this using the hidden command: `modlogset fixcasetypes` in Discord: Got outdated key in casetype: audit_type")
        if kwargs:
            log.warning("Got unexpected key(s) in casetype: %s", ",".join(kwargs.keys()))

    async def to_json(self):
        """Transforms the case type into a dict and saves it."""
        data = {"default_setting": self.default_setting, "image": self.image, "case_str": self.case_str}
        await _config.custom(_CASETYPES, self.name).set(data)

    async def is_enabled(self) -> bool:
        """Determines if the case is enabled. If the guild is not set, this will
        always return False.
        ...
        """
        return await _config.guild(self.guild).casetypes.get_raw(self.name, default=self.default_setting) if self.guild else False
...
async def get_case(case_number: int, guild: discord.Guild, bot: Melanie) -> Case:
    ...
    if not case:
        msg = f"That case does not exist for guild {guild.name}"
        raise RuntimeError(msg)
    mod_channel = await get_modlog_channel(guild)
    return await Case.from_json(mod_channel, bot, case_number, case)


async def get_latest_case(guild: discord.Guild, bot: Melanie) -> Optional[Case]:
...
async def get_all_cases(guild: discord.Guild, bot: Melanie) -> list[Case]:
    ...
    mod_channel = await get_modlog_channel(guild)
    return [await Case.from_json(mod_channel, bot, case_number, case_data) for case_number, case_data in cases.items()]


async def get_cases_for_member(guild: discord.Guild, bot: Melanie, *, member: discord.Member = None, member_id: int = None) -> list[Case]:
    ...
    if not (member_id or member):
        msg = "Expected a member or a member id to be provided."
...
async def create_case(
    ...
    try:
        ...
        log.info("Modlog failed to edit the Discord message for the case #%s from guild with ID due to missing permissions.")
    except Exception:  # `finally` with `return` suppresses unexpected exceptions
        log.exception(
            "Modlog failed to send the Discord message for the case #%s from guild with ID %s due to unexpected error.",
            case.case_number,
            case.guild.id,
        )
    finally:
...
async def get_all_casetypes(guild: discord.Guild = None) -> list[CaseType]:
    ...
    """
    return [CaseType.from_json(name, data, guild=guild) for name, data in (await _config.custom(_CASETYPES).all()).items()]


async def register_casetype(name: str, default_setting: bool, image: str, case_str: str) -> CaseType:
    """Registers a case type. If the case type exists and there are differences
    between the values passed and what is stored already, the case type will be
    ...
    if not isinstance(name, str):
        msg = "The 'name' is not a string! Check the value!"
        raise ValueError(msg)
    if not isinstance(default_setting, bool):
        msg = "'default_setting' needs to be a bool!"
        raise ValueError(msg)
    if not isinstance(image, str):
        msg = "The 'image' is not a string!"
        raise ValueError(msg)
    if not isinstance(case_str, str):
        msg = "The 'case_str' is not a string!"
        raise ValueError(msg)
...
async def register_casetypes(new_types: list[dict]) -> list[CaseType]:
    ...
    See Also
    --------
    melaniebot.core.modlog.register_casetype
...
async def get_modlog_channel(guild: discord.Guild) -> discord.TextChannel:
    ...
    if guild.id not in _lri_cache:
        ...
        _lri_cache[guild.id] = channel

    if _lri_cache[guild.id] == "KE":
        msg = "Failed to get the mod log channel!"
        raise RuntimeError(msg)

    return _lri_cache[guild.id]


async def set_modlog_channel(guild: discord.Guild, channel: Union[discord.TextChannel, None]) -> bool:
...
def _strfdelta(delta):
    ...
    if delta.days:
        ...
        if delta.days > 1:
            ds += "s"
        s.append(ds)
    hrs, rem = divmod(delta.seconds, 60**2)
    if hrs:
        hs = "%i hr" % hrs
        if hrs > 1:
            hs += "s"
