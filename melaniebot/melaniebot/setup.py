...
from melaniebot.core import config, data_manager, drivers
from melaniebot.core.cli import confirm
from melaniebot.core.drivers import BackendType
from melaniebot.core.utils._internal_utils import create_backup as red_create_backup
from melaniebot.core.utils._internal_utils import safe_delete

config_dir = None
appdir = appdirs.AppDirs("melaniebot")
...
def get_data_dir(instance_name: str):
    ...
    try:
        ...
        print("We were unable to check your chosen directory. Provided path may contain an invalid character.")

    if not exists:
        try:
            data_path.mkdir(parents=True, exist_ok=True)
        except OSError:
            print("We were unable to create your chosen directory. You may need to restart this process with admin privileges.")
...
def get_name() -> str:
    ...
    while not name:
        ...
        if re.fullmatch(r"[A-Za-z0-9_\.\-]*", name) is None:
            print("ERROR: Instance names can only include characters A-z, numbers, underscores (_) and periods (.).")
            name = ""
...
def basic_setup():
    ...
    """
    print("Hello! Before we begin, we need to gather some initial information for the new instance.")
    name = get_name()
    ...
    print()
    print(
        "Your basic configuration has been saved. Please run `melaniebot <name>` to continue your setup process and to run the bot.\n\nFirst time? Read the quickstart guide:\nhttps://docs.discord.melanie/en/stable/getting_started.html",
    )
...
async def do_migration(current_backend: BackendType, target_backend: BackendType) -> dict[str, Any]:
...
async def remove_instance(
    ...
    if interactive and delete_data is None:
        delete_data = click.confirm("Would you like to delete this instance's data?", default=False)

    if interactive and _create_backup is None:
        _create_backup = click.confirm("Would you like to make a backup of the data for this instance?", default=False)
    ...
    try:
        ...
        if interactive and remove_datapath is None:
            remove_datapath = click.confirm("Would you like to delete the instance's entire datapath?", default=False)
...
async def remove_instance_interaction():
    ...
    print("You have chosen to remove an instance. The following is a list of instances that currently exist:\n")
...
@click.option("--no-prompt", "interactive", is_flag=True, default=True, help="Don't ask for user input during the process.")
@click.option(
    "--delete-data/--no-delete-data",
    "delete_data",
    is_flag=True,
    default=None,
    help="Delete this instance's data. If these options and --no-prompt are omitted, you will be asked about this.",
)
@click.option(
    "--backup/--no-backup",
    "_create_backup",
    is_flag=True,
    default=None,
    help="Create backup of this instance's data. If these options and --no-prompt are omitted, you will be asked about this.",
)
@click.option(
    "--drop-db/--no-drop-db",
    is_flag=True,
    default=None,
    help="Drop the entire database containing this instance's data. Has no effect on JSON instances, or if --no-delete-data is set. If these options and --no-prompt are omitted,you will be asked about this.",
)
@click.option(
    "--remove-datapath/--no-remove-datapath",
    is_flag=True,
    default=None,
    help="Remove this entire instance's datapath. If these options and --no-prompt are omitted, you will be asked about this. NOTE: --remove-datapath will override --no-delete-data for JSON instances.",
)
def delete(
    instance: str,
    interactive: bool,
    ...
    """Removes an instance."""
    asyncio.run(remove_instance(instance, interactive, delete_data, _create_backup, drop_db, remove_datapath))
...
def convert(instance, backend):
    ...
    if current_backend == BackendType.MONGOV1:
        msg = "Please see the 3.2 release notes for upgrading a bot using mongo."
    ...
    if new_storage_details is not None:
        ...
        log.info(f"Conversion to {target} complete.")
    else:
        log.info(f"Cannot convert {current_backend.value} to {target.value} at this time.")


@cli.command()
@click.argument("instance", type=click.Choice(instance_list), metavar="<INSTANCE_NAME>")
@click.argument("destination_folder", type=click.Path(exists=False, dir_okay=True, file_okay=False, resolve_path=True, writable=True), default=Path.home())
