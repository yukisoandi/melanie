...
import extendedmodlog.dispy_markdown as md

from .emoji import EMOJI_LIST, UNICODE_LIST

if not demoji.last_downloaded_timestamp() or pendulum.now() > (pendulum.instance(demoji.last_downloaded_timestamp()).add(days=7)):
    demoji.download_codes()

demoji.set_emoji_pattern()
UNICODE_EMOJI_PAT = demoji._EMOJI_PAT.pattern
jumbo_pat = re.compile(rf"<a?:.*?:\d*>|:[\wñ+-]+:|{UNICODE_EMOJI_PAT}")
...
class BlockQuote(md.classes["block_quote"]):
    ...
    def html(node, output, state):
        return md.html_tag(
            "div",
            md.html_tag("div", "", {"class": "blockquoteDivider-2hH8H6"}, state) + md.html_tag("blockquote", output(node["content"], state)),
            {"class": "blockquoteContainer-U5TVEi"},
            state,
        )
...
class CodeBlock(md.classes["code_block"]):
    @staticmethod
    def html(node, output, state):
        return md.html_tag("pre", md.html_tag("code", md.markdown.sanitize_text(node["content"]), {"class": node["lang"] or "plaintext"}, state), None, state)


class InlineCode(md.classes["inline_code"]):
    @staticmethod
    def html(node, output, state):
        return md.html_tag("code", md.markdown.sanitize_text(node["content"].strip()), {"class": "inline"}, state)
...
class Spoiler(md.classes["spoiler"]):
    ...
    def html(node, output, state):
        return md.html_tag(
            "span",
            md.html_tag("span", output(node["content"], state), {"class": "inlineContent-3ZjPuv"}, state),
            {"class": "spoilerText-3p6IlD"},
            state,
        )
...
class DiscordEveryone(md.classes["discord_everyone"]):
    @staticmethod
    def html(node, output, state):
        return md.html_tag("span", "@everyone", {"class": "mention wrapper-3WhCwL mention"}, state)


class DiscordHere(md.classes["discord_here"]):
    @staticmethod
    def html(node, output, state):
        return md.html_tag("span", "@here", {"class": "mention wrapper-3WhCwL mention"}, state)
...
def _parse_emoji(emoji: str):
    ...
    return {"type": "unicode_emoji", "title": title, "emoji": emoji}
...
class DiscordTextEmoji(md.markdown.Rule):
    ...
    def parse(capture, parse, state):
        if capture[1] not in EMOJI_LIST:
...
class UnicodeEmoji(md.markdown.Rule):
    ...
    def html(node, output, state):
        ...
        return md.html_tag(
            "img",
            "",
            {"class": state["emoji_class"], "title": f':{node["title"]}:', "src": f"https://twemoji.maxcdn.com/2/svg/{codepoint}.svg", "alt": node["emoji"]},
            state,
        )
...
def to_html(source: str, options: dict = None):
    options = {"embed": False, "escape_html": True, "users": {}, **(options or {})}
