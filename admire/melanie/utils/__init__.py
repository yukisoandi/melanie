...
from .async_iter import _T, AsyncIter


class asuppress(suppress):
    def __init__(self, exceptions: type[BaseException] = None) -> None:
        if not exceptions:
            import discord

            exceptions = (discord.HTTPException,)
...
class _aiterforever(aiter):
    def __init__(self, circuit_breaker, delay: Union[float, int] = 0.05) -> None:
...
class AsyncContextDecorator:
    ...
    def __call__(self, func: Callable[..., object]):
        @wraps(func)
...
def wait_safely(coro: Callable[..., object]):
...
async def aitergather(future_like, max_concurrent, chunksize: int = 100, error_action: str = "raise"):
...
async def aiterforever(circuit_breaker: bool = False, loop_delay: int = 30, task_delay: float = 0.01, iterables=None):
    while True:
