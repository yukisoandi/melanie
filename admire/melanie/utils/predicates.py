...
_ID_RE: Pattern[str] = re.compile(r"([0-9]{15,20})$")
_USER_MENTION_RE: Pattern[str] = re.compile(r"<@!?([0-9]{15,20})>$")
_CHAN_MENTION_RE: Pattern[str] = re.compile(r"<#([0-9]{15,20})>$")
_ROLE_MENTION_RE: Pattern[str] = re.compile(r"<@&([0-9]{15,20})>$")
...
class MessagePredicate(Callable[[discord.Message], bool]):
    ...
    def same_context(
        ...
        return cls(lambda self, m: (user is None or user.id == m.author.id) and (channel is None or channel.id == m.channel.id))
    ...
    def cancelled(
        ...
        """Match if the message is ``;cancel``.
    ...
    def valid_member(
        ...
        def predicate(self: MessagePredicate, m: discord.Message) -> bool:
            ...
            if match := _ID_RE.match(m.content) or _USER_MENTION_RE.match(m.content):
    ...
    def valid_text_channel(
        ...
        def predicate(self: MessagePredicate, m: discord.Message) -> bool:
            ...
            if match := _ID_RE.match(m.content) or _CHAN_MENTION_RE.match(m.content):
    ...
    def has_role(
        ...
        if user is None:
            if ctx is None:
                msg = "One of `user` or `ctx` must be supplied to `MessagePredicate.has_role`."
                raise TypeError(msg)
            user = ctx.author
    ...
    def lower_contained_in(
        ...
        """Same as :meth:`contained_in`, but the response is set to lowercase
        before matching.
    ...
    @staticmethod
    def _get_guild(ctx: commands.Context, channel: discord.TextChannel, user: discord.Member) -> discord.Guild:
        if ctx is not None:
            return ctx.guild
...
class ReactionPredicate(Callable[[discord.Reaction, discord.abc.User], bool]):
    ...
    Confirming a yes/no question with a tick/cross reaction::
        ...
        from .menus import start_adding_reactions
    ...
    YES_OR_NO_EMOJIS: ClassVar[tuple[str, str]] = ("\N{WHITE HEAVY CHECK MARK}", "\N{NEGATIVE SQUARED CROSS MARK}")
    """Tuple[str, str] : A tuple containing the tick emoji and cross emoji, in that order."""

    ALPHABET_EMOJIS: ClassVar[list[str]] = [
        chr(code) for code in range(ord("\N{REGIONAL INDICATOR SYMBOL LETTER A}"), ord("\N{REGIONAL INDICATOR SYMBOL LETTER Z}") + 1)
    ]
    """list[str] : A list of all 26 alphabetical letter emojis."""

    NUMBER_EMOJIS: ClassVar[list[str]] = [chr(code) + "\N{COMBINING ENCLOSING KEYCAP}" for code in range(ord("0"), ord("9") + 1)]
