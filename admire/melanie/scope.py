...
class Scope:
    """Class that represents a global and local scope for both scope inspection
    and creation.
    ...
    def __init__(self, globals_: dict = None, locals_: dict = None) -> None:
        self.globals: dict = globals_ or {}
        self.locals: dict = locals_ or {}

    def clear_intersection(self, other_dict: dict) -> Scope:
        """Clears out locals and globals from this scope where the key-value pair
        matches with other_dict.
    ...
    def update(self, other: Scope) -> Scope:
    ...
    def update_globals(self, other: dict) -> Scope:
    ...
    def update_locals(self, other: dict) -> Scope:
...
def get_parent_scope_from_var(name, global_ok: str = False, skip_frames: int = 0) -> typing.Optional[Scope]:
    """Iterates up the frame stack looking for a frame-scope containing the given
    variable name.
...
def get_parent_var(name, global_ok: bool = False, default: typing.Any = None, skip_frames: int = 0) -> typing.Any:
    ...
    """
    if scope := get_parent_scope_from_var(name, global_ok=global_ok, skip_frames=skip_frames + 1):
        return scope.locals.get(name, default) if name in scope.locals else scope.globals.get(name, default)
