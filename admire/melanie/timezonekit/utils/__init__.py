...
from melanie.timezonekit.utils import variables
from melanie.timezonekit.utils.abbreviations import TIMEZONES
...
def remove_timezone(interactive: bool, name: Optional[str] = None):
    ...
    if not check_config():
        console.print("No configuration file found in your system.:x:\n", style="bold red")
    ...
    with open(config_file, "r+") as file:
        ...
        entries = handle_interaction(data) if interactive else [name]
        # Clear file content.
        file.seek(0)
        file.truncate(0)
        ...
        for entry in entries:
            ...
            if entry not in data:
                console.print(f"Timezone {entry} not found in your config file.:x:", style="bold red")

            data.remove(entry)
            removed_timezones.append(f"[bold blue]{entry}[/bold blue] :white_check_mark:")

        line_break = "\n"
        console.print(f"[bold green]Timezone removed:[bold green]\n{line_break.join(removed_timezones)}")

        if not data:
            console.print(f"\n[bold red]No timezones in config.[/bold red]\n{add_prompt}", style="bold green")
            sys.exit(0)
...
def handle_interaction(data: List, multi_select=True) -> List[str]:
    ...
    selections = []
    try:
        terminal_menu = TerminalMenu(data, multi_select=multi_select, show_multi_select_hint=multi_select, search_key=None, show_search_hint=True)
        ...
        selections.extend(data[index] for index in menu_entry_index)
...
def get_local_time(timezones: List[Any], query: Optional[str] = None, toggle=False):
    ...
    for zone in timezones:
        ...
        rows.append((TIMEZONES.get(query, zone), time_data.strftime(f"%A, %B %d, %Y | %{hours}:%M:%S %p")))
...
def get_timezones(country_code: str) -> List:
    ...
    data = pycountry.countries.get(alpha_2=country_code) if len_country_code == 2 else pycountry.countries.get(alpha_3=country_code)
...
def validate_timezone(timezone: str) -> bool:
    ...
    try:
        ...
        console.print(f"[bold green]:x:Invalid timezone:[/bold green] [bold red]{timezone}[/bold red]")
        sys.exit()
...
def validate_time(time: str) -> Tuple[int, int, str]:
    ...
    time_suffix = "PM" if "PM" in minute or _hour >= 12 else "AM"
...
def get_utc_time(hour: int, minute: int, timezone: str, time: str):
    ...
    console.print(tabulate([(timezone, time, utc_time)], headers, tablefmt="fancy_grid"))
