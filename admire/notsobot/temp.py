...
class cached_property(object):
    """Decorator that creates converts a method with a single self argument into a
...
class MMCQ(object):
    """Basic Python port of the MMCQ (modified median cut quantization) algorithm
    ...
    def get_histo(pixels):
        """Histo (1-d array, giving the number of pixels in each quantized region
        of color space).
        """
        histo = {}
        for pixel in pixels:
...
class VBox(object):
    ...
    def contains(self, pixel):
        ...
        gval = pixel[1] >> MMCQ.RSHIFT
        bval = pixel[2] >> MMCQ.RSHIFT
        return all([rval >= self.r1, rval <= self.r2, gval >= self.g1, gval <= self.g2, bval >= self.b1, bval <= self.b2])
...
class CMap(object):
    ...
    def push(self, vbox):
        self.vboxes.push({"vbox": vbox, "color": vbox.avg})
