...
from melaniebot.core.i18n import Translator
from melaniebot.core.utils import bounded_gather
from melaniebot.core.utils.chat_formatting import escape
...
class MissingTokenError(Exception):
    async def send_error(self, ctx: commands.Context):
        await ctx.send(_("You need to set your API tokens. See `{prefix}tweetset creds` for information on how.").format(prefix=ctx.clean_prefix))


class TweetListener(AsyncStream):
    def __init__(self, consumer_key: str, consumer_secret: str, access_token: str, access_token_secret: str, bot: Melanie) -> None:
        super().__init__(consumer_key=consumer_key, consumer_secret=consumer_secret, access_token=access_token, access_token_secret=access_token_secret)
        self.bot = bot
...
class TweetsAPI:
    ...
    async def _start_stream(self, tweet_list: list[str], api: tweepy.API) -> None:
        ...
        try:
            self.mystream = TweetListener(consumer, consumer_secret, access_token, access_secret, self.bot)
    ...
    async def authenticate(self) -> tweepy.API:
        ...
        return tweepy.API(auth, wait_on_rate_limit=True, retry_count=10, retry_delay=5, retry_errors=[500, 502, 503, 504])

    async def autotweet_restart(self) -> None:
        if self.mystream is not None:
            self.mystream.disconnect()
        self.twitter_loop.cancel()
        self.twitter_loop = create_task(self.start_stream())
    ...
    async def on_tweet_error(self, error: str) -> None:
        ...
        if "420" in error:
            help_msg += _("You're being rate limited. Maybe you should unload the cog for a while...")
    ...
    async def build_tweet_embed(self, status: tweepy.models.Status) -> discord.Embed:
        ...
        em = discord.Embed(colour=discord.Colour(value=int(status.user.profile_link_color, 16)), url=post_url, timestamp=status.created_at)
        em.set_footer(text=f"@{username}")
        if hasattr(status, "retweeted_status"):
            em.set_author(name=f"{status.user.name} Retweeted {status.retweeted_status.user.name}", url=post_url, icon_url=status.user.profile_image_url)
        ...
        if status.in_reply_to_screen_name:
            ...
            try:
                ...
                in_reply_to = _("In reply to {name} (@{screen_name})").format(name=reply.user.name, screen_name=reply.user.screen_name)
    ...
    async def post_tweet_status(self, channel_send: discord.TextChannel, em: discord.Embed, status: tweepy.models.Status, use_custom_embed: bool = True):
        username = status.user.screen_name
        post_url = f"https://twitter.com/{status.user.screen_name}/status/{status.id}"
        if version_info >= VersionInfo.from_str("3.4.0") and await self.bot.cog_disabled_in_guild(self, channel_send.guild):
            return
        try:
            ...
            log.exception(f"Could not post a tweet in {repr(channel_send)} for account {status.user.screen_name}")
