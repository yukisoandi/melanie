...
class TweetPages(menus.PageSource):
    ...
    async def get_page(self, page_number, *, skip_next: bool = False, skip_prev: bool = False) -> dict:
    ...
    async def next(self, skip: bool = False) -> dict:
        ...
        If no new data can be found within a reasonable number of calls
        stop
    ...
    async def prev(self, skip: bool = False) -> dict:
        ...
        If no new data can be found within a reasonable number of calls
        stop (this one is expected to have more time between calls) I
        wonder if I should traverse weekly instead of daily :blobthink:
    ...
    def _get_twitter_statuses(self) -> list[tweepy.Status]:
        msg_list = []
        try:
            if statuses := self._api.user_timeline(id=self._username, page=self._page):
                msg_list.extend(iter(statuses))
        except tweepy.TweepError:
            raise

        return msg_list

    async def _next_batch(self, *, _next: bool = False, _prev: bool = False) -> list[tweepy.Status]:
        ...
        if _next:
            ...
            self._page -= 1
        try:
            fake_task = functools.partial(self._get_twitter_statuses)
    ...
    async def format_page(self, menu: menus.MenuPages, status: tweepy.Status):
        ...
        if status.in_reply_to_screen_name:
            try:
                task = self._loop.run_in_executor(None, self._get_reply, [status.in_reply_to_status_id])
                msg_list = await asyncio.wait_for(task, timeout=60)
                if msg_list:
                    reply = msg_list[0]
...
class TweetsMenu(menus.MenuPages, inherit_buttons=False):
    ...
    async def show_page(self, page_number: int, *, skip_next: bool = False, skip_prev: bool = False) -> None:
        try:
            page = await self._source.get_page(page_number, skip_next=skip_next, skip_prev=skip_prev)
        except NoTweets:
            msg = _("No twitter information could be found for {username}.").format(username=self.source._username)
    ...
    @menus.button("\N{BLACK LEFT-POINTING DOUBLE TRIANGLE WITH VERTICAL BAR}\N{VARIATION SELECTOR-16}", position=menus.First(0))
    async def go_to_first_page(self, payload):
        """Go to the first page."""
        await self.show_page(0, skip_prev=True)

    @menus.button("\N{BLACK RIGHT-POINTING DOUBLE TRIANGLE WITH VERTICAL BAR}\N{VARIATION SELECTOR-16}", position=menus.Last(1))
