...
class CpcUpdate2judResult:
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, CpcUpdate2judResult):
            return False
        return self.estimated_value == other.estimated_value and self.difference == other.difference
...
class TairCpcCommands(CommandsProtocol):
    ...
    def cpc_array_estimate_range(self, key: KeyT, start_time: int, end_time: int) -> ResponseT:
        return self.execute_command("CPC.ARRAY.ESTIMATE.RANGE", key, start_time, end_time)

    def cpc_array_estimate_range_merge(self, key: KeyT, timestamp: int, range: int) -> ResponseT:
        return self.execute_command("CPC.ARRAY.ESTIMATE.RANGE.MERGE", key, timestamp, range)
