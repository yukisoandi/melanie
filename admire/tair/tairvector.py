...
class TairVectorScanResult:
    ...
    def __next__(self):
        if self.idx >= len(self.batch):
            ...
            self.cursor = None if res[0] == b"0" else res[0]
            self.batch = res[1]
            self.idx = 0
        if self.idx >= len(self.batch):
...
class TairVectorIndex:
    def __init__(self, client, name, **index_params):
        ...
        # bind methods
        for method in ("tvs_del", "tvs_hdel", "tvs_hgetall", "tvs_hmget", "tvs_scan"):
    ...
    def tvs_knnsearch(self, k: int, vector: Union[VectorType, str], filter_str: Optional[str] = None, **kwargs):
        """search for the top @k approximate nearest neighbors of @vector"""
        return self.client.tvs_knnsearch(self.name, k, vector, self.is_binary, filter_str, **kwargs)

    def tvs_mknnsearch(self, k: int, vectors: Sequence[VectorType], filter_str: Optional[str] = None, **kwargs):
        """batch approximate nearest neighbors search for a list of vectors"""
        return self.client.tvs_mknnsearch(self.name, k, vectors, self.is_binary, filter_str, **kwargs)
...
class TairVectorCommands(CommandsProtocol):
    ...
    def tvs_create_index(
        self, name: str, dim: int, distance_type: str = DistanceMetric.L2, index_type: str = IndexType.HNSW, data_type: str = DataType.Float32, **kwargs
    ):
        """
        create a vector
        ...
        return self.execute_command(self.CREATE_INDEX_CMD, name, dim, index_type, distance_type, "data_type", data_type, *params)
    ...
    def tvs_scan_index(self, pattern: Optional[str] = None, batch: int = 10) -> TairVectorScanResult:
        """
    ...
    def tvs_hset(self, index: str, key: str, vector: Union[VectorType, str, None] = None, is_binary=False, **kwargs):
        """
        add/update a data entry to index
          @index: index name
          @key: key for the data entry
        ...
        return self.execute_command(self.HSET_CMD, index, key, Constants.VECTOR_KEY, vector, *attributes)
    ...
    def tvs_hdel(self, index: str, key: str, *args):
        ...
        """
        return self.execute_command(self.HDEL_CMD, index, key, *args) if args else 0
    ...
    def _tvs_scan(
        ...
        if vector is not None and max_dist is not None:
            args.append("VECTOR")
            args.append(vector if isinstance(vector, bytes) else self.encode_vector(vector))
            args.append("MAX_DIST")
            args.append(max_dist)
        elif vector is not None or max_dist is not None:
            raise ValueError("missing vector or max_dist")
    ...
    def tvs_knnsearch(self, index: str, k: int, vector: Union[VectorType, str, bytes], is_binary: bool = False, filter_str: Optional[str] = None, **kwargs):
        """
        ...
        return self.execute_command(self.SEARCH_CMD, index, k, vector, filter_str, *params)

    def tvs_mknnsearch(self, index: str, k: int, vectors: Sequence[VectorType], is_binary: bool = False, filter_str: Optional[str] = None, **kwargs):
        """
        batch approximate nearest neighbors search for a list of vectors
        """
        params = reduce(lambda x, y: x + y, kwargs.items(), ())
        encoded_vectors = [TairVectorCommands.encode_vector(x, is_binary) for x in vectors]
        if filter_str is None:
            return self.execute_command(self.MSEARCH_CMD, index, k, len(encoded_vectors), *encoded_vectors, *params)
        return self.execute_command(self.MSEARCH_CMD, index, k, len(encoded_vectors), *encoded_vectors, filter_str, *params)

    def tvs_mindexknnsearch(
        self, index: Sequence[str], k: int, vector: Union[VectorType, str, bytes], is_binary: bool = False, filter_str: Optional[str] = None, **kwargs
    ):
        """
        ...
        if (not isinstance(vector, str)) and (not isinstance(vector, bytes)):
            vector = TairVectorCommands.encode_vector(vector, is_binary)
        if filter_str is None:
            return self.execute_command(self.MINDEXKNNSEARCH_CMD, len(index), *index, k, vector, *params)
        return self.execute_command(self.MINDEXKNNSEARCH_CMD, len(index), *index, k, vector, filter_str, *params)

    def tvs_mindexmknnsearch(
        self, index: Sequence[str], k: int, vectors: Sequence[VectorType], is_binary: bool = False, filter_str: Optional[str] = None, **kwargs
    ):
        """
        batch approximate nearest neighbors search for a list of vectors
        """
        ...
        encoded_vectors = [TairVectorCommands.encode_vector(x, is_binary) for x in vectors]
        if filter_str is None:
            return self.execute_command(self.MINDEXMKNNSEARCH_CMD, len(index), *index, k, len(encoded_vectors), *encoded_vectors, *params)
        return self.execute_command(self.MINDEXMKNNSEARCH_CMD, len(index), *index, k, len(encoded_vectors), *encoded_vectors, filter_str, *params)
    ...
    def _tvs_getdistance(
        ...
        return self.execute_command(self.GETDISTANCE_CMD, index_name, vector_str, len(keys), *args)
    ...
    def tvs_getdistance(
        ...
        def process_batch(batch):
            return self.execute_command(self.GETDISTANCE_CMD, index_name, vector_str, len(batch), *(batch + args))
        ...
        with ThreadPoolExecutor(max_workers=parallelism) as executor:
            ...
            for f in futures:
                result = f.result()
                queue = heapq.merge(queue, [(float(result[i + 1]), result[i]) for i in range(0, len(result), 2)])
...
def parse_tvs_hincrbyfloat_result(resp) -> Union[float, None]:
    return resp if resp is None else float(resp.decode())
