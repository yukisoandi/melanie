...
class TairTsSkeyItem:
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, TairTsSkeyItem):
            return False
        return self.skey == other.skey and self.ts == other.ts and self.value == other.value
...
class Aggregation:
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, Aggregation):
            return False
        return self.aggregation_type == other.aggregation_type and self.time_bucket == other.time_bucket
...
class TairTsCommands(CommandsProtocol):
    ...
    def exts_s_create(
        ...
        if labels is not None:
            for label, val in labels.items():
                pieces.extend(("LABELS", label, val))
        return self.execute_command("EXTS.S.CREATE", *pieces)

    def exts_s_alter(self, pkey: KeyT, skey: KeyT, data_et: Optional[int] = None) -> ResponseT:
        if data_et is not None:
    ...
    def exts_s_madd(
        ...
        for item in items:
            pieces.extend((item.skey, item.ts, item.value))
        if data_et is not None:
            pieces.append("DATA_ET")
            pieces.append(data_et)
        if chunk_size is not None:
    ...
    def exts_s_queryindex(self, pkey: KeyT, filters: Iterable[str]) -> ResponseT:
        return self.execute_command("EXTS.S.QUERYINDEX", pkey, *filters)

    def exts_s_range(
        self, pkey: KeyT, skey: KeyT, from_ts: int, to_ts: Union[int, str], maxcount: Optional[int] = None, aggregation: Optional[Aggregation] = None
    ) -> ResponseT:
        ...
        if aggregation is not None:
            pieces.extend(("AGGREGATION", aggregation.aggregation_type, aggregation.time_bucket))
    ...
    def exts_p_range(
        ...
        if pkey_aggregation is not None:
            ...
            if pkey_time_bucket is not None:
                ...
                raise DataError("pkey_time_bucket is required when pkey_aggregation is specified")
        if maxcount is not None:
            pieces.append("MAXCOUNT")
            pieces.append(maxcount)
        if aggregation is not None:
