...
class TairHashCommands(CommandsProtocol):
    ...
    def exhpexpireat(self, key: KeyT, field: FieldT, pxat: AbsExpiryT, ver: Optional[int] = None, abs: Optional[int] = None) -> ResponseT:
    ...
    def exhpexpire(self, key: KeyT, field: FieldT, px: ExpiryT, ver: Optional[int] = None, abs: Optional[int] = None) -> ResponseT:
    ...
    def exhexpireat(self, key: KeyT, field: FieldT, exat: AbsExpiryT, ver: Optional[int] = None, abs: Optional[int] = None) -> ResponseT:
    ...
    def exhexpire(self, key: KeyT, field: FieldT, ex: ExpiryT, ver: Optional[int] = None, abs: Optional[int] = None) -> ResponseT:
        pieces = [key, field, ex]
...
def parse_exhgetwithver(resp) -> Union[ValueVersionItem, None]:
    return None if resp is None else ValueVersionItem(resp[0], resp[1])
...
def parse_exhgetall(resp) -> List[FieldValueItem]:
    result: List[FieldValueItem] = [FieldValueItem(resp[i], resp[i + 1]) for i in range(0, len(resp), 2)]
    return result
...
def parse_exhscan(resp) -> Union[ExhscanResult, None]:
    if resp == [b"", []]:
        return None
    items: List[FieldValueItem] = [FieldValueItem(resp[1][i], resp[1][i + 1]) for i in range(0, len(resp[1]), 2)]
