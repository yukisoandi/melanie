...
def getClipSide(clip):
    ...
    return "width" if getClipWidth(clip) < getClipHeight(clip) else "height"
...
def showOrigInfo(width, height, framesCount, fps, duration, colors, size):
    ...
    print("  Frames Count: %(fr)d (%(fps)d fps *  %(du).2f s)" % {"fr": framesCount, "fps": fps, "du": duration})
...
def compressClip(clip_path):
    ...
    current_dimension = min(original_dimension, 300)

    # Change dimension based on the shortest side.
    temp_clip = clip.resize(width=current_dimension) if shortest_side == "width" else clip.resize(height=current_dimension)

    # Change fps.
    current_fps = min(original_fps, 15)
    ...
    showChangedInfo(
        getClipWidth(temp_clip),
        getClipHeight(temp_clip),
        current_framesCount,
        temp_clip.fps,
        current_duration,
        current_colorsCount,
...
clip_path = "/home/melanie/shared-api/api-cache/a.gif"

for i in range(files_count):
    print("\n----------------------------------------------\nCurrent job: %s" % clip_path)
    print("\nOverall progress: %(current)d/%(overall)d Started." % {"current": (i + 1), "overall": files_count})

    compressClip(clip_path)

    print("\nOverall progress: %(current)d/%(overall)d Finished." % {"current": (i + 1), "overall": files_count})
