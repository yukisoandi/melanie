...
class Poll:
    ...
    def __init__(self, bot, **kwargs) -> None:
        ...
        self.end_time: Optional[datetime] = kwargs.get("end_time", self.parse_duration(self.duration))
        ...
        self._bot: Melanie = bot

    def as_dict(self) -> dict[str, Union[None, dict[str, list[int]], dict[str, str], list[str], float, str]]:
    ...
    async def add_vote(self, user_id: int, emoji: str) -> None:
    ...
    async def remove_vote(self, user_id: int, emoji: str) -> None:
    ...
    async def build_poll(self):
        ...
        end = "| ends at" if self.duration else ""
        em.set_footer(text=f"{self.author} created a poll {end}", icon_url=str(self.author.avatar_url))
        if self.end_time:
    ...
    async def close_poll(self) -> None:
        ...
        try:
            ...
            log.exception("Cannot find message")
        except Exception:
            log.exception("error tallying results")
        ...
        if sum(len(v) for k, v in self.tally.items()) == 0:
            msg += "***NO ONE VOTED.***\n"
            with contextlib.suppress(discord.errors.Forbidden, discord.errors.NotFound, AttributeError):
                if self.embed:
