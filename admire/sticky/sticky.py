...
class Sticky(commands.Cog):
    ...
    def __init__(self, bot) -> None:
        ...
        self.conf.register_channel(
            ...
            last=None,
        )  # This is for ;sticky  # This is for ;stickyexisting
    ...
    async def retry_delete(self, msg: discord.Message):
        try:
            async with asyncio.timeout(0.01):
                await self.locks[msg.id].acquire()
        except TimeoutError:
            return
    ...
    async def sticky(self, ctx: commands.Context, *, content: str):
        ...
        async with settings.all() as settings_dict:
            ...
            if settings_dict["last"] is not None:
                ...
                await self.retry_delete(last_message)
    ...
    @checks.mod_or_permissions(manage_messages=True)
    @commands.guild_only()
    @sticky.command(name="header", aliases=["toggleheader"])
    async def sticky_toggleheader(self, ctx: commands.Context, true_or_false: bool = None):
        ...
        if not true_or_false:
            state = await self.conf.channel(ctx.channel).header_enabled()
            true_or_false = bool(not state)
        await self.conf.channel(ctx.channel).header_enabled.set(true_or_false)
        msg = "Enabled the header for this channel" if true_or_false else "Disabled the header for this channel"
        return await ctx.send(embed=make_e(msg))
    ...
    async def unsticky(self, ctx: commands.Context, force: bool = False):
        ...
        async with self._lock_channel(channel):
            ...
            if last_id is None:
                await ctx.send(embed=make_e("There is no stickied message in this channel.", 2))
                return

            conf, _msg = await yesno("This will unsticky the current sticky message from this channel. Are you sure you want to do this?")
            if not conf:
                return
            await settings.set(
    ...
    async def on_message(self, message: discord.Message):
        ...
        await self._maybe_repost_stickied_message(channel, responding_to_message=message, delete_last=True)
    ...
    async def _maybe_repost_stickied_message(
        ...
        *,
        ...
        async with cv:
            ...
            utcnow = datetime.utcnow() if last_message.created_at.tzinfo is None else datetime.now(timezone.utc)
    ...
    async def _send_stickied_message(channel: discord.TextChannel, settings_dict: dict[str, Any]):
        ...
        header_text = "**__sticky message__**"
        if settings_dict.get("stickied") is not None:
            ...
            if embed_dict := settings_dict["advstickied"]["embed"]:
