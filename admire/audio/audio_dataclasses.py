...
APPLE_MUSIC_PLAYLIST_URL_PAT = re.compile(r"https:\/\/(embed.)?music\.apple\.com\/(?P<storefront>.+)\/playlist(\/.+)?\/(?P<playlist_id>.+)")
...
class LocalPath:
    ...
    Used to handle system dir trees in a cross system manner. The only
    use of this class is for

    """

    _all_music_ext = _FULLY_SUPPORTED_MUSIC_EXT + _PARTIALLY_SUPPORTED_MUSIC_EXT

    def __init__(self, path, localtrack_folder, **kwargs) -> None:
    ...
    def rglob(self, pattern, folder: bool = False) -> Iterator[str]:
        if folder:
    ...
    def glob(self, pattern, folder: bool = False) -> Iterator[str]:
        if folder:
    ...
    async def multiglob(self, *patterns, folder: bool = False) -> AsyncIterator["LocalPath"]:
        async for p in AsyncIter(patterns):
            async for path in self._multiglob(p, folder, self.glob):
                yield path

    async def multirglob(self, *patterns, folder: bool = False) -> AsyncIterator["LocalPath"]:
        async for p in AsyncIter(patterns):
...
class Query:
    ...
    def __init__(self, query: Union[LocalPath, str], local_folder_current_path: Path, **kwargs) -> None:
    ...
    def _parse(track, _local_folder_current_path: Path, **kwargs) -> MutableMapping:
        ...
        if isinstance(track, type(LocalPath)) and (track.is_file() or track.is_dir()) and track.exists():
            ...
            if track.startswith("spotify:"):
                ...
                _id = _id.split("?")[0]
                returning["id"] = _id
                if "#" in _id and (match := re.search(_RE_SPOTIFY_TIMESTAMP, track)):
                    returning["start_time"] = int(match[1]) * 60 + int(match[2])
                returning["uri"] = track
                return returning
            if track.startswith(("sc ", "list ")):
            ...
            try:
                ...
                if all([query_url.scheme, query_url.netloc, query_url.path]):
                    ...
                    if url_domain in {"youtube.com", "youtu.be"}:
                        ...
                        if ("&t=" in track or "?t=" in track) and (match := re.search(_RE_YOUTUBE_TIMESTAMP, track)):
                            returning["start_time"] = int(match[1])
                        if _has_index and (match := re.search(_RE_YOUTUBE_INDEX, track)):
                            returning["track_index"] = int(match[1]) - 1
                        ...
                        if "#" in _id:
                            _id = _id.split("#")[0]
                            if match := re.search(_RE_SPOTIFY_TIMESTAMP, track):
                                returning["start_time"] = int(match[1]) * 60 + int(match[2])
                        ...
                        returning["soundcloud"] = True
                        if "#t=" in track and (match := re.search(_RE_SOUNDCLOUD_TIMESTAMP, track)):
                            returning["start_time"] = int(match[1]) * 60 + int(match[2])
                        if "/sets/" in track and "?in=" in track or "/sets/" not in track:
                            returning["single"] = True
                        else:
                            returning["playlist"] = True
                        ...
                        if "?t=" in track and (match := re.search(_RE_TWITCH_TIMESTAMP, track)):
                            returning["start_time"] = int(match[1]) * 60 * 60 + int(match[2]) * 60 + int(match[3])
    ...
    @property
    def suffix(self):
        return self.local_track_path.suffix if self.is_local else None

    def __eq__(self, other):
        return self.to_string_user() == other.to_string_user() if isinstance(other, Query) else NotImplemented
