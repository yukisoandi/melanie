...
__all__ = ["ComplexScopeParser", "PlaylistConverter", "ScopeParser", "LazyGreedyConverter", "standardize_scope", "get_lazy_converter", "get_playlist_converter"]
...
async def global_unique_guild_finder(ctx: commands.Context, arg: str) -> discord.Guild:
    ...
    if not maybe_matches:
        raise NoMatchesFound(_('"{arg}" was not found. It must be the ID or complete name of a server which the bot can see.').format(arg=arg))
    elif len(maybe_matches) == 1:
        return maybe_matches[0]
    else:
        raise TooManyMatches(_('"{arg}" does not refer to a unique server. Please use the ID for the server you\'re trying to specify.').format(arg=arg))


async def global_unique_user_finder(ctx: commands.Context, arg: str, guild: discord.guild = None) -> discord.abc.User:
    bot: Melanie = ctx.bot
    ...
    if not maybe_matches:
        raise NoMatchesFound(_('"{arg}" was not found. It must be the ID or name or mention a user which the bot can see.').format(arg=arg))
    elif len(maybe_matches) == 1:
        return maybe_matches[0]
    else:
        raise TooManyMatches(_('"{arg}" does not refer to a unique server. Please use the ID for the server you\'re trying to specify.').format(arg=arg))
...
class PlaylistConverter(commands.Converter):
    async def convert(self, ctx: commands.Context, arg: str) -> MutableMapping:
        ...
        global_matches = []
        if cog := ctx.cog:
            global_matches = await get_all_playlist_converter(PlaylistScope.GLOBAL.value, ctx.bot, cog.playlist_api, arg, guild=ctx.guild, author=ctx.author)
            guild_matches = await get_all_playlist_converter(PlaylistScope.GUILD.value, ctx.bot, cog.playlist_api, arg, guild=ctx.guild, author=ctx.author)
            user_matches = await get_all_playlist_converter(PlaylistScope.USER.value, ctx.bot, cog.playlist_api, arg, guild=ctx.guild, author=ctx.author)
        if not user_matches and not guild_matches and not global_matches:
            msg = f"Could not match '{arg}' to a playlist."
...
class ScopeParser(commands.Converter):
    async def convert(self, ctx: commands.Context, argument: str) -> tuple[Optional[str], discord.User, Optional[discord.Guild], bool]:
        ...
        if vals["scope"]:
            ...
            valid_scopes = [*PlaylistScope.list(), "GLOBAL", "GUILD", "AUTHOR", "USER", "SERVER", "MEMBER", "BOT"]
            if scope not in valid_scopes:
                msg = "--scope"
                raise commands.ArgParserFailure(msg, scope_raw, custom_help=_(_SCOPE_HELP))
            target_scope = standardize_scope(scope)
        elif "--scope" in argument:
            msg = "--scope"
            raise commands.ArgParserFailure(msg, "Nothing", custom_help=_(_SCOPE_HELP))
        ...
        if is_owner and guild:
            ...
            if target_guild is None:
                msg = "--guild"
                raise commands.ArgParserFailure(msg, guild_raw, custom_help=f"{server_error}{_(_GUILD_HELP)}")

        elif not is_owner and (guild or any(x in argument for x in ["--guild", "--server"])):
            msg = "You cannot use `--guild`"
            raise commands.BadArgument(msg)
        elif any(x in argument for x in ["--guild", "--server"]):
            msg = "--guild"
            raise commands.ArgParserFailure(msg, "Nothing", custom_help=_(_GUILD_HELP))

        if author := vals.get("author") or vals.get("user") or vals.get("member"):
            user_error = ""
            target_user = None
            ...
            if target_user is None:
                msg = "--author"
                raise commands.ArgParserFailure(msg, user_raw, custom_help=f"{user_error}{_(_USER_HELP)}")
        elif any(x in argument for x in ["--author", "--user", "--member"]):
            msg = "--scope"
            raise commands.ArgParserFailure(msg, "Nothing", custom_help=_(_USER_HELP))
...
class ComplexScopeParser(commands.Converter):
    async def convert(
        ...
        argument: str,
    ) -> tuple[str, discord.User, Optional[discord.Guild], bool, str, discord.User, Optional[discord.Guild], bool]:
        target_scope: Optional[str] = None
        target_user: Optional[Union[discord.Member, discord.User]] = None
        target_guild: Optional[discord.Guild] = None
        ...
        if vals["to_scope"]:
            ...
            if to_scope not in valid_scopes:
                msg = "--to-scope"
                raise commands.ArgParserFailure(msg, to_scope_raw, custom_help=_SCOPE_HELP)
        ...
        if vals["from_scope"]:
            ...
            if from_scope not in valid_scopes:
                msg = "--from-scope"
                raise commands.ArgParserFailure(msg, from_scope_raw, custom_help=_SCOPE_HELP)
        ...
        if is_owner and to_guild:
            ...
            if target_guild is None:
                msg = "--to-guild"
                raise commands.ArgParserFailure(msg, to_guild_raw, custom_help=f"{target_server_error}{_(_GUILD_HELP)}")
        elif not is_owner and (to_guild or any(x in argument for x in ["--to-guild", "--to-server"])):
            msg = "You cannot use `--to-server`"
            raise commands.BadArgument(msg)
        ...
        if is_owner and from_guild:
            ...
            if source_guild is None:
                msg = "--from-guild"
                raise commands.ArgParserFailure(msg, from_guild_raw, custom_help=f"{source_server_error}{_(_GUILD_HELP)}")
        elif not is_owner and (from_guild or any(x in argument for x in ["--from-guild", "--from-server"])):
            msg = "You cannot use `--from-server`"
            raise commands.BadArgument(msg)
        elif any(x in argument for x in ["--from-guild", "--from-server"]):
            msg = "--from-server"
            raise commands.ArgParserFailure(msg, "Nothing", custom_help=_(_GUILD_HELP))

        if to_author := vals.get("to_author") or vals.get("to_user") or vals.get("to_member"):
            target_user_error = ""
            target_user = None
            ...
            if target_user is None:
                msg = "--to-author"
                raise commands.ArgParserFailure(msg, to_user_raw, custom_help=f"{target_user_error}{_(_USER_HELP)}")
        elif any(x in argument for x in ["--to-author", "--to-user", "--to-member"]):
            msg = "--to-user"
            raise commands.ArgParserFailure(msg, "Nothing", custom_help=_(_USER_HELP))

        if from_author := vals.get("from_author") or vals.get("from_user") or vals.get("from_member"):
            source_user_error = ""
            source_user = None
            from_user_raw = " ".join(from_author).strip()
            try:
                source_user = await global_unique_user_finder(ctx, from_user_raw, guild=target_guild)
                specified_target_user = True
            except TooManyMatches as err:
            ...
            if source_user is None:
                msg = "--from-author"
                raise commands.ArgParserFailure(msg, from_user_raw, custom_help=f"{source_user_error}{_(_USER_HELP)}")
...
class LazyGreedyConverter(commands.Converter):
    ...
    async def convert(self, ctx: commands.Context, argument: str) -> str:
        ...
        if len(full_message) == 1:
            full_message = (argument if argument not in full_message else "") + " " + full_message[0]
        elif len(full_message) > 1:
            full_message = (argument if argument not in full_message else "") + " " + full_message[-1]
        greedy_output = (" " + full_message.replace("—", "--")).partition(f" {self.splitter_Value}")[0]
        return f"{greedy_output}".strip()


def get_lazy_converter(splitter: str) -> type:
    """Returns a typechecking safe `LazyGreedyConverter` suitable for use with
    discord.py.
    """
...
def get_playlist_converter() -> type:
    """Returns a typechecking safe `PlaylistConverter` suitable for use with
    discord.py.
    """
