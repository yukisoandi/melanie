...
from audio.apis.playlist_wrapper import PlaylistWrapper
from audio.audio_logging import debug_exc_log
from audio.core.abc import MixinMeta  # type: ignore
from audio.core.cog_utils import _SCHEMA_VERSION, CompositeMetaClass
from audio.errors import DatabaseError, TrackEnqueueError
from audio.utils import task_callback
...
class StartUpTasks(MixinMeta, metaclass=CompositeMetaClass):
    def start_up_task(self) -> None:
        ...
        self.cog_init_task = create_task(self.initialize())
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.player_automated_timer_task = create_task(self.player_automated_timer())
    ...
    async def restore_players(self) -> None:
        ...
        for guild_id, track_data in itertools.groupby(tracks_to_restore, key=lambda x: x.guild_id):
            ...
            try:
                ...
                if player is None:
                    while tries < 5 and vc is not None:
                        try:
                            ...
                            tries += 1
                            debug_exc_log(log, exc, "Failed to restore music voice channel {}", vc_id)
                            if vc is None:
                                break
                            else:
                                await asyncio.sleep(1)
                ...
                debug_exc_log(log, err, "Error restoring player in {}", guild_id)
                await self.api_interface.persistent_queue_api.drop(guild_id)
            ...
            if player is None:
                ...
                if not player.is_playing:
                    ...
                    try:
                        ...
                        if notify_channel := self.bot.get_channel(notify_channel):
