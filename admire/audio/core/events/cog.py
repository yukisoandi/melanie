...
from audio.apis.playlist_interface import Playlist, delete_playlist, get_playlist
...
class AudioEvents(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def on_red_audio_track_start(self, guild: discord.Guild, track: lavalink.Track, requester: discord.Member) -> None:
        ...
        if self.playlist_api is not None:
            daily_cache = self._daily_playlist_cache.setdefault(guild.id, await self.config.guild(guild).daily_playlists())
            global_daily_playlists = self._daily_global_playlist_cache.setdefault(self.bot.user.id, await self.config.daily_playlists())
            ...
            try:
                ...
                debug_exc_log(log, err, "Failed to delete daily playlist ID: {}", too_old_id)
            try:
                await delete_playlist(
                    scope=PlaylistScope.GLOBAL.value,
                    playlist_id=too_old_id,
                ...
                debug_exc_log(log, err, "Failed to delete global daily playlist ID: {}", too_old_id)
        persist_cache = self._persist_queue_cache.setdefault(guild.id, await self.config.guild(guild).persist_queue())
        if persist_cache:
            await self.api_interface.persistent_queue_api.played(guild_id=guild.id, track_id=track_identifier)

    @commands.Cog.listener()
    async def on_red_audio_queue_end(self, guild: discord.Guild, track: lavalink.Track, requester: discord.Member) -> None:
        if not (track and guild):
            return
    ...
    @commands.Cog.listener()
    async def on_red_audio_track_enqueue(self, guild: discord.Guild, track: lavalink.Track, requester: discord.Member) -> None:
        if not (track and guild):
            return
        persist_cache = self._persist_queue_cache.setdefault(guild.id, await self.config.guild(guild).persist_queue())
        if persist_cache:
            await self.api_interface.persistent_queue_api.enqueued(guild_id=guild.id, room_id=track.extras["vc"], track=track)

    @commands.Cog.listener()
    async def on_red_audio_track_end(self, guild: discord.Guild, track: lavalink.Track, requester: discord.Member) -> None:
        if not (track and guild):
            return
        if self.api_interface is not None and self.playlist_api is not None:
            await self.api_interface.local_cache_api.youtube.clean_up_old_entries()
    ...
    @commands.Cog.listener()
    async def on_red_audio_track_auto_play(self, guild: discord.Guild, track: lavalink.Track, requester: discord.Member, player: lavalink.Player) -> None:
        ...
        if notify_channel and has_perms and not player.fetch("autoplay_notified", False):
            if (
                ...
                and player._last_resume + datetime.timedelta(seconds=60) > datetime.datetime.now(tz=datetime.timezone.utc)
            ):
