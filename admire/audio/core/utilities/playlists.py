...
from audio.apis.playlist_interface import Playlist, PlaylistCompat23, create_playlist
...
from audio.errors import TooManyMatches, TrackEnqueueError
from audio.utils import Notifier, PlaylistScope
...
CURRATED_DATA = "https://gist.githubusercontent.com/aikaterna/4b5de6c420cd6f12b83cb895ca2de16a/raw/json"
...
class PlaylistUtilities(MixinMeta, metaclass=CompositeMetaClass):
    async def can_manage_playlist(self, scope: str, playlist: Playlist, ctx: commands.Context, user, guild, bypass: bool = False) -> bool:
        ...
        if not has_perms:
            if hasattr(playlist, "name"):
                msg = ("You do not have the permissions to manage {name} (`{id}`) [**{scope}**].").format(
                    name=playlist.name,
                    id=playlist.id,
                ...
                msg = ("You do not have the permissions to manage that playlist in {guild}.").format(guild=guild_to_query)
            elif playlist.scope in [PlaylistScope.GUILD.value, PlaylistScope.USER.value] and is_different_user:
                msg = ("You do not have the permissions to manage playlist owned by {user}.").format(user=playlist_author)
            else:
                msg = ("You do not have the permissions to manage playlists in {scope} scope.").format(scope=self.humanize_scope(scope, the=True))

            await self.send_embed_msg(ctx, title="No access to playlist.", description=msg)
            return False
        return True
    ...
    async def get_playlist_match(
        ...
        if match_count > 1:
            if correct_scope_matches2 := [p for p in correct_scope_matches if p.name == str(original_input).strip()]:
                correct_scope_matches = correct_scope_matches2
            elif original_input.isnumeric():
                arg = int(original_input)
                if correct_scope_matches3 := [p for p in correct_scope_matches if p.id == arg]:
        ...
        if match_count > 10:
            ...
            msg = "{match_count} playlists match {original_input}: Please try to be more specific, or use the playlist ID.".format(
        ...
        async for number, playlist in AsyncIter(correct_scope_matches).enumerate(start=1):
            author = self.bot.get_user(playlist.author) or playlist.author or ("Unknown")
            line = (
        ...
        embed = discord.Embed(
            title=("{playlists} playlists found, which one would you like?").format(playlists=number),
        ...
        try:
            ...
            with contextlib.suppress(discord.HTTPException):
                await msg.delete()
            raise TooManyMatches("Too many matches found and you did not select which one you wanted.") from e

        if emojis[pred.result] == "\N{CROSS MARK}":
            with contextlib.suppress(discord.HTTPException):
                await msg.delete()
            raise TooManyMatches("Too many matches found and you did not select which one you wanted.")
        with contextlib.suppress(discord.HTTPException):
            await msg.delete()
    ...
    async def _build_playlist_list_page(self, ctx: commands.Context, page_num: int, abc_names: list, scope: Optional[str]) -> discord.Embed:
        ...
        async for i, playlist_info in AsyncIter(abc_names[plist_idx_start:plist_idx_end]).enumerate(start=plist_idx_start):
            plist += f"`{i + 1}.` {playlist_info}"
        if scope is None:
            embed = discord.Embed(colour=await ctx.embed_colour(), title="Playlists you can access in this server:", description=plist)
        else:
            embed = discord.Embed(colour=await ctx.embed_colour(), title=("Playlists for {scope}:").format(scope=scope), description=plist)
        embed.set_footer(text=("Page {page_num}/{total_pages} | {num} playlists.").format(page_num=page_num, total_pages=plist_num_pages, num=len(abc_names)))
        return embed
    ...
    async def _load_v3_playlist(
        ...
        embed1 = discord.Embed(title="Please wait, adding tracks...")
        playlist_msg = await self.send_embed_msg(ctx, embed=embed1)
        track_count = len(track_list)
        uploaded_track_count = len(track_list)
        await asyncio.sleep(1)
        embed2 = discord.Embed(colour=await ctx.embed_colour(), title=("Loading track {num}/{total}...").format(num=track_count, total=uploaded_track_count))
        ...
        if not track_count:
            msg = ("Empty playlist {name} (`{id}`) [**{scope}**] created.").format(name=playlist.name, id=playlist.id, scope=scope_name)
        elif uploaded_track_count != track_count:
            bad_tracks = uploaded_track_count - track_count
            msg = ("Added {num} tracks from the {playlist_name} playlist. {num_bad} track(s) could not be loaded.").format(
                num=track_count,
                playlist_name=playlist.name,
                num_bad=bad_tracks,
            )
        else:
            msg = ("Added {num} tracks from the {playlist_name} playlist.").format(num=track_count, playlist_name=playlist.name)
        embed3 = discord.Embed(colour=await ctx.embed_colour(), title="Playlist Saved", description=msg)
    ...
    async def _load_v2_playlist(
        ...
        embed1 = discord.Embed(title="Please wait, adding tracks...")
        playlist_msg = await self.send_embed_msg(ctx, embed=embed1)
        notifier = Notifier(ctx, playlist_msg, {"playlist": "Loading track {num}/{total}..."})
        async for track_count, song_url in AsyncIter(uploaded_track_list).enumerate(start=1):
            try:
                try:
                    ...
                    log.warning("Unable to load {} E: {}", song_url, e)
                    continue
        ...
        if not successful_count:
            ...
            msg = ("Added {num} tracks from the {playlist_name} playlist. {num_bad} track(s) could not be loaded.").format(
                num=successful_count,
                playlist_name=playlist.name,
                num_bad=bad_tracks,
            )
        else:
            msg = ("Added {num} tracks from the {playlist_name} playlist.").format(num=successful_count, playlist_name=playlist.name)
        embed3 = discord.Embed(colour=await ctx.embed_colour(), title="Playlist Saved", description=msg)
        await playlist_msg.edit(embed=embed3)
    ...
    async def _playlist_check(self, ctx: commands.Context) -> bool:
        if not self._player_check(ctx):
            ...
            try:
                ...
                await self.send_embed_msg(ctx, title="Unable To Get Playlists", description="Connection to Lavalink has not yet been established.")
                return False
            except AttributeError:
                await self.send_embed_msg(ctx, title="Unable To Get Playlists", description="Connect to a voice channel first.")
                return False
        ...
        if (not ctx.author.voice or ctx.author.voice.channel != player.channel) and not await self._can_instaskip(ctx, ctx.author):
            await self.send_embed_msg(ctx, title="Unable To Get Playlists", description="You must be in the voice channel to use the playlist command.")
            return False
        ...
        if query.is_spotify:
            with contextlib.suppress(KeyError):
                if self.play_lock[ctx.guild.id]:
                    return await self.send_embed_msg(ctx, title="Unable To Get Tracks", description="Wait until the playlist has finished loading.")
    ...
    def humanize_scope(self, scope: str, ctx: Union[discord.Guild, discord.abc.User, str] = None, the: bool = None) -> Optional[str]:
        if scope == PlaylistScope.GLOBAL.value:
            return "the Global" if the else ("Global")
        elif scope == PlaylistScope.GUILD.value:
            return ctx.name if ctx else "the Server" if the else ("Server")
        elif scope == PlaylistScope.USER.value:
            return str(ctx) if ctx else "the User" if the else ("User")
    ...
    async def _build_bundled_playlist(self, forced: bool = False) -> None:
        ...
        playlist_data = {"name": "Aikaterna's curated tracks", "tracks": tracks}
