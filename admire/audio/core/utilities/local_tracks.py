...
class LocalTrackUtilities(MixinMeta, metaclass=CompositeMetaClass):
    async def get_localtracks_folders(self, ctx: commands.Context, search_subfolders: bool = True) -> list[Union[Path, LocalPath]]:
    ...
    async def get_localtrack_folder_list(self, ctx: commands.Context, query: Query) -> list[Query]:
        ...
        if not query.local_track_path.exists():
            return []
        return await query.local_track_path.tracks_in_tree() if query.search_subfolders else await query.local_track_path.tracks_in_folder()
    ...
    async def _local_play_all(self, ctx: commands.Context, query: Query, from_search: bool = False) -> None:
        if not await self.localtracks_folder_exists(ctx) or query.local_track_path is None:
            return None
        if from_search:
            query = Query.process_input(query.local_track_path.to_string(), self.local_folder_current_path, invoked_from="local folder")
        await ctx.invoke(self.command_search, query=query)

    async def get_all_localtrack_folder_tracks(self, ctx: commands.Context, query: Query) -> list[Query]:
        if not await self.localtracks_folder_exists(ctx) or query.local_track_path is None:
            return []
        return await query.local_track_path.tracks_in_tree() if query.search_subfolders else await query.local_track_path.tracks_in_folder()
    ...
    async def localtracks_folder_exists(self, ctx: commands.Context) -> bool:
        ...
        if folder.localtrack_folder is None:
            ...
            await self.send_embed_msg(ctx, title="Invalid Environment", description="No localtracks folder.")
        return False

    async def _build_local_search_list(self, to_search: list[Query], search_words: str) -> list[str]:
        to_search_string = {i.local_track_path.name for i in to_search if i.local_track_path is not None}
            ...
            if percent_match > 85:
                search_list.extend(
                    [
                        ...
                        for i in to_search
                        if i.local_track_path is not None and i.local_track_path.name == track_match
                    ],
                )
        return search_list
