...
from melaniebot.core.utils.chat_formatting import bold, escape
...
from audio.core.abc import MixinMeta
from audio.core.cog_utils import CompositeMetaClass
from audio.errors import QueryUnauthorized, SpotifyFetchError, TrackEnqueueError
from audio.utils import Notifier
...
class PlayerUtilities(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def update_bot_presence(self, track: Optional[str], playing_servers: int) -> None:
        if playing_servers == 0:
            ...
            await self.bot.change_presence(activity=discord.Activity(name=f"music in {playing_servers} servers", type=discord.ActivityType.playing))
    ...
    async def is_requester(self, ctx: commands.Context, member: discord.Member) -> bool:
        try:
            ...
            log.debug("Current requester is {}", player.current.requester)
    ...
    async def _skip_action(self, ctx: commands.Context, skip_to_track: int = None) -> None:
        ...
        if not player.current or (not player.queue and not autoplay):
            try:
                ...
                await self.send_embed_msg(ctx, title="There's nothing in the queue.")
                return
            time_remain = self.format_time(dur - pos)
            if player.current.is_stream:
                embed = discord.Embed(title="There's nothing in the queue.")
                embed.set_footer(text=("Currently livestreaming {track}").format(track=player.current.title))
            else:
                embed = discord.Embed(title="There's nothing in the queue.")
                embed.set_footer(text=("{time} left on {track}").format(time=time_remain, track=player.current.title))
        ...
        if skip_to_track is not None and skip_to_track != 1:
            if skip_to_track < 1:
                await self.send_embed_msg(ctx, title="Track number must be equal to or greater than 1.")
                return
            elif skip_to_track > len(player.queue):
                await self.send_embed_msg(ctx, title=("There are only {queuelen} songs currently queued.").format(queuelen=len(player.queue)))
                return
            embed = discord.Embed(title=("{skip_to_track} Tracks Skipped").format(skip_to_track=skip_to_track))
            await self.send_embed_msg(ctx, embed=embed)
            if player.repeat:
            ...
            embed = discord.Embed(title="Track Skipped", description=await self.get_track_description(player.current, self.local_folder_current_path))
    ...
    async def _get_spotify_tracks(
        ...
        if any([not api_data["spotify_client_id"], not api_data["spotify_client_secret"]]):
            return await self.send_embed_msg(
                ...
                description=(
                    "The owner needs to set the Spotify client ID and Spotify client secret, before Spotify URLs or codes can be used. \nSee `{prefix}audioset spotifyapi` for instructions."
        ...
        if query.single_track:
            try:
                ...
                if not res:
                    ...
                    if query.is_local and query.suffix in _PARTIALLY_SUPPORTED_MUSIC_EXT:
                        ...
                        description = ("**{suffix}** is not a fully supported format and some tracks may not play.").format(suffix=query.suffix)
            ...
            return await self.send_embed_msg(ctx, title="Unable To Find Tracks", description="This doesn't seem to be a supported Spotify URL or code.")
    ...
    async def _enqueue_tracks(
        ...
        if not isinstance(query, list):
            if not await self.is_query_allowed(self.config, ctx, f"{query}", query_obj=query):
                msg = "{query} is not an allowed query.".format(query=query.to_string_user())
                raise QueryUnauthorized(msg)
            if query.single_track:
        ...
        if not first_track_only and len(tracks) > 1:
            ...
            if len(player.queue) >= 10000:
                return await self.send_embed_msg(ctx, title="Queue size limit reached.")
            track_len = 0
            ...
            playlist_name = escape(playlist_data.name if playlist_data else ("No Title"), formatting=True)
            embed = discord.Embed(description=bold(f"[{playlist_name}]({playlist_url})") if playlist_url else playlist_name, title="Playlist Enqueued")
            embed.set_footer(text=("Added {num} tracks to the queue.{maxlength_msg}").format(num=track_len, maxlength_msg=maxlength_msg))
            ...
            # this is in the case of ;play <query>, a single Spotify url/code
            ...
            try:
                ...
                if not await self.is_query_allowed(
                    ...
                    if IS_DEBUG:
                        log.debug("Query is not allowed in %r ({})", ctx.guild.name, ctx.guild.id)
                    self.update_player_lock(ctx, False)
                    return await self.send_embed_msg(ctx, title="This track is not allowed in this server.")
                elif guild_data["maxlength"] > 0:
            ...
            embed = discord.Embed(title="Track Enqueued", description=description)
            if not guild_data["shuffle"] and queue_dur > 0:
                embed.set_footer(text=("{time} until track playback: #{position} in queue").format(time=queue_total_duration, position=before_queue_length + 1))
        inital_play = False
        if not player.current:
            inital_play = True
            await player.play()
    ...
    async def fetch_spotify_playlist(self, ctx: commands.Context, stype: str, query: Query, enqueue: bool = False, forced: bool = False):
        player = lavalink.get_player(ctx.guild.id)
        try:
            embed1 = discord.Embed(title="Please wait, finding tracks...")
            playlist_msg = await self.send_embed_msg(ctx, embed=embed1)
            notifier = Notifier(
                ctx,
                playlist_msg,
                {
                    "spotify": "Getting track {num}/{total}...",
                    "youtube": "Matching track {num}/{total}...",
                    "lavalink": "Loading track {num}/{total}...",
                    "lavalink_time": "Approximate time remaining: {seconds}",
                },
            )
            ...
            self.update_player_lock(ctx, False)
            return await self.send_embed_msg(ctx, title="Invalid Environment", description=error.message.format(prefix=ctx.prefix))
        except TrackEnqueueError:
            self.update_player_lock(ctx, False)
            return await self.send_embed_msg(
                ctx,
            ...
            self.update_player_lock(ctx, False)
            error_embed = discord.Embed(title="The connection was reset while loading the playlist.")
    ...
    def is_track_length_allowed(self, track: lavalink.Track, maxlength: int) -> bool:
        return True if track.is_stream else bool(track.length / 1000 <= maxlength)
