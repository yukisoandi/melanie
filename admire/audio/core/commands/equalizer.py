...
from melaniebot.core.utils.chat_formatting import box, humanize_number, pagify
...
class EqualizerCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_equalizer_delete(self, ctx: commands.Context, eq_preset: str):
        ...
        async with self.config.custom("EQUALIZER", ctx.guild.id).eq_presets() as eq_presets:
            ...
            try:
                if eq_presets[eq_preset]["author"] != ctx.author.id and not await self._can_instaskip(ctx, ctx.author):
                    return await self.send_embed_msg(ctx, title="Unable To Delete Preset", description="You are not the author of that preset setting.")
                del eq_presets[eq_preset]
            except KeyError:
                return await self.send_embed_msg(ctx, title="Unable To Delete Preset", description=f"{eq_preset.capitalize()} is not in the eq preset list.")
            except TypeError:
        ...
        await self.send_embed_msg(ctx, title=f"The {eq_preset} preset was deleted.")
    ...
    async def command_equalizer_list(self, ctx: commands.Context):
        ...
        header = box(f"[{header_name}]{space * 9}[{header_author}]\n", lang="ini")
        preset_list = ""
        ...
        for page in pagify(preset_list, delims=[", "], page_length=1000):
            ...
            embed.set_footer(text=("{num} preset(s)").format(num=humanize_number(len(list(eq_presets.keys())))))
    ...
    async def command_equalizer_load(self, ctx: commands.Context, eq_preset: str):
        ...
        try:
            ...
            return await self.send_embed_msg(ctx, title="No Preset Found", description=f"Preset named {eq_preset} does not exist.")
        except TypeError:
        ...
        player = lavalink.get_player(ctx.guild.id)
        if dj_enabled and not await self._can_instaskip(ctx, ctx.author):
            return await self.send_embed_msg(ctx, title="Unable To Load Preset", description="You need the DJ role to load equalizer presets.")
        ...
        message = await ctx.send(
            ...
            embed=discord.Embed(colour=await ctx.embed_colour(), title=f"The {eq_preset} preset was loaded."),
        )
    ...
    async def command_equalizer_reset(self, ctx: commands.Context):
        ...
        dj_enabled = self._dj_status_cache.setdefault(ctx.guild.id, await self.config.guild(ctx.guild).dj_enabled())
        if dj_enabled and not await self._can_instaskip(ctx, ctx.author):
            return await self.send_embed_msg(ctx, title="Unable To Modify Preset", description="You need the DJ role to reset the equalizer.")
        player = lavalink.get_player(ctx.guild.id)
        player.store("notify_channel", ctx.channel.id)
        eq = player.fetch("eq", Equalizer())
        ...
        message = await ctx.send(
            content=box(eq.visualise(), lang="ini"),
            embed=discord.Embed(colour=await ctx.embed_colour(), title="Equalizer values have been reset."),
        )
        player.store("eq_message", message)
    ...
    async def command_equalizer_save(self, ctx: commands.Context, eq_preset: str = None):
        ...
        if dj_enabled and not await self._can_instaskip(ctx, ctx.author):
            ctx.command.reset_cooldown(ctx)
            return await self.send_embed_msg(ctx, title="Unable To Save Preset", description="You need the DJ role to save equalizer presets.")
        if not eq_preset:
            await self.send_embed_msg(ctx, title="Please enter a name for this equalizer preset.")
            try:
                eq_name_msg = await self.bot.wait_for("message", timeout=15.0, check=MessagePredicate.regex(rf"^(?!{re.escape(ctx.prefix)})", ctx))
        ...
        if len(eq_preset) > 20:
            ctx.command.reset_cooldown(ctx)
            return await self.send_embed_msg(ctx, title="Unable To Save Preset", description="Try the command again with a shorter name.")
        if eq_preset in eq_list:
            eq_exists_msg = await self.send_embed_msg(ctx, title="Preset name already exists, do you want to replace it?")
        ...
        embed3 = discord.Embed(colour=await ctx.embed_colour(), title=("Current equalizer saved to the {preset_name} preset.").format(preset_name=eq_preset))
        if eq_exists_msg:
            await self._clear_react(eq_exists_msg)
    ...
    async def command_equalizer_set(self, ctx: commands.Context, band_name_or_position, band_value: float):
        ...
        dj_enabled = self._dj_status_cache.setdefault(ctx.guild.id, await self.config.guild(ctx.guild).dj_enabled())
        if dj_enabled and not await self._can_instaskip(ctx, ctx.author):
            return await self.send_embed_msg(ctx, title="Unable To Set Preset", description="You need the DJ role to set equalizer presets.")

        player = lavalink.get_player(ctx.guild.id)
        player.store("notify_channel", ctx.channel.id)
        band_names = ["25", "40", "63", "100", "160", "250", "400", "630", "1k", "1.6k", "2.5k", "4k", "6.3k", "10k", "16k"]
        ...
        if band_number not in range(bands_num) and band_name_or_position not in band_names:
            return await self.send_embed_msg(
                ctx,
                title="Invalid Band",
                description="Valid band numbers are 1-15 or the band names listed in the help for this command.",
            )
        ...
        message = await ctx.send(
            ...
            embed=discord.Embed(
                ...
                description=("The {band_name}Hz band has been set to {band_value}.").format(band_name=band_name, band_value=band_value),
            ),
        )
        player.store("eq_message", message)
