...
from melaniebot.core.utils.chat_formatting import bold, pagify
...
from audio.apis.api_utils import FakePlaylist
from audio.apis.playlist_interface import (
    Playlist,
    create_playlist,
    delete_playlist,
    get_all_playlist,
...
from audio.audio_logging import IS_DEBUG, debug_exc_log
from audio.converters import ComplexScopeParser, ScopeParser
from audio.core.abc import MixinMeta  # type: ignore
from audio.core.cog_utils import (
    CompositeMetaClass,
    LazyGreedyConverter,
    PlaylistConverter,
)
from audio.errors import MissingGuild, TooManyMatches, TrackEnqueueError
...
class PlaylistCommands(MixinMeta, metaclass=CompositeMetaClass):
    @commands.group(name="playlist", hidden=True)
    @commands.guild_only()
    async def command_playlist(self, ctx: commands.Context) -> None:
        """Playlist configuration options."""

    @command_playlist.command(name="append", usage="<playlist_name_OR_id> <track_name_OR_url> [args]")
        ...
        **Guild** can be one of the following:
        ...
        if self.playlist_api is None:
            return await self.send_embed_msg(
                ...
                footer="Check your logs." if await self.bot.is_owner(ctx.author) else None,
            )
        ...
        async with ctx.typing():
            ...
            to_append = await self.fetch_playlist_tracks(ctx, player, Query.process_input(query, self.local_folder_current_path))

            if isinstance(to_append, discord.Message):
                return None

            if not to_append:
                return await self.send_embed_msg(ctx, title="Could not find a track matching your query.")
            ...
            if to_append and to_append_count == 1:
                ...
                if to in tracks_obj_list:
                    return await self.send_embed_msg(
                        ...
                        description=("{track} is already in {playlist} (`{id}`) [**{scope}**].").format(
                            track=to.title,
                            playlist=playlist.name,
                            id=playlist.id,
                            scope=scope_name,
                        ),
                        footer=("Playlist limit reached: Could not add track.").format() if not_added > 0 else None,
                    )
                else:
                    appended += 1
            if to_append and to_append_count > 1:
            ...
            if to_append_count == 1 and appended == 1:
                ...
                return await self.send_embed_msg(
                    ctx,
                    title="Track added",
                    description=("{track} appended to {playlist} (`{id}`) [**{scope}**].").format(
                        track=track_title,
                        playlist=playlist.name,
                        id=playlist.id,
                        scope=scope_name,
            ...
            embed = discord.Embed(title="Playlist Modified", description=desc)
            await self.send_embed_msg(ctx, embed=embed, footer=("Playlist limit reached: Could not add track.").format() if not_added > 0 else None)

    @commands.cooldown(1, 150, commands.BucketType.member)
    @command_playlist.command(name="copy", usage="<id_or_name> [args]", cooldown_after_parsing=True)
    async def command_playlist_copy(self, ctx: commands.Context, playlist_matches: PlaylistConverter, *, scope_data: ComplexScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        \u200b \u200b \u200b \u200b `;playlist copy MyPersonalPlaylist --from-scope user --to-author Draper#6666 --to-scope Guild --to-guild Melanie - Discord Bot`
        ...
        if scope_data is None:
            scope_data = [PlaylistScope.GUILD.value, ctx.author, ctx.guild, False, PlaylistScope.GUILD.value, ctx.author, ctx.guild, False]
        (from_scope, from_author, from_guild, specified_from_user, to_scope, to_author, to_guild, specified_to_user) = scope_data
        ...
        async with ctx.typing():
            ...
            return await self.send_embed_msg(
                ...
                description=("Playlist {name} (`{from_id}`) copied from {from_scope} to {to_scope} (`{to_id}`).").format(
    ...
    async def command_playlist_create(self, ctx: commands.Context, playlist_name: str, *, scope_data: ScopeParser = None):
        ...
        async with ctx.typing():
            ...
            if playlist_name.isnumeric():
                return await self.send_embed_msg(
                    ...
                    description="Playlist names must be a single word (up to 32 characters) and not numbers only.",
                )
            playlist = await create_playlist(ctx, self.playlist_api, scope, playlist_name, None, None, author, guild)
            return await self.send_embed_msg(
                ctx,
                title="Playlist Created",
                description=("Empty playlist {name} (`{id}`) [**{scope}**] created.").format(name=playlist.name, id=playlist.id, scope=scope_name),
            )

    @command_playlist.command(name="delete", aliases=["del"], usage="<playlist_name_OR_id> [args]")
    async def command_playlist_delete(self, ctx: commands.Context, playlist_matches: PlaylistConverter, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            if not await self.can_manage_playlist(scope, playlist, ctx, author, guild):
                return
            scope_name = self.humanize_scope(scope, ctx=guild if scope == PlaylistScope.GUILD.value else author)
            await delete_playlist(self.bot, self.playlist_api, scope, playlist.id, guild or ctx.guild, author or ctx.author)

            await self.send_embed_msg(
                ctx,
                title="Playlist Deleted",
                description=("{name} (`{id}`) [**{scope}**] playlist deleted.").format(name=playlist.name, id=playlist.id, scope=scope_name),
            )

    @commands.cooldown(1, 30, commands.BucketType.member)
    @command_playlist.command(name="dedupe", usage="<playlist_name_OR_id> [args]", cooldown_after_parsing=True)
    async def command_playlist_remdupe(self, ctx: commands.Context, playlist_matches: PlaylistConverter, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            track_objects = [x for x in track_objects if x not in unique_tracks and not unique_tracks_add(x)]
        ...
        if original_count - final_count != 0:
            ...
            await self.send_embed_msg(
                ...
                description=("Removed {track_diff} duplicated tracks from {name} (`{id}`) [**{scope}**] playlist.").format(
                    name=playlist.name,
                    id=playlist.id,
            ...
            await self.send_embed_msg(
                ...
                description=("{name} (`{id}`) [**{scope}**] playlist has no duplicate tracks.").format(name=playlist.name, id=playlist.id, scope=scope_name),
            )

    @command_playlist.command(name="download", usage="<playlist_name_OR_id> [v2=False] [args]", cooldown_after_parsing=True)
    @commands.is_owner()
    @commands.cooldown(1, 30, commands.BucketType.guild)
    async def command_playlist_download(self, ctx: commands.Context, playlist_matches: PlaylistConverter, v2: UserInputOptional[bool] = False, *, scope_data: ScopeParser = None):  # type: ignore
        """Download a copy of a playlist.

        These files can be used with the `;playlist upload` command.
        Melanie v2-compatible playlists can be generated by passing True
        for the v2 variable.

        **Usage**:
        \u200b \u200b \u200b \u200b `;playlist download playlist_name_OR_id [v2=True_OR_False] [args]`
        ...
        **Guild** can be one of the following:
        ...
        Example use:
        \u200b \u200b \u200b \u200b `;playlist download MyGuildPlaylist True`
        \u200b \u200b \u200b \u200b `;playlist download MyGlobalPlaylist False --scope Global`
        \u200b \u200b \u200b \u200b `;playlist download MyPersonalPlaylist --scope User`
        ...
        async with ctx.typing():
            ...
            if not playlist.tracks:
                ctx.command.reset_cooldown(ctx)
                return await self.send_embed_msg(ctx, title="That playlist has no tracks.")
            if version == "v2":
                ...
                playlist_data = {"author": playlist.author, "link": playlist.url, "playlist": song_list, "name": playlist.name}
                ...
                playlist_songs_backwards_compatible = [track["info"]["uri"] for track in playlist.tracks]
            ...
            playlist_data = orjson.dumps(playlist_data)
            to_write = BytesIO()
            ...
            if to_write.getbuffer().nbytes > ctx.guild.filesize_limit - 10000:
                ...
                with tarfile.open(str(temp_tar), "w:gz") as tar:
                    tar.add(str(temp_file), arcname=str(temp_file.relative_to(datapath)), recursive=False)
                try:
                    if os.path.getsize(str(temp_tar)) > ctx.guild.filesize_limit - 10000:
                        await ctx.send("This playlist is too large to be send in this server.")
                    else:
                        await ctx.send(content="Playlist is too large, here is the compressed version.", file=discord.File(str(temp_tar)))
    ...
    @command_playlist.command(name="info", usage="<playlist_name_OR_id> [args]", cooldown_after_parsing=True)
    async def command_playlist_info(self, ctx: commands.Context, playlist_matches: PlaylistConverter, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            async for numb, page in AsyncIter(pages).enumerate(start=1):
                embed = discord.Embed(colour=await ctx.embed_colour(), title=embed_title, description=page)
                author_obj = self.bot.get_user(playlist.author) or playlist.author or ("Unknown")
                embed.set_footer(
    ...
    async def command_playlist_list(self, ctx: commands.Context, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            if scope is None:
                ...
                if not playlists:
                    ctx.command.reset_cooldown(ctx)
                    return await self.send_embed_msg(ctx, title="Playlist Not Found", description=("No saved playlists available in this server.").format())
            else:
                try:
                ...
                if not playlists and specified_user:
                    ...
                    return await self.send_embed_msg(
                        ctx,
                        title="Playlist Not Found",
                        description=("No saved playlists for {scope} created by {author}.").format(scope=name, author=author),
                    )
                elif not playlists:
                    ctx.command.reset_cooldown(ctx)
                    return await self.send_embed_msg(ctx, title="Playlist Not Found", description=("No saved playlists for {scope}.").format(scope=name))
                ...
                playlist_list.append(
                    ("\n" + space * 4).join(
                        (
                            ...
                            ("Author: {name}").format(name=self.bot.get_user(playlist.author) or playlist.author or ("Unknown")),
    ...
    async def command_playlist_queue(self, ctx: commands.Context, playlist_name: str, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            async for track in AsyncIter(to_add):
                ...
                tracklist.append(track_obj)
                playlist = await create_playlist(ctx, self.playlist_api, scope, playlist_name, None, tracklist, author, guild)
        await self.send_embed_msg(
            ctx,
            title="Playlist Created",
            description=("Playlist {name} (`{id}`) [**{scope}**] saved from current queue: {num} tracks added.").format(
                name=playlist.name,
                num=len(playlist.tracks),
                id=playlist.id,
                scope=scope_name,
            ),
            footer=f"Playlist limit reached: Could not add {not_added} tracks." if not_added > 0 else None,
        )

    @command_playlist.command(name="remove", usage="<playlist_name_OR_id> <url> [args]")
    async def command_playlist_remove(self, ctx: commands.Context, playlist_matches: PlaylistConverter, url: str, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            if len(track_list) == len(clean_list):
                return await self.send_embed_msg(ctx, title="URL not in playlist.")
            ...
            if not clean_list:
                ...
                return await self.send_embed_msg(ctx, title="No tracks left, removing playlist.")
            ...
            if del_count > 1:
                await self.send_embed_msg(
                    ctx,
                    title="Playlist Modified",
                    description=("{num} entries have been removed from the playlist {playlist_name} (`{id}`) [**{scope}**].").format(
                        num=del_count,
                        playlist_name=playlist.name,
                        id=playlist.id,
                        scope=scope_name,
                ...
                await self.send_embed_msg(
                    ctx,
                    title="Playlist Modified",
                    description=("The track has been removed from the playlist: {playlist_name} (`{id}`) [**{scope}**].").format(
                        playlist_name=playlist.name,
                        id=playlist.id,
                        scope=scope_name,
                    ),
    ...
    @command_playlist.command(name="save", usage="<name> <url> [args]", cooldown_after_parsing=True)
    @commands.cooldown(1, 60, commands.BucketType.member)
    async def command_playlist_save(self, ctx: commands.Context, playlist_name: str, playlist_url: str, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            if not await self._playlist_check(ctx):
                ...
                return
            player = lavalink.get_player(ctx.guild.id)
            tracklist = await self.fetch_playlist_tracks(ctx, player, Query.process_input(playlist_url, self.local_folder_current_path))
            ...
            if tracklist is not None:
                ...
                playlist = await create_playlist(ctx, self.playlist_api, scope, playlist_name, playlist_url, tracklist, author, guild)
                if playlist is not None:
                    return await self.send_embed_msg(
                        ctx,
                        title="Playlist Created",
                        description=("Playlist {name} (`{id}`) [**{scope}**] saved: {num} tracks added.").format(
                            name=playlist.name,
                            num=len(tracklist),
                            id=playlist.id,
                            scope=scope_name,
                    ...
                    return await self.send_embed_msg(ctx, title="Playlist Couldn't be created", description="Unable to create your playlist.")

    @commands.cooldown(1, 30, commands.BucketType.member)
    @command_playlist.command(name="start", aliases=["play"], usage="<playlist_name_OR_id> [args]", cooldown_after_parsing=True)
    async def command_playlist_start(self, ctx: commands.Context, playlist_matches: PlaylistConverter, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        if dj_enabled and not await self._can_instaskip(ctx, ctx.author):
            ctx.command.reset_cooldown(ctx)
            await self.send_embed_msg(ctx, title="Unable To Play Tracks", description="You need the DJ role to start playing playlists.")
            return False
        async with ctx.typing():
            try:
                playlist, playlist_arg, scope = await self.get_playlist_match(ctx, playlist_matches, scope, author, guild, specified_user)
            ...
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=("{name} - (`{id}`) [**{scope}**]\nAdded {num} tracks to the queue.{maxlength_msg}").format(
                        num=track_len,
    ...
    @commands.cooldown(1, 60, commands.BucketType.member)
    @command_playlist.command(name="update", usage="<playlist_name_OR_id> [args]", cooldown_after_parsing=True)
    async def command_playlist_update(self, ctx: commands.Context, playlist_matches: PlaylistConverter, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            if not await self._playlist_check(ctx):
                ...
                return
            try:
                if not await self.can_manage_playlist(scope, playlist, ctx, author, guild, bypass=True):
                    return
                if playlist.url or getattr(playlist, "id", 0) == 42069:
                    ...
                    ctx.command.reset_cooldown(ctx)
                    return await self.send_embed_msg(ctx, title="Invalid Playlist", description="Custom playlists cannot be updated.")
            except RuntimeError:
                ctx.command.reset_cooldown(ctx)
                return await self.send_embed_msg(
                    ctx,
                ...
                if not added and not removed:
                    return await self.send_embed_msg(
                        ctx,
                        title="Playlist Has Not Been Modified",
                        description=("No changes for {name} (`{id}`) [**{scope}**].").format(id=playlist.id, name=playlist.name, scope=scope_name),
                    )
                ...
                if removed:
                    ...
                    async for i, track in AsyncIter(removed).enumerate(start=1):
                        if len(track.title) > 40:
                            ...
                            track_title = f'{track_title[:40].rstrip(" ")}...'
                        else:
                            track_title = track.title
                        removed_text += f"`{i}.` **[{track_title}]({track.uri})**\n"
                        if i % 10 == 0 or i == total_removed:
                            page_count += 1
                            embed = discord.Embed(title="Tracks removed", colour=_colour, description=removed_text)
                            text = ("Page {page_num}/{total_pages}").format(page_num=page_count, total_pages=total_pages)
                ...
                if added:
                    ...
                    async for i, track in AsyncIter(added).enumerate(start=1):
                        ...
                        if i % 10 == 0 or i == total_added:
                            page_count += 1
                            embed = discord.Embed(title="Tracks added", colour=_colour, description=added_text)
                            text = ("Page {page_num}/{total_pages}").format(page_num=page_count, total_pages=total_pages)
                            embed.set_footer(text=text)
    ...
    @commands.is_owner()
    async def command_playlist_upload(self, ctx: commands.Context, *, scope_data: ScopeParser = None):
        """Uploads a playlist file as a playlist for the bot.

        V2 and old V3 playlist will be slow.
        V3 Playlist made with `;playlist download` will load a lot faster.

        **Usage**:
        \u200b \u200b \u200b \u200b `;playlist upload [args]`
        ...
        **Guild** can be one of the following:
        ...
        Example use:
        \u200b \u200b \u200b \u200b `;playlist upload`
        \u200b \u200b \u200b \u200b `;playlist upload --scope Global`
        \u200b \u200b \u200b \u200b `;playlist upload --scope User`
        ...
        async with ctx.typing():
            ...
            if not ctx.message.attachments:
                await self.send_embed_msg(ctx, title="Please upload the playlist file. Any other message will cancel this operation.")
                try:
                    file_message = await self.bot.wait_for("message", timeout=30.0, check=MessagePredicate.same_context(ctx))
                except TimeoutError:
                    return await self.send_embed_msg(ctx, title="No file detected, try again later.")
            else:
                file_message = ctx.message
            try:
                file_url = file_message.attachments[0].url
            except IndexError:
                return await self.send_embed_msg(ctx, title="Upload cancelled.")
            file_suffix = file_url.rsplit(".", 1)[1]
            if file_suffix != "txt":
                return await self.send_embed_msg(ctx, title="Only Melanie playlist files can be uploaded.")
            try:
                async with self.bot.aio.request("GET", file_url) as r:
                    uploaded_playlist = await r.json(content_type="text/plain", encoding="utf-8", loads=json.loads)
            except UnicodeDecodeError:
                return await self.send_embed_msg(ctx, title="Not a valid playlist file.")
            ...
            if len(track_list) > 10000:
                return await self.send_embed_msg(ctx, title="This playlist is too large.")
            uploaded_playlist_name = uploaded_playlist.get("name", (file_url.split("/")[6]).split(".")[0])
            try:
                if self.api_interface is not None and (
                    not uploaded_playlist_url
                    or not self.match_yt_playlist(uploaded_playlist_url)
                    or not (await self.api_interface.fetch_track(ctx, player, Query.process_input(uploaded_playlist_url, self.local_folder_current_path)))[
                        0
                    ].tracks
                ):
                    if version == "v3":
                        return await self._load_v3_playlist(ctx, scope, uploaded_playlist_name, uploaded_playlist_url, track_list, author, guild)
                    return await self._load_v2_playlist(ctx, track_list, player, uploaded_playlist_url, uploaded_playlist_name, scope, author, guild)
    ...
    @commands.cooldown(1, 60, commands.BucketType.member)
    @command_playlist.command(name="rename", usage="<playlist_name_OR_id> <new_name> [args]", cooldown_after_parsing=True)
    async def command_playlist_rename(self, ctx: commands.Context, playlist_matches: PlaylistConverter, new_name: str, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            await self.send_embed_msg(ctx, title="Playlist Modified", description=msg)
