...
class QueueCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        async def _queue_menu(ctx: commands.Context, pages: list, controls: MutableMapping, message: discord.Message, page: int, timeout: float, emoji: str):
            if message:
    ...
    async def command_queue_clear(self, ctx: commands.Context):
        ...
        if not self._player_check(ctx) or not player.queue:
            return await self.send_embed_msg(ctx, title="There's nothing in the queue.")
        if dj_enabled and not await self._can_instaskip(ctx, ctx.author) and not await self.is_requester_alone(ctx):
            return await self.send_embed_msg(ctx, title="Unable To Clear Queue", description="You need the DJ role to clear the queue.")
        ...
        player.queue.clear()
        await self.send_embed_msg(ctx, title="Queue Modified", description="The queue has been cleared.")

    @command_queue.command(name="clean")
    async def command_queue_clean(self, ctx: commands.Context):
        """Removes songs from the queue if the requester is not in the voice
        channel.
        """
        try:
            player = lavalink.get_player(ctx.guild.id)
        ...
        if not self._player_check(ctx) or not player.queue:
            return await self.send_embed_msg(ctx, title="There's nothing in the queue.")
        if dj_enabled and not await self._can_instaskip(ctx, ctx.author) and not await self.is_requester_alone(ctx):
            return await self.send_embed_msg(ctx, title="Unable To Clean Queue", description="You need the DJ role to clean the queue.")
        clean_tracks = []
        removed_tracks = 0
        ...
        if removed_tracks == 0:
            ...
            await self.send_embed_msg(
                ...
                description=("Removed {removed_tracks} tracks queued by members outside of the voice channel.").format(removed_tracks=removed_tracks),
            )
    ...
    async def command_queue_cleanself(self, ctx: commands.Context):
        ...
        if removed_tracks == 0:
            ...
            await self.send_embed_msg(
                ctx,
                title="Removed Tracks From The Queue",
                description=("Removed {removed_tracks} tracks queued by {member.display_name}.").format(removed_tracks=removed_tracks, member=ctx.author),
            )
    ...
    async def command_queue_shuffle(self, ctx: commands.Context):
        ...
        if dj_enabled and not await self._can_instaskip(ctx, ctx.author) and not await self.is_requester_alone(ctx):
            ...
            return await self.send_embed_msg(ctx, title="Unable To Shuffle Queue", description="You need the DJ role to shuffle the queue.")
        if not self._player_check(ctx):
            ctx.command.reset_cooldown(ctx)
            return await self.send_embed_msg(ctx, title="Unable To Shuffle Queue", description="There's nothing in the queue.")
        try:
            try:
                if (
            ...
            ctx.command.reset_cooldown(ctx)
            return await self.send_embed_msg(ctx, title="Unable To Shuffle Queue", description="Connect to a voice channel first.")
        except IndexError:
            ctx.command.reset_cooldown(ctx)
            return await self.send_embed_msg(ctx, title="Unable To Shuffle Queue", description="Connection to Lavalink has not yet been established.")
        except KeyError:
            ctx.command.reset_cooldown(ctx)
            return await self.send_embed_msg(ctx, title="Unable To Shuffle Queue", description="There's nothing in the queue.")
