...
from audio.errors import (
    DatabaseError,
    SpotifyFetchError,
    TrackEnqueueError,
    YouTubeApiError,
)
from audio.utils import CacheLevel, Notifier
from melanie import log, spawn_task
from melanie import log as log2
...
class AudioAPIInterface:
    ...
    Always tries the Local cache first, then Global cache before making
    API calls.

    """

    def __init__(self, bot: Melanie, config: Config, session: aiohttp.ClientSession, conn: APSWConnectionWrapper, cog: Union[Audio, Cog]) -> None:
        self.bot = bot
        self.config = config
        self.conn = conn
        self.cog = cog
    ...
    async def get_random_track_from_db(self, tries: int = 0) -> Optional[MutableMapping]:
    ...
    async def route_tasks(self, action_type: str = None, data: Union[list[MutableMapping], MutableMapping] = None) -> None:
    ...
    async def run_tasks(self, ctx: Optional[commands.Context] = None, message_id=None) -> None:
        ...
        async with self._lock:
            if lock_id in self._tasks:
                if IS_DEBUG:
                    log.debug("Running database writes for {} ({})", lock_id, lock_author)
                try:
                    ...
                    debug_exc_log(log, exc, "Failed database writes for {} ({})", lock_id, lock_author)
                else:
                    if IS_DEBUG:
                        log.debug("Completed database writes for {} ({})", lock_id, lock_author)
    ...
    async def fetch_spotify_query(
        ...
        youtube_urls = []
        tracks = await self.fetch_from_spotify_api(query_type, uri, params=None, notifier=notifier, ctx=ctx)
        ...
        async for track in AsyncIter(tracks):
            if isinstance(track, str):
                ...
                continue
            (song_url, track_info, uri, artist_name, track_name, _id, _type) = await self.spotify_api.get_spotify_track_info(track, ctx)
            ...
            if not skip_youtube:
                ...
                if youtube_cache:
                    try:
                        (val, last_update) = await self.local_cache_api.youtube.fetch_one({"track": track_info})
                    except Exception as exc:
                        debug_exc_log(log, exc, "Failed to fetch %r from YouTube table", track_info)

                if val is None:
                    try:
                        val = await self.fetch_youtube_query(ctx, track_info, current_cache_level=current_cache_level)
            ...
            if notifier is not None and youtube_api_error and not global_api:
                error_embed = discord.Embed(colour=await ctx.embed_colour(), title="Failing to get tracks, skipping remaining.")
    ...
    async def fetch_from_spotify_api(
        ...
        with contextlib.suppress(KeyError):
            if results["error"]["status"] == 401 and not recursive:
                msg = "The Spotify API key or client secret has not been set properly. \nUse `{prefix}audioset spotifyapi` for instructions."
                raise SpotifyFetchError(msg)
            elif recursive:
                return {"next": None}
        if recursive:
            return results
        ...
        while True:
            ...
            try:
                if results.get("next") is None:
                    break
                results = await self.fetch_from_spotify_api(query_type, uri, results["next"], params, notifier=notifier)
                continue
            except KeyError as e:
                msg = "This doesn't seem to be a valid Spotify playlist/album URL or code."
                raise SpotifyFetchError(msg) from e
    ...
    async def spotify_query(
        ...
        if query_type == "track" and cache_enabled:
            try:
                (val, last_update) = await self.local_cache_api.spotify.fetch_one({"uri": f"spotify:track:{uri}"})
            except Exception as exc:
                debug_exc_log(log, exc, "Failed to fetch 'spotify:track:{}' from Spotify table", uri)
                val = None
        else:
            val = None
        youtube_urls = []
        if val is None:
            urls = await self.fetch_spotify_query(ctx, query_type, uri, notifier, skip_youtube, current_cache_level=current_cache_level)
    ...
    async def spotify_enqueue(
        ...
        """Queries the Database then falls back to Spotify and YouTube APIs then
        Enqueued matched tracks.
        ...
        try:
            ...
            tracks_from_spotify = await self.fetch_from_spotify_api(query_type, uri, params=None, notifier=notifier)
            total_tracks = len(tracks_from_spotify)
            if total_tracks < 1 and notifier is not None:
                lock(ctx, False)
                embed3 = discord.Embed(colour=await ctx.embed_colour(), title="This doesn't seem to be a supported Spotify URL or code.")
            ...
            async for track_count, track in AsyncIter(tracks_from_spotify, delay=0.1).enumerate(start=1):

                async def _load_track(track_count, track):
                    youtube_api_error = None
                    nonlocal consecutive_fails
                    nonlocal has_not_allowed
                    nonlocal enqueued_tracks
                    nonlocal loaded_tracks
                    async with sem, asyncio.timeout(6):
                        (song_url, track_info, uri, artist_name, track_name, _id, _type) = await self.spotify_api.get_spotify_track_info(track, ctx)
                        val = None
                        llresponse = None
                        try:
                            if not (player.connected and player.ready and lavalink.get_player(ctx.guild.id)):
                                return
                            val = await self.fetch_youtube_query(ctx, track_info, current_cache_level=current_cache_level)
                        except YouTubeApiError as err:
                            val = None
                        ...
                        if not youtube_api_error:
                            ...
                            if isinstance(llresponse, LoadResult):
                                ...
                                if not result:
                                    ...
                                    try:
                                        ...
                                        lock(ctx, False)
                                        error_embed = discord.Embed(colour=await ctx.embed_colour(), title="Player timeout, skipping remaining tracks.")
                                        if notifier is not None:
                                            await notifier.update_embed(error_embed)
                                        return
                            ...
                            track_object = []
                        if ((track_count % 2 == 0) or (track_count == total_tracks)) and notifier is not None:
                            key = "lavalink"
                            seconds = "???"
                            second_key = None
                            await notifier.notify_user(current=track_count, total=total_tracks, key=key, seconds_key=second_key, seconds=seconds)
                        if (youtube_api_error and not global_entry) or consecutive_fails >= (20 if global_entry else 10):
                            error_embed = discord.Embed(colour=await ctx.embed_colour(), title="Failing to get tracks, skipping remaining.")
                            if notifier is not None:
                                await notifier.update_embed(error_embed)
                            ...
                            try:
                                with contextlib.suppress(asyncio.TimeoutError):
                                    async with asyncio.timeout(3):
                                        while loaded_tracks != track_count:
                                            if not (player.connected and player.ready):
                                                return
                                            await asyncio.sleep(0.01)
                                if player.connected and player.ready and lavalink.get_player(ctx.guild.id):
                                        ...
                                        if guild_data["maxlength"] > 0:
                                            if self.cog.is_track_length_allowed(single_track, guild_data["maxlength"]):
                                                ...
                                                single_track.extras.update(
                                                    {"enqueue_time": int(time.time()), "vc": player.channel.id, "requester": ctx.author.id},
                                                )
                ...
                tasks.append(spawn_task(_load_track(track_count, track), self.active_tasks))
            for t in asyncio.as_completed(tasks):
                with log2.catch(exclude=asyncio.CancelledError), contextlib.suppress(asyncio.TimeoutError):
                    await t

            ##### end
            if enqueue and tracks_from_spotify:
                if total_tracks > enqueued_tracks:
                    maxlength_msg = (" {bad_tracks} tracks cannot be queued.").format(bad_tracks=(total_tracks - enqueued_tracks))
                else:
                    maxlength_msg = ""
                ...
                embed = discord.Embed(
                    ...
                    description=("Added {num} tracks to the queue.{maxlength_msg}").format(num=enqueued_tracks, maxlength_msg=maxlength_msg),
                )
                if not guild_data["shuffle"] and queue_dur > 0:
                    embed.set_footer(
                        text=("{time} until start of playlist playback: starts at #{position} in queue").format(
    ...
    async def fetch_youtube_query(self, ctx: commands.Context, track_info: str, current_cache_level: CacheLevel = CacheLevel.all()) -> Optional[str]:
        """Call the Youtube API and returns the youtube URL that the query
        matched.
        """
        return await self.youtube_api.get_call(track_info)

    async def fetch_from_youtube_api(self, ctx: commands.Context, track_info: str) -> Optional[str]:
        """Gets an YouTube URL from for the query."""
        return await self.fetch_youtube_query(ctx, track_info)
    ...
    async def fetch_track(
        ...
        """A replacement for :code:`lavalink.Player.load_tracks`. This will try to
        get a valid cached entry first if not found or if in valid it will then
        call the lavalink API.
        ...
        if cache_enabled and not forced and not query.is_local:
            try:
                (val, last_updated) = await self.local_cache_api.lavalink.fetch_one({"query": query_string})
            except Exception as exc:
            ...
            if val and not forced and isinstance(val, dict):
                ...
                val = None
        if globaldb_toggle and not val and should_query_global and not forced and not query.is_local and not query.is_spotify:
            valid_global_entry = False
            with contextlib.suppress(Exception):
                ...
                if results.load_type in [LoadType.PLAYLIST_LOADED, LoadType.TRACK_LOADED, LoadType.SEARCH_RESULT, LoadType.V2_COMPAT]:
        ...
        if valid_global_entry:
            ...
            try:
                ...
                raise TrackEnqueueError from e
        if results is None:
            results = LoadResult({"loadType": "LOAD_FAILED", "playlistInfo": {}, "tracks": []})
            valid_global_entry = False
        update_global = globaldb_toggle and not valid_global_entry and self.global_cache_api.has_api_key
        with contextlib.suppress(Exception):
            if update_global and not query.is_local and not results.has_error and len(results.tracks) >= 1:
                global_task = ("global", {"llresponse": results, "query": query})
                self.append_task(ctx, *global_task)
        if cache_enabled and results.load_type and not results.has_error and not query.is_local and results.tracks:
            try:
                time_now = int(datetime.datetime.now(datetime.timezone.utc).timestamp())
                data = json.dumps(results._raw)
                if all(k in data for k in ["loadType", "playlistInfo", "isSeekable", "isStream"]):
                    task = ("insert", ("lavalink", [{"query": query_string, "data": data, "last_updated": time_now, "last_fetched": time_now}]))
                    self.append_task(ctx, *task)
            except Exception as exc:
                debug_exc_log(log, exc, "Failed to enqueue write task for %r to Lavalink table", query_string)
    ...
    async def autoplay(self, player: lavalink.Player, playlist_api: PlaylistWrapper):
        ...
        if autoplaylist["enabled"]:
            try:
                playlist = await get_playlist(autoplaylist["id"], autoplaylist["scope"], self.bot, playlist_api, player.guild, player.guild.me)
        ...
        if tracks:
            ...
            while not valid and multiple:
                ...
                if (not query.valid) or (query.is_local and query.local_track_path is not None and not query.local_track_path.exists()):
                    continue
                notify_channel = self.bot.get_channel(notify_channel_id)
                if not await self.cog.is_query_allowed(self.config, notify_channel, f"{track.title} {track.author} {track.uri} {query}", query_obj=query):
                    if IS_DEBUG:
                        log.debug("Query is not allowed in %r ({})", player.guild.name, player.guild.id)
                    continue
                valid = True
            track.extras.update({"autoplay": True, "enqueue_time": int(time.time()), "vc": player.channel.id, "requester": player.guild.me.id})
            player.add(player.guild.me, track)
            self.bot.dispatch("red_audio_track_auto_play", player.guild, track, player.guild.me, player)
            if notify_channel_id:
                await self.config.guild_from_id(guild_id=player.guild.id).currently_auto_playing_in.set([notify_channel_id, player.channel.id])
            else:
                await self.config.guild_from_id(guild_id=player.guild.id).currently_auto_playing_in.set([])
            if not player.current:
                await player.play()
