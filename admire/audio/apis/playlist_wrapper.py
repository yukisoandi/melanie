...
class PlaylistWrapper:
    def __init__(self, bot: Melanie, config: Config, conn: APSWConnectionWrapper) -> None:
        self.bot = bot
        self.database = conn
        self.config = config
    ...
    async def fetch_all(self, scope: str, scope_id: int, author_id=None) -> list[PlaylistFetchResult]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            if author_id is not None:
                ...
                for future in concurrent.futures.as_completed(
                    [executor.submit(self.database.cursor().execute, self.statement.get_all, ({"scope_type": scope_type, "scope_id": scope_id}))],
                ):
                    try:
                        row_result = future.result()
                    except Exception as exc:
            ...
            for future in concurrent.futures.as_completed(
                [
                    executor.submit(
                        ...
                        ({"scope_type": scope_type, "playlist_name": playlist_name, "playlist_id": playlist_id}),
                    ),
                ],
            ):
                try:
    ...
    async def handle_playlist_user_id_deletion(self, user_id: int) -> None:
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
