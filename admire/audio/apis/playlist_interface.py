...
from audio.errors import NotAllowed
...
class Playlist:
    ...
    async def edit(self, data: MutableMapping):
        ...
        # Disallow ID editing
        if "id" in data:
            msg = "Playlist ID cannot be edited."
    ...
    def to_json(self) -> MutableMapping:
        ...
        """
        return {"id": self.id, "author": self.author_id, "guild": self.guild_id, "name": self.name, "playlist_url": self.url, "tracks": self.tracks}

    @classmethod
    async def from_json(cls, bot: Melanie, playlist_api: PlaylistWrapper, scope: str, playlist_number: int, data: PlaylistFetchResult, **kwargs) -> "Playlist":
...
async def get_all_playlist_for_migration23(
    ...
    if scope == PlaylistScope.GLOBAL.value:
        return [
            await PlaylistCompat23.from_json(bot, playlist_api, scope, playlist_number, playlist_data, guild=guild, author=int(playlist_data.get("author", 0)))
            async for playlist_number, playlist_data in AsyncIter(playlists.items())
        ]
    elif scope == PlaylistScope.USER.value:
        return [
            await PlaylistCompat23.from_json(bot, playlist_api, scope, playlist_number, playlist_data, guild=guild, author=int(user_id))
...
async def get_playlist(
    ...
    if not (playlist_data and playlist_data.playlist_id):
        msg = f"That playlist does not exist for the following scope: {scope}"
