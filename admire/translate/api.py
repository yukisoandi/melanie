...
from melaniebot.core import Config, VersionInfo, commands, version_info
from melaniebot.core.bot import Melanie
from regex.regex import Pattern

from melanie import aiter
...
FLAG_REGEX: Pattern[str] = re.compile(r"|".join(rf"{re.escape(f)}" for f in FLAGS))


class FlagTranslation(Converter):
    """This will convert flags and languages to the correct code to be used by the
    API.
        ...
        if argument in FLAGS:
            ...
            for lang in FLAGS:
                ...
                if FLAGS[lang]["code"] in argument.lower() and len(argument) == 2:
                    ...
                    break
        if not result:
            msg = f'Language "{argument}" not found'
...
class AzureTranslateAPI:
    ...
    async def add_detect(self, guild: Optional[discord.Guild]) -> None:
        if guild:
    ...
    async def add_requests(self, guild: Optional[discord.Guild], message: str) -> None:
        if guild:
    ...
    async def check_bw_list(self, guild: discord.Guild, channel: discord.TextChannel, member: Union[discord.Member, discord.User]) -> bool:
        can_run = True
    ...
    async def detect_language(self, text: str) -> list[list[dict[str, str]]]:
        ...
        key2 = f"detect:{xxhash.xxh64_hexdigest( text.strip().lower())}"
        data = await self.bot.redis.hget("translations_cache", key2)
        if data:
            ...
            async with self.bot.aiohttpx.post(url, params=params, headers=headers, json=json_payload) as resp:
                resp.raise_for_status()
                data = await resp.json()

                await self.bot.redis.hset("translations_cache", key2, msgpack.packb(data))
    ...
    async def translation_embed(
        ...
        em.set_author(name=f"{author.display_name} said:", icon_url=str(author.avatar_url))
        detail_string = f"{translation[1].upper()} to {translation[2].upper()} | Requested by " + (str(requestor) if requestor else str(author))
        em.set_footer(text=detail_string.lower())
        return em

    async def translate_text(self, from_lang: str, target: str, text: str) -> Optional[str]:
        """Request to translate the text."""
        key2 = xxhash.xxh32_hexdigest(f"{from_lang}{target}{text}")
        translated_text = await self.bot.redis.hget("translations_cache", key2)
        if not translated_text:
            ...
            params = {"api-version": "3.0", "from": from_lang, "to": [target]}
            url = f"{BASE_URL}/translate"
            json_payload = [{"Text": text}]
            async with self.bot.aiohttpx.post(url, params=params, headers=headers, json=json_payload) as resp:
                resp.raise_for_status()
                data = await resp.json()
            translated_text: str = data[0]["translations"][0]["text"]
            if translated_text:
                await self.bot.redis.hset("translations_cache", key2, lz4.frame.compress(translated_text.encode()))
        else:
            translated_text = lz4.frame.decompress(translated_text)
    ...
    async def on_message_no_cmd(self, message: discord.Message) -> None:
        ...
        if not await self.check_ignored_channel(message):
            return
        if flag := FLAG_REGEX.search(message.clean_content):
    ...
    async def translate_message(self, message: discord.Message, flag: str, reacted_user: Optional[discord.Member] = None) -> None:
        guild = cast(discord.Guild, message.guild)
        channel = cast(discord.TextChannel, message.channel)
        if message.id in self.cache["cooldown_translations"]:
            if flag in self.cache["cooldown_translations"][message.id]["past_flags"]:
                return
            ...
            if time.time() < self.cache["cooldown_translations"][message.id]["wait"]:
                ...
                await channel.send("You're translating too many messages!", delete_after=delete_after)
                return
        to_translate = None
        if message.embeds == []:
        ...
        from_lang = detected_lang[0]["language"].upper()
        ...
        cooldown["past_flags"].append(flag)
        ...
        if channel.permissions_for(guild.me).embed_links:
            ...
            msg = "{author} said:\n{translated_text}".format(author=author)
    ...
    async def local_perms(self, guild: discord.Guild, author: discord.Member) -> bool:
        ...
        try:
            return await self.bot.allowed_by_whitelist_blacklist(author, who_id=author.id, guild_id=guild.id, role_ids=[r.id for r in author.roles])
        except AttributeError:
    ...
    async def check_ignored_channel(self, message: discord.Message, reacting_user: Optional[discord.Member] = None) -> bool:
        ...
        surpass_ignore = (
            isinstance(channel, discord.abc.PrivateChannel) or perms.manage_guild or await self.bot.is_owner(author) or await self.bot.is_admin(author)
        )
        if surpass_ignore:
            return True
