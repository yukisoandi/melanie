    ...
    async def format_bot_help(self, ctx: Context, help_settings: HelpSettings, get_pages: bool = False):
        if await ctx.embed_requested():
            ...
            for i, vals in enumerate(pagify(cat_titles, page_length=1000)):
                emb["fields"].append(EmbedField("List of Categories" if i < 1 else (f"{EMPTY_STRING} "), vals, False))

            pages = await self.make_embeds(ctx, emb, help_settings=help_settings)
            if get_pages:
                return pages
            else:
                await self.send_pages(ctx, pages, embed=True, help_settings=help_settings, emoji_mapping=filtered_categories)
        else:
    ...
    async def format_category_help(self, ctx: Context, obj: Category, help_settings: HelpSettings, get_pages: bool = False, **kwargs):
        ...
        if await ctx.embed_requested():
            ...
            for cog_name, data in coms:
                ...
                cog_text = [f"{ctx.clean_prefix}{name:<15}{command.aliases}" for name, command in sorted(data.items())]
                # need to customize this
                for i, page in enumerate(cog_text[n : n + 7] for n in range(0, len(cog_text), 7)):
                    field = EmbedField(title if i < 1 else f"{title} (continued)", box("\n".join(page), lang="css"), True)
