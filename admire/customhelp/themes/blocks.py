...
def grouper(a, n):
    return [a[k : k + n] for k in range(0, len(a), n)]
...
class Blocks(ThemesMeta):
    ...
    async def format_category_help(self, ctx: Context, obj: Category, help_settings: HelpSettings, get_pages: bool = False, **kwargs):
        ...
        for _cog_name, data in coms:
            all_cog_text.extend(ctx.clean_prefix + x for x in data)
        ...
        if await ctx.embed_requested():
            ...
            await self.send_pages(ctx, list(map(box, pagify(all_cog_str, shorten_by=0, page_length=2042))), embed=False, help_settings=help_settings)
    ...
    async def format_cog_help(self, ctx: Context, obj: commands.Cog, help_settings: HelpSettings) -> None:
        ...
        cmd_list = tabulate(grouper([ctx.clean_prefix + x for x in sorted(coms.keys())], 3), tablefmt="plain")
        ...
        if await ctx.embed_requested():
            ...
            await self.send_pages(ctx, list(map(box, pagify(cmd_list, shorten_by=0, page_length=2042))), embed=False, help_settings=help_settings)
