...
class MinimalHelp(ThemesMeta):
    ...
    async def format_bot_help(self, ctx: Context, help_settings: HelpSettings, get_pages: bool = False):
        ...
        if get_pages:
            return text_no
        await self.send_pages(ctx, text_no, embed=False, help_settings=help_settings, emoji_mapping=filtered_categories)
    ...
    async def format_category_help(self, ctx: Context, obj: Category, help_settings: HelpSettings, get_pages: bool = False, **kwargs):
        ...
        for _cogname, data in coms:
            full_text += "\n".join(f"`{name:<{spacing}}`:{command.format_shortdoc_for_context(ctx)}" for name, command in data.items())
            full_text += "\n"
        text_no = list(pagify(full_text))
        if get_pages:
            return text_no
        await self.send_pages(ctx, text_no, embed=False, help_settings=help_settings)
    ...
    async def format_cog_help(self, ctx: Context, obj: commands.Cog, help_settings: HelpSettings) -> None:
        ...
        full_text += "\n".join(f"`{name:<{spacing}}:`{command.format_shortdoc_for_context(ctx)}" for name, command in sorted(coms.items()))
    ...
    async def format_command_help(self, ctx: Context, obj: commands.Command, help_settings: HelpSettings) -> None:
        ...
        if subcommands:
            ...
            subtext = "\n" + "\n".join(f"`{name:<{spacing}}`:{command.format_shortdoc_for_context(ctx)}" for name, command in sorted(subcommands.items()))
            for i, page in enumerate(pagify(subtext, shorten_by=0)):
                title = "**__Subcommands:__**" if i == 0 else ""
