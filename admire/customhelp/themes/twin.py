        ...
        if await ctx.embed_requested():
            ...
            for cat in filtered_categories:
                if cat.cogs:
                    ...
                    for i, _page in enumerate(pagify(cog_names, page_length=1000, shorten_by=0)):
                        title = (str(cat.reaction) or "") + f"  __{cat.name.capitalize()}:__ " if i == 0 else EMPTY_STRING
    ...
    async def format_category_help(self, ctx: Context, obj: Category, help_settings: HelpSettings, get_pages: bool = False, **kwargs):
        ...
        if await ctx.embed_requested():
            ...
            for cog_name, data in coms:
                title = f"__**{cog_name}**__" if cog_name else ("**No Category:**")
                cog_text = ", ".join(f"`{name}`" for name, command in sorted(data.items()))
                for i, page in enumerate(pagify(cog_text, page_length=1000, delims=[","], shorten_by=0)):
                    if i > 0:
                        title = f"**{cog_name} (continued):**"
                    field = EmbedField(title, page[1:] if page.startswith(",") else page, False)  # precision matters xd
