...
from melaniebot.core.utils import menus, predicates
...
class CustomHelp(commands.Cog):
    ...
    def __init__(self, bot: Melanie) -> None:
        self.bot = bot
        self.feature_list = {"category": "format_category_help", "main": "format_bot_help", "cog": "format_cog_help", "command": "format_command_help"}
        self.config = Config.get_conf(self, identifier=278198241009, force_registration=True)  # I'm gonna regret this
        self.chelp_global = {
            "categories": [],
    ...
    async def refresh_arrows(self) -> None:
    ...
    async def _setup(self) -> None:
        ...
        if all(theme.values()) is not None:
            for feature in theme:
                if theme[feature]:
                    inherit_feature = getattr(themes.list[theme[feature]], self.feature_list[feature])
                    setattr(main_theme, self.feature_list[feature], MethodType(inherit_feature, main_theme))
        self.bot.set_help_formatter(main_theme)

    @commands.Cog.listener("on_cog_add")
    async def handle_new_cog_entries(self, cog: commands.Cog) -> None:
    ...
    async def refresh(self, ctx) -> None:
        """Force refresh the list of categories, This would reset all the
        uninstalled/unloaded cogs and will put them into uncategorised.
        """
    ...
    async def auto(self, ctx) -> None:
            ...
            if path.isfile(check):
                with open(check, encoding="utf-8") as f:
                    try:
                        ...
                        print(f"[ERROR] Invaild JSON in cog {k}")
                        data[k] = []
        ...
        for i in [box(page, lang="yaml") for page in pagify(yaml.dump(final), shorten_by=0, page_length=1990)]:
            await ctx.send(i)
    ...
    async def show(self, ctx) -> None:
        ...
        other_settings = [f"`{setting_mapping[i]:<15}`: {j}" for i, j in settings.items() if i in setting_mapping]

        val = await self.config.theme()
        val = "\n".join([f"`{i:<10}`: " + (j or "default") for i, j in val.items()])
        emb = discord.Embed(title="Custom help settings", description=f"Cog Version: {self.__version__}", color=await ctx.embed_color())
        emb.add_field(name="Theme", value=val)
        emb.add_field(name="Other Settings", value="\n".join(other_settings), inline=False)

        emb.add_field(name="Arrows", value="\n".join(f"`{i:<7}`: {j}" for i, j in ARROWS.items()), inline=False)

        if blocklist["nsfw"] or blocklist["dev"]:
            emb.add_field(
                name=EMPTY_STRING,
                value="".join(f"**{i.capitalize()} categories:**\n{', '.join(blocklist[i])}\n" for i in blocklist if blocklist[i]) or EMPTY_STRING,
                inline=False,
            )
        await ctx.send(embed=emb)

    @chelp.command(name="set")
    async def set_formatter(self, ctx, setval: bool) -> None:
        """Set to toggle custom formatter or the default help formatter\n`;chelp
        set 0` to turn custom off \n`;chelp set 1` to turn it on.
        """
    ...
    async def create(self, ctx, *, yaml_txt=None):
        ...
        if yaml_txt:
            ...
            await ctx.send("Your next message should be a yaml with the specfied format as in the docs\nExample:\ncategory1:\n - Cog1\n - Cog2")
            try:
                msg = await self.bot.wait_for("message", timeout=180, check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
                content = msg.content
            except TimeoutError:
        ...
        for i in parsed_data.values():
            if any(not isinstance(j, str) for j in i):
        ...
        all_cogs = set(self.bot.cogs.keys())
        uncategorised = all_cogs - set(chain(*(category.cogs for category in GLOBAL_CATEGORIES[:-1])))
        failed_cogs = []
        success_cogs = []
        ...
        for page in pagify(
            (f"Successfully loaded: `{'`,`'.join(success_cogs)}`" if success_cogs else ("Nothing successful"))
            + (
    ...
    async def edit(self, ctx, *, yaml_txt=None):
        ...
        for i in parsed_data.values():
            if any(isinstance(j, str) for j in i):
                await ctx.send("Invalid Format, Likely you added an extra ':' or '-'")
                return
        # Some more rearrangement parsed_data = {category:[('name', 'notrandom'), ('emoji', 'asds'), ('emoji', '😓'), ('desc', 'this iasdiuasd')]}
        parsed_data = {i: [(k, v) for f in my_list for k, v in f.items()] for i, my_list in parsed_data.items()}
        ...
        uncat_name = GLOBAL_CATEGORIES[-1].name
        already_present_emojis = [str(i.reaction) for i in GLOBAL_CATEGORIES if i.reaction] + list(ARROWS.values())
        ...
        def validity_checker(category, item):
            ...
            if item[0] in check:
                if item[0] == "name" and item[1] not in available_categories or item[0] not in ["name", "reaction"]:
                    return item[1]
        ...
        for page in pagify(
            "The following things failed:\n" + "\n".join([f"`{reason[0]}`: {reason[1]}  failed in `{category}`" for reason, category in failed])
            if failed
            else "Successfully added the edits",
        ):
            await ctx.send(page)
        await self.refresh_cache()
    ...
    async def list(self, ctx) -> None:
        ...
        all_cogs = set(self.bot.cogs.keys())
        uncategorised = all_cogs - set(chain(*(category["cogs"] for category in available_categories_raw)))
        joined = "Set Categories:\n" if len(available_categories_raw) > 1 else ("Set Category:\n")
    ...
    @chelp.command()
    async def load(self, ctx, theme: str, feature: str) -> None:
        """Load another preset theme.\nUse `;chelp load <theme> all` to load
        everything from that theme.
        """
        if isinstance(self.bot._help_formatter, commands.help.RedHelpFormatter):
            await ctx.send("You are not using the custom formatter")
            return

        def loader(theme, feature) -> bool:
            ...
            if hasattr(inherit_theme, self.feature_list[feature]):
                ...
                setattr(self.bot._help_formatter, self.feature_list[feature], MethodType(inherit_feature, self.bot._help_formatter))
                return True
            return False
    ...
    async def reset(self, ctx) -> None:
        ...
        to revert back to the old help.
        """
        ...
        if pred.result is True:
            ...
            await self.config.theme.set({"cog": None, "category": None, "command": None, "main": None})
            await self.refresh_cache()
    ...
    async def hard(self, ctx):
        ...
        await ctx.send("Warning: You are about to delete EVERYTHING!, type `y` to continue else this will abort")
        try:
    ...
    @chelp.command()
    async def unload(self, ctx, feature: str) -> None:
        ...
        if feature in self.feature_list:
            setattr(
                ...
                MethodType(getattr(BaguetteHelp, self.feature_list[feature]), self.bot._help_formatter),
            )
        else:
    ...
    async def all(self, ctx):
        ...
        await ctx.send("Warning: You are about to delete all your categories, type `y` to continue else this will abort")
        try:
            msg = await self.bot.wait_for("message", check=lambda m: m.author == ctx.author and m.channel == ctx.channel, timeout=60)
        except TimeoutError:
            return await ctx.send("Timed out, please try again.")
    ...
    async def category(self, ctx, *categories: str) -> None:
        """Remove a multiple categories."""
        # from ;load
        category_names = {cat.rstrip(",") for cat in categories}
        to_config = []
        invalid = []
        ...
        for category in category_names:
            for ind in range(len(all_cat)):
                ...
                if category == GLOBAL_CATEGORIES[-1].name:
                    text += f"You can't remove {category} cause it is where the uncategorised cogs go into\n\n"
                else:
        ...
        text += "Sucessfully removed: " + (", ".join(all_cat[x] for x in to_config) + "\n") if to_config else ""

        if invalid:
            text += "These categories aren't present in the list:\n" + ",".join(invalid)
        await self.refresh_cache()
        await ctx.send(text)

    @remove.command(aliases=["cogs"], require_var_positional=True)
    async def cog(self, ctx, *cog_names: str) -> None:
        """Remove a cog(s) from across categories."""
        # From Core ;load xD, using set to avoid dupes
        cog_names: set[str] = {cog.rstrip(",") for cog in cog_names}
        ...
        for cog_name in cog_names:
            ...
            if self.bot.get_cog(cog_name):
                ...
                if index is not None:
                    ...
                    await ctx.send(f"Something errored out, kindly report to the owner of this cog, \ncog name:{cog_name}")
            else:
        ...
        if to_config:
            ...
            for thing in sorted(to_config, key=lambda x: x[0]):
                if last == thing[0]:
                    ...
                    text += f"From {GLOBAL_CATEGORIES[thing[0]].name}:\n - {thing[1]}\n"
                    last = thing[0]
        if uncat:
            text += "The following cogs are present in 'uncategorised' and cannot be removed:\n" + ", ".join(uncat)
        if invalid:
    ...
    async def usereactions(self, ctx, toggle: bool) -> None:
    ...
    async def thumbnail(self, ctx, url: str = None) -> None:
        """Set your thumbnail image here.\n use `;chelp settings thumbnail` to
        reset this.
        """
        if url:
    ...
    async def usereply(self, ctx, option: bool) -> None:
    ...
    async def usebuttons(self, ctx, option: bool) -> None:
    ...
    @settings.command()
    async def timeout(self, ctx, wait: int) -> None:
    ...
    async def deletemessage(self, ctx, toggle: bool) -> None:
    ...
    async def arrows(self, ctx, *, correct_txt=None):
        ...
        already_present_emojis = [str(i.reaction) for i in GLOBAL_CATEGORIES if i.reaction] + list((await self.config.settings.arrows()).values())
        ...
        async with self.config.settings.arrows() as conf:
            for k, v in parsed_data.items():
                conf[k] = v
        await ctx.send("Successfully added the changes:\n" + "\n".join(f"`{i} `: {j}" for i, j in parsed_data.items()))
        await self.refresh_arrows()

    @chelp.group()
    async def nsfw(self, ctx) -> None:
    ...
    async def add_nsfw(self, ctx, category: str):
        ...
        if cat_obj := get_category(category):
            if "Core" in cat_obj.cogs:
                return await ctx.send("This category contains Core cog and shouldn't be hidden under any circumstances")
    ...
    async def remove_nsfw(self, ctx, category: str) -> None:
        """Remove categories from the nsfw list."""
        cat_obj = get_category(category) or (category if category in await self.config.blacklist.nsfw() else None)
        if cat_obj:
            async with self.config.blacklist.nsfw() as conf:
                if category in conf:
    ...
    async def remove_dev(self, ctx, category: str) -> None:
        """Remove categories from the dev list."""
        cat_obj = get_category(category) or (category if category in await self.config.blacklist.dev() else None)
        if cat_obj:
            async with self.config.blacklist.dev() as conf:
                if category in conf:
                    conf.remove(category)
    ...
    async def listthemes(self, ctx) -> None:
    ...
    async def reorder(self, ctx, *, categories: str = None):
        ...
        if categories:
            ...
            await ctx.send("Your next message should be valid category names each in a new line\nExample:\ngeneral\nfun\nmoderation\n")
            try:
                msg = await self.bot.wait_for("message", timeout=180, check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
                content = msg.content
            except TimeoutError:
    ...
    @commands.command(aliases=["findcat"], hidden=True)
    async def findcategory(self, ctx, *, command) -> None:
