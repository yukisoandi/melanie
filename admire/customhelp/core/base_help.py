...
from melaniebot.core.commands.context import Context
from melaniebot.core.commands.help import (
    HelpSettings,
    NoCommand,
    NoSubCommand,
    dpy_commands,
...
HelpTarget = Union[commands.Command, commands.Group, commands.Cog, Category, dpy_commands.bot.BotBase, str]
...
class BaguetteHelp(commands.RedHelpFormatter):
    ...
    async def get_category_help_mapping(self, ctx, category, help_settings: HelpSettings, bypass_checks: bool = False):
        ...
        for cogname in sorted_cogs:
            ...
            if ((not cogname) or cog) and ((isuncategory and cogname is None) or (cogname in category.cogs)):
                cm = await self.get_cog_help_mapping(ctx, cog, help_settings=help_settings)
                if cm:
                    sorted_iterable.append((cogname, cm))
        return sorted_iterable

    async def send_help(self, ctx: Context, help_for: HelpTarget = None, *, from_help_command: bool = False) -> None:
    ...
    async def format_cog_help(self, ctx: Context, obj: commands.Cog, help_settings: HelpSettings) -> None:
        ...
        if await ctx.embed_requested():
            ...
            if coms:
                ...
                command_text = "\n".join(
                    shorten_line(f"`{name:<{spacing}}:`{command.format_shortdoc_for_context(ctx)}") for name, command in sorted(coms.items())
                )
    ...
    async def format_command_help(self, ctx: Context, obj: commands.Command, help_settings: HelpSettings) -> None:
        ...
        if await ctx.embed_requested():
            ...
            if subcommands:
                ...
                subtext = "\n" + "\n".join(
                    shorten_line(f"`{name:<{spacing}}:`{command.format_shortdoc_for_context(ctx)}") for name, command in sorted(subcommands.items())
                )
    ...
    async def make_embeds(self, ctx, embed_dict: dict, help_settings: HelpSettings):
    ...
    async def send_pages(
        ...
        if not (channel_permissions.add_reactions and help_settings.use_menus):
            ...
            if (
                ...
                async def _delete_delay_help(channel: discord.TextChannel, messages: list[discord.Message], delay: int) -> None:
                    await asyncio.sleep(delay)
                    await mass_purge(messages, channel)

                asyncio.create_task(_delete_delay_help(destination, messages, delete_delay))
        else:
            trans = {"left": prev_page, "cross": close_menu, "right": next_page}
            ...
            if add_emojis and emoji_mapping:
                ...
                for cat in emoji_mapping:
                    if cat.reaction:
                        final_menu.add_button(await react_page(ctx, cat.reaction, help_settings, bypass_checks=True))
                final_menu.add_button(await home_page(ctx, emoji_converter(ctx.bot, ARROWS["home"]), help_settings))
            await final_menu.start(ctx)

    async def blacklist(self, ctx, name) -> bool:
        """Some blacklist checks utils Returns true if needed to be shown."""
        blocklist = await self.config.blacklist()
        a = (ctx.channel.is_nsfw() if hasattr(ctx.channel, "is_nsfw") else True) or name not in blocklist["nsfw"]
    ...
    async def filter_categories(self, ctx, categories: list) -> list:
        """Applies blacklist to all the categories, Filters based on the current
        context.
        """
        blocklist = await self.config.blacklist()
        ...
        return [
            ...
            if ((ctx.channel.is_nsfw() if hasattr(ctx.channel, "is_nsfw") else True) or name not in blocklist["nsfw"])
