...
def get_perms(command):
    ...
    if (perms := command.requires.bot_perms) and (perms_list := ", ".join(neat_format(i) for i, j in perms if j)):
        final_perms += f"Bot Permission(s): {perms_list}"
...
async def react_page(ctx, emoji, help_settings, bypass_checks: bool = False):
    for x in GLOBAL_CATEGORIES:
        if x.reaction == emoji:
            category = x
            break
    pages = await ctx.bot._help_formatter.format_category_help(ctx, category, help_settings, get_pages=True, bypass_checks=bypass_checks)
    if pages:

        async def action(menu, payload) -> None:
            ...
            if len(pages) == 1:
                ...
                asyncio.create_task(menu.add_button(prev_page(ARROWS["left"]), react=True, interaction=payload))
                asyncio.create_task(menu.add_button(next_page(ARROWS["right"]), react=True, interaction=payload))
...
def first_page(emoji):
    ...
    return menus.Button(emoji, go_to_first_page, position=menus.First(), skip_if=_skip_double_triangle_buttons)
