...
from melaniebot.core.utils import AsyncIter, menus
from melaniebot.core.utils.chat_formatting import box, escape, humanize_list, pagify
...
class CommandObj:
    ...
    async def redact_author_ids(self, user_id: int):
        ...
        for guild_id in all_guilds:
            ...
            async with self.config.guild_from_id(guild_id).commands() as all_commands:
                async for _com_name, com_info in AsyncIter(all_commands.items(), steps=100):
                    if not com_info:
                        continue
    ...
    async def edit(self, ctx: commands.Context, command: str, *, response=None, cooldowns: Mapping[str, int] | None = None, ask_for: bool = True):
...
class CustomCommands(commands.Cog):
    ...
    async def cc_search(self, ctx: commands.Context, *, query):
        ...
        **Arguments:**
        ...
        accepted = [(entry[0], cc_commands[entry[0]]) for entry in extracted if entry[1] > 60]
        if not accepted:
    ...
    @checks.mod_or_permissions(administrator=True)
    async def cc_cooldown(self, ctx, command: str.lower, cooldown: int | None = None, *, per: str.lower = "member"):
        ...
        Examples
        --------
            - `;customcom cooldown pingrole`
            - `;customcom cooldown yourcommand 30`
            - `;cc cooldown mycommand 30 guild`
    ...
    @checks.mod_or_permissions(administrator=True)
    async def cc_edit(self, ctx, command: str.lower, *, text: str | None = None):
    ...
    async def cc_list(self, ctx: commands.Context):
        ...
        if not cc_dict:
            await ctx.send(
                _("There are no custom commands in this server. Use `{command}` to start adding some.").format(command=f"{ctx.clean_prefix}customcom create"),
            )
            return
    ...
    async def cc_command(self, ctx, *cc_args, raw_response, **cc_kwargs) -> None:
        ...
        if results := re.findall(r"{((\d+)[^.}]*(\.[^:}]+)?[^}]*)\}", raw_response):
    ...
    def prepare_args(raw_response) -> Mapping[str, Parameter]:
        ...
        for arg in args:
            ...
            if anno is not Parameter.empty and fin[index].annotation is not Parameter.empty and anno != fin[index].annotation:
                raise ArgParseError(
                    _('Conflicting colon notation for argument {index}: "{name1}" and "{name2}".').format(
                        index=index + low,
