...
class NotFound(Exception):
    """Something was not found in the WarnSystem data.

    The meaning of this exception depends of what you called, it may be
    a missing WarnSystem channel.
...
class MissingMuteRole(Exception):
    """You requested a mute warn but the mute role doesn't exist.
...
class MissingPermissions(Exception):
    ...
    This is raised instead of :class:`discord.errors.Forbidden` to
    prevent a useless API call, we check the bot's permissions before
    calling.
...
class MemberTooHigh(Exception):
    """The member to take action on is above the bot in the guild's role
    hierarchy.
...
class NotAllowedByHierarchy(Exception):
    """The bot is set to respect the role hierarchy; the moderator requested a
    warn against someone equal or higher than him in the hierarchy, which is
    not allowed by Discord permissions rules.
...
class LostPermissions(Exception):
    r"""The bot lost a permission he had.

    This can be the permission to send messages in the modlog channel or
    use\ the mute role.
...
class SuicidePrevention(Exception):
    ...
    Warning Melanie will cause issues and is not designed for this.
