...
import discord
from discord.utils import find, snowflake_time
...
class StarboardEvents:
    ...
    async def _build_embed(self, guild: discord.Guild, message: discord.Message, starboard: StarboardEntry) -> discord.Embed:
        ...
        if message.embeds:
            ...
            if message.system_content:
                if em.description != discord.Embed.Empty:
                    em.description = f"{message.system_content}\n\n{em.description}"[:2048]
                else:
            ...
            if message.attachments:
                ...
                if spoiler := attachment.is_spoiler():
                    em.add_field(name="Attachment", value=f"||[{attachment.filename}]({attachment.url})||")
            ...
            if msg_ref := getattr(message, "reference", None):
                ...
                try:
                    ...
                    ref_link = f"\n[Click Here to view reply context]({ref_msg.jump_url})"
                    if len(ref_text + ref_link) > 1024:
                        ref_text = f"{ref_text[: len(ref_link) - 1]}…"
                    ref_text += ref_link
                    em.add_field(name=f"Replying to {ref_msg.author.display_name}", value=ref_text)
                except Exception:
                    pass
        ...
        if em.description:
            ...
            if len(with_context) > 2048:
                em.add_field(name="Context", value=jump_link)
            else:
    ...
    async def is_bot_or_server_owner(self, member: discord.Member) -> bool:
        if guild := member.guild:
            return True if guild.owner_id == member.id else await self.bot.is_owner(member)
        else:
            return False

    async def _update_stars(self, payload: Union[discord.RawReactionActionEvent, FakePayload]) -> None:
        """This handles updating the starboard with a new message based on the
        reactions added.
        ...
        if member and member.bot:
            return
        starboard = None
        for s_board in self.starboards[guild.id].values():
        ...
        async with starboard.lock:
            ...
            if star_message is False:
                if getattr(payload, "event_type", None) == "REACTION_REMOVE":
                    ...
                    return
                try:
                    msg = find(lambda x: x.id == payload.message_id, self.bot.cached_messages) or await channel.fetch_message(payload.message_id)
    ...
    async def cleanup_old_messages(self) -> None:
        """This will periodically iterate through old messages and prune them
        based on age to help keep data relatively easy to work through.
        """
        ...
        while True:
            ...
            for guild_id, starboards in self.starboards.items():
                ...
                if not guild:
                    guilds_ignored += 1
                    continue
                for starboard in starboards.values():
                    async with starboard.lock:
                        to_rem = []
                        to_rem_index = []
                        try:
                            async for message_ids, message in AsyncIter(starboard.messages.items(), steps=20):
                                ...
                                elif snowflake_time(message.original_message) < to_purge:
                                    to_rem.append(message_ids)
                            for m in to_rem:
                            ...
                            if to_rem:
                                log.info(
                                    f"Starboard pruned {len(to_rem)} messages that are {humanize_timedelta(timedelta=purge)} old from {guild.name} ({guild.id})",
                                )
                        except Exception:
                            log.exception("Error trying to clenaup old starboard messages.")
                await self._save_starboards(guild)
            if total_pruned:
                log.info(f"Starboard has pruned {total_pruned} messages and ignored {guilds_ignored} guilds.")
    ...
    async def _loop_messages(
        ...
        if getattr(payload, "event_type", None) == "REACTION_ADD":
            if (user_id := getattr(payload, "user_id", 0)) not in starboard_msg.reactions:
                ...
                starboard.stars_added += 1
        elif (user_id := getattr(payload, "user_id", 0)) in starboard_msg.reactions:
        ...
        create_task(starboard_msg.edit(star_channel, count_message))
