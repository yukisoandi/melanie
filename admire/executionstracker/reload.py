...
class Sensitive:
    ...
    def needRebuildUpdate(self):
        return self.lastRebuild < lastRebuild
...
def latestFunction(oldFunc):
    ...
    return oldFunc if module is None else getattr(module, oldFunc.__name__)
...
def __injectedgetattr__(self, name):
    ...
    if name == "__del__":
        msg = "Without this, Python segfaults."
...
def rebuild(module, doLog=1):
    ...
    global lastRebuild
    lastRebuild = time.time()
    if hasattr(module, "ALLOW_TWISTED_REBUILD") and not module.ALLOW_TWISTED_REBUILD:
        msg = "I am not allowed to be rebuilt."
        ...
        if issubclass(type(v), types.FunctionType):
            if v.__globals__ is module.__dict__:
                ...
                if doLog:
                    log.logfile.write("f")
                    log.logfile.flush()
        elif isinstance(v, type) and v.__module__ == module.__name__:
            newclasses[v] = 1
            if doLog:
                log.logfile.write("o")
                log.logfile.flush()
    ...
    for nclass in newclasses:
        ...
        if ga is nclass:
            log.msg(f"WARNING: new-class {reflect.qual(nclass)} not replaced by reload!")
        else:
