...
from melaniebot.core.commands import NoParseOptional as Optional
...
class NitroRole(commands.Cog):
    ...
    def __init__(self, bot: Melanie) -> None:
        self.bot = bot
        self.config = Config.get_conf(self, identifier=176070082584248320, force_registration=True)
        self.config.register_guild(role_id=None, channel_id=None, message_templates=[], unassign_on_boost_end=False)
    ...
    async def get_guild_data(self, guild: discord.Guild) -> GuildData:
        ...
        data = self.guild_cache[guild.id] = GuildData(guild.id, self.config, **guild_settings)
        return data
    ...
    async def nitrorole_unassign_on_boost_end(self, ctx: GuildContext, enabled: Optional[bool] = None) -> None:
        """Set if booster role should be unassigned when someone stops boosting
        server.
        ...
        if enabled is None:
            if guild_data.unassign_on_boost_end:
                message = "Bot unassigns booster role when user stops boosting the server."
            else:
                message = "Bot doesn't unassign booster role when user stops boosting the server."
        ...
        if enabled:
            message = "Bot will now unassign booster role when user stops boosting the server."
        else:
            message = "Bot will no longer unassign booster role when user stops boosting the server."
        await ctx.send(message)

    @nitrorole.command(name="autoassignrole")
    async def nitrorole_autoassignrole(self, ctx: GuildContext, *, role: Optional[discord.Role] = None) -> None:
        ...
        if role is None:
            ...
            await ctx.send("Role will not be autoassigned anymore when someone boosts server.")
            return
        if guild.owner_id != ctx.author.id and role > ctx.author.top_role and not await self.bot.is_owner(ctx.author):
    ...
    async def nitrorole_channel(self, ctx: GuildContext, channel: Optional[discord.TextChannel] = None) -> None:
    ...
    async def nitrorole_addmessage(self, ctx: GuildContext, *, message: str) -> None:
        ...
        To set it, use `;nitrorole setimage`

        """
        guild = ctx.guild
        guild_data = await self.get_guild_data(guild)
        template = await guild_data.add_message(message)
        content = template.safe_substitute(mention=ctx.author.mention, username=ctx.author.display_name, server=guild.name, count="2", plural="s")
        ...
        if filename is not None:
            ...
            if channel is not None and not channel.permissions_for(guild.me).attach_files:
                warning = "WARNING: Bot doesn't have permissions to send images in channel used for new booster messages.\n\n"

            if not ctx.channel.permissions_for(guild.me).attach_files:
                await ctx.send(f'{warning}New booster message set.\nI wasn\'t able to send test message here due to missing "Attach files" permission.')
                return

            file = discord.File(str(filename))
        await ctx.send(f"{warning}New booster message set, sending a test message here...")
    ...
    async def nitrorole_removemessage(self, ctx: GuildContext) -> None:
        ...
        try:
            await self.bot.wait_for("message", check=lambda m: pred(m) and cast(int, pred.result) >= 1, timeout=30)
        except TimeoutError:
    ...
    async def nitrorole_listmessages(self, ctx: GuildContext) -> None:
        ...
        guild_data = await self.get_guild_data(ctx.guild)
        if not guild_data.messages:
            await ctx.send("This guild doesn't have any new booster message templates set.")
            return
    ...
    async def nitrorole_setimage(self, ctx: GuildContext) -> None:
        ...
        channel = guild.get_channel(channel_id) if channel_id is not None else None
        if channel is not None and not channel.permissions_for(guild.me).attach_files:
            await ctx.send("WARNING: Bot doesn't have permissions to send images in channel used for new booster messages.\n\nImage set.")
        else:
    ...
    async def cog_disabled_in_guild(self, guild: Optional[discord.Guild]) -> bool:
        # compatibility layer with Melanie 3.3.10
        func: Optional[Callable[[commands.Cog, Optional[discord.Guild]], Awaitable[bool]]] = getattr(self.bot, "cog_disabled_in_guild", None)
        return False if func is None else await func(self, guild)
    ...
    def get_role_to_assign(self, guild: discord.Guild, guild_data: GuildData) -> Optional[discord.Role]:
        role_id = guild_data.role_id
        if role_id is None:
            return None
        role = guild.get_role(role_id)
        if role is None:
            log.info("Role with ID {} can't be found in guild with ID {}.", role_id, guild.id)
            return None
        if role >= guild.me.top_role:
            log.info("Role with ID {} (guild ID: {}) is higher in hierarchy than any bot's role.", role_id, guild.id)
            return None
        return role

    async def maybe_assign_role(self, guild_data: GuildData, member: discord.Member) -> None:
        ...
        try:
            ...
            log.exception("Bot was unable to add role with ID {} (guild ID: {}) to member with ID {}.", role.id, role.guild.id, member.id)
            await self.config.guild(member.guild).role_id.set(None)
            log.info("Removed that role from configuration")

    async def maybe_unassign_role(self, guild_data: GuildData, member: discord.Member) -> None:
        ...
        if role not in member.roles:
            return
        try:
            await member.remove_roles(role, reason="No longer nitro booster - role unassigned")
        except discord.Forbidden:
            await self.config.guild(member.guild).role_id.set(None)

            log.exception("Bot was unable to remove role with ID {} (guild ID: {}) from member with ID {}.", role.id, role.guild.id, member.id)

            log.info("Removed that role from configuration")

    async def maybe_announce(self, guild_data: GuildData, member: discord.Member) -> None:
        channel_id = guild_data.channel_id
        ...
        if channel is None:
            log.exception("Channel with ID {} can't be found in guild with ID {}.", channel_id, guild.id)
            await self.config.guild(guild).channel_id.set(None)
            log.info("Disabled the notification for that channel.")
            return
        ...
        if filename is not None:
            if channel.permissions_for(guild.me).attach_files:
                ...
                log.info('Bot doesn\'t have "Attach files" in channel with ID {} (guild ID: {})', channel_id, guild.id)
        ...
        try:
            ...
            log.exception("Bot can't send messages in channel with ID {} (guild ID: {})", channel_id, guild.id)
            await self.config.guild(guild).channel_id.set(None)
            log.info("Disabled the notification for that channel.")
            return
