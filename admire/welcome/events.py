...
class Events:
    ...
    async def convert_parms(self, member: Union[discord.Member, list[discord.Member]], guild: discord.Guild, msg: str, is_welcome: bool) -> str:
        ...
        username = humanize_list(member) if isinstance(member, list) else str(member)
        for result in results:
            if int(result[1]) == 1:
                ...
                if isinstance(member, discord.Member):
                    ...
                    params = [self.transform_arg(result[0], result[2], m) for m in member]
                    param = humanize_list(params) if len(params) > 1 else params[0]
            ...
            if bad_name:
                for word in bad_name:
                    ...
                    if isinstance(member, list):
                        raw_response = re.sub("|".join(m.mention for m in member), filter_setting, raw_response)
                    else:
                        raw_response = re.sub(rf"(?i){member.mention}", filter_setting, raw_response)
        if await self.config.guild(guild).JOINED_TODAY() and is_welcome:
            raw_response = f"{raw_response}\n\n{user_count} users joined today!"
        return raw_response

    async def make_embed(self, member: Union[discord.Member, list[discord.Member]], guild: discord.Guild, msg: str, is_welcome: bool) -> discord.Embed:
        ...
        if EMBED_DATA["footer"]:
            em.set_footer(text=await self.convert_parms(member, guild, EMBED_DATA["footer"], False))
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        ...
        has_filter = self.bot.get_cog("Filter")
        filter_setting = await self.config.guild(guild).FILTER_SETTING()
        if has_filter and filter_setting is None and await has_filter.filter_hits(member.name, guild):
    ...
    async def bot_welcome(self, member: discord.Member, guild: discord.Guild) -> None:
        ...
        if bot_role:
            try:
                ...
                await member.add_roles(role, reason="Automatic Bot Role")
            except Exception:
                log.exception(f"welcome.py: unable to add  a role.  {bot_role} {member}")
            else:
                log.debug(f"welcome.py: added {str(role)} role to bot, {str(member)}")
        if bot_welcome:
            ...
            if is_embed and channel.permissions_for(guild.me).embed_links:
                ...
                await channel.send(await self.convert_parms(member, guild, bot_welcome, False), **sanitize)

    async def get_welcome_channel(self, member: Union[discord.Member, list[discord.Member]], guild: discord.Guild) -> Optional[discord.TextChannel]:
        ...
        if channel is None:  # complain even if only whisper
            if not only_whisper:
                log.info(f"welcome.py: Channel not found. It was most likely deleted. User joined: {str(member)}")
            return None
        # we can stop here
    ...
    async def send_member_join(self, member: Union[discord.Member, list[discord.Member]], guild: discord.Guild) -> None:
        if await self.config.guild(guild).JOIN_ROLE_ID():
            return
        ...
        if is_embed and channel.permissions_for(guild.me).embed_links:
            ...
            if await self.config.guild(guild).EMBED_DATA.mention():
                if await self.config.guild(guild).GROUPED():
                    ...
                    save_msg = await channel.send(humanize_list([m.mention for m in members]), embed=em, delete_after=delete_after, **sanitize)
                else:
                    member = cast(discord.Member, member)
                    save_msg = await channel.send(str(member.mention), embed=em, delete_after=delete_after, **sanitize)
                    await self.bot.redis.set(_warden_key, save_msg.id, ex=3000)

            else:
                save_msg = await channel.send(embed=em, delete_after=delete_after, **sanitize)
                await self.bot.redis.set(_warden_key, save_msg.id)
        else:
            save_msg = await channel.send(await self.convert_parms(member, guild, msg, True), delete_after=delete_after, **sanitize)
            await self.bot.redis.set(_warden_key, save_msg.id)
    ...
    async def on_member_remove(self, member: discord.Member) -> None:
        ...
        if channel is None:  # complain even if only whisper
            log.info(f"welcome.py: Channel not found in {guild}. It was most likely deleted.")
            await self.config.guild(guild).LEAVE_ON.set(False)
            log.warning(f"Disabled leave messages for {guild}")
        ...
        if not channel.permissions_for(guild.me).send_messages:
            ...
            if is_embed and channel.permissions_for(guild.me).embed_links:
                em = await self.make_embed(member, guild, msg, False)
                if await self.config.guild(guild).EMBED_DATA.mention():
                    save_msg = await channel.send(member.mention, embed=em, delete_after=delete_after, **sanitize)
                else:
                    save_msg = await channel.send(embed=em, delete_after=delete_after)
            else:
                save_msg = await channel.send(await self.convert_parms(member, guild, msg, False), delete_after=delete_after, **sanitize)
        if save_msg is not None:
            await self.config.guild(guild).LAST_GOODBYE.set(save_msg.id)

    async def send_testing_msg(self, ctx: commands.Context, bot: bool = False, msg: str = None, leave: bool = False) -> None:
        ...
        if channel is None and whisper_settings not in ["BOTH", True]:
            msg = "I can't find the specified channel. It might have been deleted."
            await ctx.send(msg)
            return
        if channel is None:
            await ctx.send("`Sending a testing message to " + "` DM", **sanitize)
        else:
            await ctx.send(f"`Sending a testing message to `{channel.mention}", **sanitize)
        if not bot and guild_settings["WHISPER"]:
            if is_embed:
                ...
                await ctx.author.send(await self.convert_parms(member, guild, rand_msg, is_welcome), delete_after=60, **sanitize)
        ...
        if bot or whisper_settings is not True:
            ...
            if is_embed and channel.permissions_for(guild.me).embed_links:
                ...
                if await self.config.guild(guild).EMBED_DATA.mention():
                    if guild_settings["GROUPED"]:
                        await channel.send(humanize_list([m.mention for m in member]), embed=em, delete_after=60)
                    else:
                        await channel.send(member.mention, embed=em, delete_after=60, **sanitize)
                else:
                    await channel.send(embed=em, delete_after=60, **sanitize)
            else:
                await channel.send(await self.convert_parms(member, guild, rand_msg, is_welcome), delete_after=60, **sanitize)
