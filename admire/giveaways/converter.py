...
class Args(Converter):
    async def convert(self, ctx, argument) -> dict[str, Any]:
        ...
        try:
            ...
            msg = "Could not parse flags correctly, ensure flags are correctly used."
            raise BadArgument(msg) from error

        if not vals["prize"]:
            msg = "You must specify a prize. Use `--prize` or `-p`"
            raise BadArgument(msg)  #

        if not any([vals["duration"], vals["end"]]):
            msg = "You must specify a duration or end date. Use `--duration` or `-d` or `--end` or `-e`"
            raise BadArgument(msg)
        ...
        for role in vals["multi-roles"]:
            try:
                ...
                msg = f"The role {role} does not exist within this server."
        ...
        for role in vals["blacklist"]:
            try:
                ...
                msg = f"The role {role} does not exist within this server."
                raise BadArgument(msg) from err
        ...
        for role in vals["mentions"]:
            ...
            except BadArgument as exception:
                msg = f"The role {role} does not exist within this server."
                raise BadArgument(msg) from exception
        ...
        if vals["levelreq"] or vals["repreq"]:
            ...
            if not cog:
                msg = "Leveler cog not loaded."
                raise BadArgument(msg)
            if not hasattr(cog, "db"):
                msg = "This may be the wrong leveling cog. Ensure you are using Fixators."
                raise BadArgument(msg)

        if vals["tatsu_level"] or vals["tatsu_rep"]:
            token = await ctx.bot.get_shared_api_tokens("tatsumaki")
            if not token.get("authorization"):
                msg = f"You do not have a valid Tatsumaki API token. Check `{ctx.clean_prefix}gw integrations` for more info."
                raise BadArgument(msg)

        if vals["amari_level"] or vals["amari_weekly_xp"]:
            token = await ctx.bot.get_shared_api_tokens("amari")
            if not token.get("authorization"):
                msg = f"You do not have a valid Amari API token. Check `{ctx.clean_prefix}gw integrations` for more info."
                raise BadArgument(msg)

        if (vals["multi"] or vals["multi-roles"]) and not (vals["multi"] and vals["multi-roles"]):
            msg = "You must specify a multiplier and roles. Use `--multiplier` or `-m` and `--multi-roles` or `-mr`"
            raise BadArgument(msg)
        ...
        if (
            ...
            msg = "You do not have permission to mention everyone. Please ensure the bot and you have `Mention Everyone` permission."
            raise BadArgument(msg)
        ...
        if vals["emoji"]:
            ...
            try:
                ...
                msg = "Invalid emoji."
                raise BadArgument(msg) from an_exception
            if custom:
        ...
        if vals["duration"]:
            ...
            except BadArgument as an_error:
                msg = "Invalid duration. Use `--duration` or `-d`"
                raise BadArgument(msg) from an_error
        else:
            try:
                ...
                msg = "Invalid end date. Use `--end` or `-e`. Ensure to pass a timezone, otherwise it defaults to UTC."
                raise BadArgument(msg) from an_error
