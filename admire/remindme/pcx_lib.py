...
async def type_message(destination: discord.abc.Messageable, content: str, **kwargs) -> discord.Message:
    ...
    Will send a typing indicator, wait a variable amount of time based
    on the length of the text (to simulate typing speed), then send the
    message.
...
class Perms:
    ...
    def __init__(self, overwrites: dict[Union[discord.Role, discord.Member], discord.PermissionOverwrite] = None) -> None:
        """Init."""
        self.__overwrites = {}
        self.__original = {}
        if overwrites:
    ...
    def set(self, target: Union[discord.Role, discord.Member], permission_overwrite: discord.PermissionOverwrite) -> None:
        """Set the permissions for a target."""
        if not permission_overwrite.is_empty():
            self.__overwrites[target] = discord.PermissionOverwrite().from_pair(*permission_overwrite.pair())

    def update(self, target: Union[discord.Role, discord.Member], perm: Union[str, list[str]], value: Union[bool, None]) -> None:
    ...
    @property
    def modified(self):
        """Check if current overwrites are different than when this object was
        first initialized.
        """
