...
from melaniebot.core.commands.requires import PrivilegeLevel
...
class DashboardRPC:
    ...
    This class contains the basic RPC functions, that don't belong to
    any other cog
    ...
    async def build_cmd_list(self, cmd_list: list[commands.Command], details: bool = True, do_escape: bool = True):
        try:
            final = []
            async for cmd in AsyncIter(cmd_list):
                if details:
                    ...
                    try:
                        if do_escape:
                            details = {"name": escape(f"{cmd.qualified_name} {cmd.signature}"), "desc": escape(cmd.short_doc or ""), "subs": []}
                        else:
                            details = {"name": f"{cmd.qualified_name} {cmd.signature}", "desc": cmd.short_doc or "", "subs": []}
                    except ValueError:
                        continue
                    ...
                    if isinstance(cmd, commands.Group):
                        final += await self.build_cmd_list(cmd.commands, details=False, do_escape=do_escape)
            return final
        except:
            log.exception("Unable to build cmd list")
            raise
    ...
    async def notify_owners_of_blacklist(self, ip) -> None:
        async with self.cog.config.blacklisted() as data:
            data.append(ip)
        await self.bot.send_to_owners(f"[Dashboard] Detected suspicious activity from IP {ip}.  They have been blacklisted.")

    @rpccheck()
    @alru_cache(ttl=60)
    async def get_variables(self) -> dict[str, float]:
        ...
        returning["last_set"] = time.time()

        return returning

    @rpccheck()
    @alru_cache(ttl=30)
    async def get_secret(self) -> dict[str, Any]:
            ...
            if name in self.cog_info_cache:
                ...
                if installed:
                    ...
                    try:
                        ...
                        repo = "Unknown"
                elif cog.__module__.startswith("melaniebot."):
                    author = "Melanie"
                    repo = "https://melaniebot.net"
                self.cog_info_cache[name] = {"author": author, "repo": repo}
            returning.append({"name": escape(name or ""), "desc": escape(cog.__doc__ or ""), "cmds": cmds, "author": escape(author or ""), "repo": repo})
    ...
    async def get_server(self, userid: int, serverid: int):
        ...
        for i, p in enumerate(parts):
            parts[i] = p.upper() if p in ["eu", "us", "vip"] else p.title()
        region = " ".join(parts)
