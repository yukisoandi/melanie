...
class DashboardWebserverMixin(MixinMeta):
    @checks.is_owner()
    @dashboard.group()
    async def webserver(self, ctx) -> None:
    ...
    async def clients(self, ctx: commands.Context):
        """View connected RPC clients.  These could be dashboard or other
        processes.
    ...
    async def blacklist_view(self, ctx: commands.Context) -> None:
        """See blacklisted IP addresses."""
        blacklisted = await self.config.blacklisted() or ["None"]
        await ctx.author.send(f"The following IP addresses are blocked: {humanize_list(blacklisted)}")

    @blacklist.command(name="remove")
    async def blacklist_remove(self, ctx: commands.Context, *, ip) -> None:
    ...
    async def blacklist_add(self, ctx: commands.Context, *, ip) -> None:
    ...
    @webserver.command()
    async def redirect(self, ctx: commands.Context, redirect: str) -> None:
    ...
    async def clientid(self, ctx: commands.Context, cid: int) -> None:
