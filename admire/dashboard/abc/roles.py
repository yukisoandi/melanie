...
class DashboardRolesMixin(MixinMeta):
    ...
    async def roles(self, ctx: commands.Context) -> None:
        """Customize the roles that have permission to certain parts of the
        dashboard.
        """

    @roles.command()
    async def create(self, ctx: commands.Context, role: discord.Role, *permissions) -> None:
        ...
        for p in permissions:
            if p.lower() in HUMANIZED_PERMISSIONS and p.lower() not in disallowed:
        ...
        if await ctx.embed_requested():
            e = discord.Embed(
                ...
                description=f"**Permissions assigned**: {humanize_list(list(map(inline, assigning)))}\n**Permissions unidentified**: {humanize_list(list(map(inline, missing or ['None'])))}",
    ...
    async def edit(self, ctx: commands.Context, role: discord.Role, *permissions):
        ...
        if not ro["perms"]:
            await ctx.send(f"Failed to edit role. If you wish to remove all permissions from the role, please use `{ctx.clean_prefix}dashboard roles delete`.")
            return
        ...
        if await ctx.embed_requested():
            e = discord.Embed(
                ...
                description=f"**Permissions added**: {humanize_list(list(map(inline, added or ['None'])))}\n**Permissions removed**: {humanize_list(list(map(inline, removed or ['None'])))}\n**Permissions unidentified**: {humanize_list(list(map(inline, missing or ['None'])))}\n",
                color=(await ctx.embed_color()),
            )
            await ctx.send(embed=e)
        else:
            await ctx.send(
                f"**Successfully edited role**```css\n[Permissions added]: {humanize_list(added or ['None'])}\n[Permissions removed]: {humanize_list(removed or ['None'])}\n[Permissions unidentified]: {humanize_list(missing or ['None'])}```",
            )
    ...
    async def roles_list(self, ctx: commands.Context):
        ...
        roles = [ctx.guild.get_role(role["roleid"]).mention for role in data if ctx.guild.get_role(role["roleid"])]
        if not roles:
    ...
    async def perms(self, ctx: commands.Context) -> None:
        ...
        if await ctx.embed_requested():
            ...
            description = "".join(f"[{key.title()}]: {value}\n" for key, value in HUMANIZED_PERMISSIONS.items() if key not in disallowed)
