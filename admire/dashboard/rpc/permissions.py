...
import discord
from melaniebot.cogs.permissions.converters import (
    CogOrCommand,
...
class DashboardRPC_Permissions:
                ...
                if model_rules is not None:
                    for target, rule in model_rules.items():
                        ...
                        if target != "default":
                            ...
                            saving = {"type": objtype, "name": escape(name), "id": str(target), "permission": "allowed" if rule else "denied"}
                        else:
                            saving = {"type": "Default", "permission": "allowed" if rule else "denied"}
    ...
    async def fetch_guild_targets(self, guild: discord.Guild, member: discord.Member):
        ...
        async for user in AsyncIter(guild.members, steps=100):
            data["USERS"].append((str(user.id), escape(f"{user.display_name}#{user.discriminator}")))

        async for role in AsyncIter(guild.roles, steps=100):
            data["ROLES"].append((str(role.id), escape(role.name)))

        async for channel in AsyncIter(guild.channels, steps=100):
    ...
    @rpccheck()
    @permcheck("Permissions", ["permissions"])
    async def fetch_cog_commands(self, guild: discord.Guild, member: discord.Member) -> dict[str, Any]:
        return {"COGS": list(self.bot.cogs.keys()), "COMMANDS": await self.cog.rpc.build_cmd_list(self.bot.commands, details=False)}
    ...
    async def add_rule(
        ...
        target: int,
        command: str,
    ) -> Union[dict[str, int], dict[str, Union[int, str]]]:
        ...
        await cog._add_rule(rule=cast(bool, allow_or_deny), cog_or_cmd=cog_or_command, model_id=who_or_what.id, guild_id=guild.id)

        return {"status": 1}

    @rpccheck()
    @permcheck("Permissions", ["permissions"])
    async def add_default_rule(self, guild: discord.Guild, member: discord.Member, t: str, command: str) -> Union[dict[str, int], dict[str, Union[int, str]]]:
        ctx = FakePermissionsContext(self.bot, guild)
        cog = self.bot.get_cog("Permissions")
        try:
            cog_or_command = await CogOrCommand.convert(ctx, command)
        ...
        await cog._set_default_rule(rule=cast(bool, allow_or_deny), cog_or_cmd=cog_or_command, guild_id=guild.id)

        return {"status": 1}

    @rpccheck()
    @permcheck("Permissions", ["permissions"])
    async def remove_rule(self, guild: discord.Guild, member: discord.Member, target: int, command: str) -> Union[dict[str, int], dict[str, Union[int, str]]]:
        ctx = FakePermissionsContext(self.bot, guild)
        cog = self.bot.get_cog("Permissions")
        try:
            cog_or_command = await CogOrCommand.convert(ctx, command)
        ...
        await cog._remove_rule(cog_or_cmd=cog_or_command, model_id=who_or_what.id, guild_id=guild.id)

        return {"status": 1}

    @rpccheck()
    @permcheck("Permissions", ["permissions"])
    async def remove_default_rule(self, guild: discord.Guild, member: discord.Member, command: str) -> Union[dict[str, int], dict[str, Union[int, str]]]:
        ctx = FakePermissionsContext(self.bot, guild)
        cog = self.bot.get_cog("Permissions")
        try:
            cog_or_command = await CogOrCommand.convert(ctx, command)
        ...
        await cog._set_default_rule(rule=None, cog_or_cmd=cog_or_command, guild_id=guild.id)
